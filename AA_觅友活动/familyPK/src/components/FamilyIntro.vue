  ...
  <div class="family__intro">
    ...
    <div class="list__container">
      ...
      <div class="guard_title">
        {{ lang.menber_list.replace('%s', list.length) }}
      </div>
      <div class="list__main">
        <div class="cont__list" v-for="(item, index) in list" :key="index">
          <img
            class="avatar"
            :class="{boss_avatar: item.patriarch}"
            @click="goHomepage(item.uid)"
            v-lazy="item.avatar"
          />
          <div class="info_middle">
            <div class="nick">{{ item.nick }}</div>
            <img v-if="item.patriarch" class="patriarch" :src="images.patriarch" alt="">
          </div>
          <div class="score">{{ item.score }}</div>
        </div>
      </div>
      <div class="btn" @click="showApplyDialog = true" v-if="!familyRank.familyId && list.length < initData.maxNumber">{{ lang.join_family }}</div>
      <div class="btn" @click="showFriendDialog = true" v-if="familyRank.familyId == familyId && list.length < initData.maxNumber">{{ lang.invite_friend }}</div>
