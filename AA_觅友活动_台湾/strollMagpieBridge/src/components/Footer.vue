  ...
  <div class="footerBar" v-if="astState">
    <div class="acrStatus" :class="{bg:astState!=1}">
      ...
      <div class="actIng" v-if="astState === 3" :class="['rank' + owner.rank]">
        ...
        <div class="userAv">
          <img v-lazy="owner.avatar" alt="" class="man">
          <img v-lazy="owner.cp_avatar" alt="" class="woman">
        </div>
        <div class="userNick">
          <div class="man">{{owner.nick}}</div>
          <div class="woman">{{owner.cp_nick}}</div>
        </div>
        <div class="score">
          <em class="tips">{{lang.scoreName}}</em>
          <em class="score">{{owner.score}}</em>
        </div>
      </div>
      <!-- noFriend -->
...
export default {
  ...
  computed: {
    ...mapState(['activity', 'owner', 'tab', 'groupsUserMsg']),
    astState () {
