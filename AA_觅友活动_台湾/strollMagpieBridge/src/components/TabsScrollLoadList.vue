        ...
        <ul>
          <li v-for="(item,index) in top3" :key="index" :class="'rank' + item.rank">
            ...
            <div class="userAv">
              ...
              <div class="hear">
                <em class="tips">{{lang.scoreName}}</em>
...
export default {
  ...
  computed: {
    ...mapState(['rankGroups', "isShare", "actStatus", "inited", "second", "total", "day", 'activity']),
    rankKey () {
    ...
    rankApi () {
      if (this.isShare) {
        var dayApi = `/index.php?action=strollMagpieBridge.rank&date={date}&from={from}`;
        return dayApi.replace('{date}', this.rankKey == 'total' ? 0 : 1)
      } else {
        var dayApi = `/index.php?action=strollMagpieBridge.rank&date={date}&uid={uid}&token={token}&from={from}`;
