...
export default {
  ...
  computed: {
    ...mapState(['rankGroups', "isShare", "inited", "second", "total", "day", 'activity']),
    rankKey () {
    ...
    rankApi () {
      if (this.isShare) {
        ...
        var dayApi = `/index.php?action=summerCarnival.rank&signture=innerserver&from={from}&type={type}&uid={uid}&token={token}`;
    ...
    rank () {
      ...
      if (this.activity.seconds) {
        this.downTimeGo('time' + this.rankKey, this.rankKey == 'total' ? this.activity.seconds : this.activity.today_seconds)
      }
      return rankConf;
    }
  },
