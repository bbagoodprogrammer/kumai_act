export default {
    title: "神豪來了",
    noStart: "活動未開始",
    actEnd: "活動已結束",
    noNick: "神豪擂主",
    scoreName: "豪爽值",
    ruleTips: "規則&獎勵",
    synthesis: "合成",
    qureyMsg: "是否合成%n*1？",
    cancel: "我再想想",
    loading: "加載中...",
    noData: "暫無數據",
    roolMsg: "太豪爽！【%n】送出「%s」大禮物",
    //PeopleTotal
    volTips: "vol.%v頭號神豪",
    todayTitle: "今日神豪挑戰者",
    top3Tips: "今日神豪挑戰者指，今日神豪榜的前三名",
    //MagicLand
    synthesis: "合成",
    synthesisSucTips: 
    "恭喜合成【%n*%s】,禮物已發放到您的背包，請查收",
    //rank
    rankTab1: "日榜",
    rankTab2: "總榜",
    rank_day: "天",
    rank_hour: "時",
    rank_min: "分",
    rank_second: "秒",
    rankTips:
        "玩家在活動期間內，在房間、私訊、動態中送出任意金幣禮物都可以獲得豪爽值，1金幣=1豪爽值，滿5000豪爽值才可上日榜",
    //rule
    ruleTm: "活動時間：",
    ruleTips1: "1、如何登上神豪榜",
    ruleTips2:
        "-玩家送出任意金幣禮物可獲得神豪值，1金幣=1神豪值，神豪值滿足5000時有機會登上日榜，且有機會爭奪神豪擂主席位",
    ruleTips3:
        "-神豪榜按照玩家神豪值從高到低排序，當分數一致時，先達到的玩家優先，排行榜展示前50名玩家",
    ruleTips4: "2、如何合成禮物",
    ruleTips5:
        "-玩家送出水晶鞋和粉絲牌所需的指定禮物，並滿足一定的數量，即可合成並獲得水晶鞋-特效禮物或者粉絲牌-特效禮物；禮物有效期均為7天，收到後請盡快送出哦",
    ruleTips6:
        "-水晶鞋和粉絲牌特效禮物均可用於贈送給主播，增加主播在#摩天大樓#活動的高度",
    ruleTips7: "3、神豪榜獎勵",
    ruleTips8:
        "總榜第一名：9月神豪-認證（15天）+神豪-勛章（15天）+神豪（金）-頭像框（15天）+鈔票槍-禮物冠名（15天）",
    ruleTips9:
        "總榜第二名：神豪-勛章（10天）+神豪（銀）-頭像框（15天）+星光沙漏-禮物冠名（15天）",
    ruleTips10:
        "總榜第三名：神豪-勛章（7天）+神豪（銅）-頭像框（15天）+啤酒乾杯-禮物冠名（15天）",
    ruleTips11: "4、其他",
    ruleTips12:
        "-活動中，若發現玩家用不正當手段參與活動，小音有權在不事先通知的情況下按情節嚴重對參與玩家、違規者取消上榜資格、封禁賬號、收回該玩家所有獎勵等處罰，包括但不限於：<br />a）活動期間對參賽玩家惡意評論、造謠、影射等<br />b)非法獲得金幣、指定禮物",
    ruleTips13: "-此活動與蘋果公司無關",
    ruleTips14: "*活動最終解釋權歸活動主辦方所有",
    gift1: [
        {
            img: require(`./img/ruleGift/7.png`),
            name: "水晶鞋"
        },
        {
            img: require(`./img/ruleGift/8.png`),
            name: "粉絲牌"
        }
    ],
    gift2: [
        {
            img: require(`./img/ruleGift/1.png`),
            name: "9月神豪-認證"
        },
        {
            img: require(`./img/ruleGift/2.png`),
            name: "神豪-勛章"
        },
        {
            img: require(`./img/ruleGift/3.png`),
            name: "禮物冠名"
        },
        {
            img: require(`./img/ruleGift/4.png`),
            name: "神豪（金）-頭像框"
        },
        {
            img: require(`./img/ruleGift/5.png`),
            name: "神豪（銀）-頭像框"
        },
        {
            img: require(`./img/ruleGift/6.png`),
            name: "神豪（銅）-頭像框"
        }
    ]
};
