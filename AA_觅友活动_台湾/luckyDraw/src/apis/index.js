...
axios.interceptors.request.use(
    async config => {
        ...
        if (url) {
            ...
            const from_pos = getUrlString("from_pos") || "";
            const uid = getUrlString("uid") || "";
            const token = getUrlString("token") || "";
            const target_uid = getUrlString("target_uid") || "";
            url = url
                .replace("{from_pos}", euc(from_pos))
                .replace("{uid}", euc(uid))
                .replace("{token}", euc(token))
                .replace("{target_uid}", euc(target_uid));
        }
...
function getInitInfo() {
    ...
    return get("/index.php?action=luckyDraw.init&uid={uid}&token={token}");
}
...
export { get, post, loadData, getInitInfo, notice, open };
