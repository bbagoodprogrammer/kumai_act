<template>
  <div class="apply__dialog">
    ...
    <div class="apply__main">
      ...
      <div class="apply_des">{{ lang.apply_des.replace("%s", name) }}</div>
      <div class="btn__box">
        <div class="btn" @click="handleCloseDialog">{{ lang.think_again }}</div>
        <div class="btn" @click="handleApply">{{ lang.apply_join }}</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async handleApply() {
      let res = await joinFamily({
        familyId: this.familyId,
      });
      let { response_status, response_data } = res.data;
      if (response_status.code === 0) {
        toast(this.lang.apply_succ);
        this.handleCloseDialog();
      }else if(response_status.code == 30012) {
        let text = dateFormat(this.lang.apply_fail, this.initData.quitFamilyTime * 1000)
        toast(text);
        setTimeout(() => {
          this.handleCloseDialog();
        }, 1500)
