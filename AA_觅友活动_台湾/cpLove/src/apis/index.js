...
axios.interceptors.request.use(
    async config => {
        ...
        if (url) {
            ...
            // 网页不带token参数时，移除请求接口的空token参数，像活动接口才能正常进入分享模式
            if (!token) {
                url = url.replace(/token=&?/i, '');
            }
        }
...
function getInitInfo() {
    ...
    return get('/index.php?action=cpLove.init&uid={uid}&token={token}');
}

//2，通知
function getNotice() {
    return get('/index.php?action=cpLove.notice&uid={uid}&token={token}');
}

//3，拒绝
function reject() {
    return get('/index.php?action=cpLove.reject&uid={uid}&token={token}');
}

//4，接受
function accept() {
    return get('/index.php?action=cpLove.accept&uid={uid}&token={token}');
}

//5，邀请
function invite() {
    return get('/index.php?action=cpLove.invite&uid={uid}&token={token}');
}

//6，展示CP
function showCpInfo() {
    return get('/index.php?action=cpLove.showCpInfo&uid={uid}&token={token}');
}

//7，分享
function getShare() {
    return get('/index.php?action=cpLove.share&uid={uid}');
}
