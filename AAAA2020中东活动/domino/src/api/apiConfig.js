...
function getDefault() {
    if (token) {
        return get(`/dominoes/init.php?token=${token}`);
    } else {
        return get(`/dominoes/init.php`);
    }
}

//註冊
function singUp() {
    return get(`/dominoes/register.php?token=${token}`)
}

//抽獎
function luck() {
    return get(`/dominoes//dayGift.php?token=${token}`)
}
const httpConfig = {
    getDefault,
    singUp,
