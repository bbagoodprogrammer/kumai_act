  ...
  <div class="box">
    ...
    <div class="header">
      <div class="tipsBox" :class="{top:isShare}">
        ...
        <span class="ruleTips ruleTips2" @click="goBar()" v-if="is_admin">{{lang.myAct}}</span>
        <span class="ruleTips ruleTips2" @click="goRank()">{{lang.rank}}</span>
      </div>
    ...
    <div class="mask" v-show="showGiftPup">
      <transition name="slide">
        <div class="giftPup" v-if="showGiftPup">
          ...
          <p v-if="giftStates == 0">متابعة النشاطين الاثنين كل يوم، يمكن الحصول على 30 فول</p>
          <p v-else-if="giftStates == 1">قد وصلت المكافأة 30 فول إلى محفظتك، يرجي التحقق</p>
          <p v-else>يمكن الحصول مرة واحدة فقط، في انتظارك غدا!</p>
          <div class="num" v-if="giftStates == 0">
            {{day_gift.num}}/{{day_gift.limit}}
          </div>
          <div class="btn" @click="closeGift()" v-else>حسنا</div>
        </div>
      </transition>
    </div>
...
export default {
  ...
  methods: {
    ...
    showRules() {
      let regstr = getString('token')
      location.href = `./index5.html?token=${regstr}&is_admin=${this.is_admin}`
    },
    closeRule() {
      this.showRule = false
    },
