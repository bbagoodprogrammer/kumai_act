      ...
      <li @click="gokRoom(item.rid)" v-for="(item,index) in list" :key="index" :class="'list'+item.cover">
        ...
        <div class="actCon">
          ...
          <div class="userMsg">
            ...
            <!-- {{item.is_ktv?"رقم غرفة الغناء":"رقم غرفة الدردشة"}} -->
            <div class="roomMsg"> <em> {{item.rid}} </em></div>
            <!-- {{lang.date}} -->
        ...
        <div class="userTop">
          ...
          <span :class="{ml:item.user.length <=2}" v-else>{{item.nums}}شخص تم المتابعة</span>
        </div>
        <div class="actBtm">
    ...
    <div class="mask" v-show="showActMsg">
      <transition name="slide">
        <div class="actMsg" v-if="showActMsg" :class="'list'+showParty.cover">
          ...
          <!-- <em>{{showParty.is_ktv==1?'غرفة الغناء':'غرفة الدردشة'}}</em> -->
          <div class="time"><span> وقت الحفلة: </span> {{getDate(showParty.stime)}} </div>
          <div class="room">
  ...
  methods: {
    ...
    getDate(time) {
      ...
      if (partyTime == 0) {
        ...
        return `غدا ${getDate(new Date(time * 1000), 1)}`
      } else {
