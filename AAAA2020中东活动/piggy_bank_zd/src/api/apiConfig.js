...
function getDefault() {
    if (token) {
        return get(`/piggy_bank/init.php?token=${token}`);
    } else {
        return get(`/piggy_bank/init.php`);
    }
}

//积分榜
function scoreRank(from) {
    return get(`/piggy_bank/anchorRank.php?from=${from}`);
}

//守护榜
function coutRank(type, from) {
    if (type == "more") {
        return axios.get(`/piggy_bank/guardRank.php?from=${from}`);
    } else {
        return get(`/piggy_bank/guardRank.php?from=${from}`);
    }

}

//积分记录
function getDetail(from, type) {
    if (type == "more") {
        return axios.get(`/piggy_bank/scoreDetail.php?token=${token}&from=${from}`);
    } else {
        return get(`/piggy_bank/scoreDetail.php?token=${token}&from=${from}`);
    }
}
const httpConfig = {
    getDefault,
