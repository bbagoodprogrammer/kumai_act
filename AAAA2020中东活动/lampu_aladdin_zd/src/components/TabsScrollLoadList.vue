<template>
  <div class="rankGroups">
    ...
    <div class="mainTabs" :class="{maxH:mainTab!=2 && mainTab!=1}">
      <div class="tabItem">
        <span class="taskTab" @click.prevent="mainTabClick(2)" :class="{current:mainTab==2}" href="">المهمة اليومية</span>
        <div class="tabs">
          <a @click.prevent="mainTabClick(0)" :class="{current:mainTab==0}" href="">القائمة اليومية</a>
          <a @click.prevent="mainTabClick(1)" :class="{current:mainTab==1}" href="">القائمة الإجمالية</a>
        </div>
      </div>
      <div class="listBg" v-if="mainTab!=2 && mainTab!=1">
        ...
        <div class="dayScore" :class="{mt:tab<nowDay}">
          ...
          <div class="scoreMsg">
            <div class="total">قيمة استدعاء المصباح السحري قد وصلت إلى {{rank.amount}} تراكميا الآن </div>
            <div class="conis" v-if="tab>=nowDay">مستخدمون في مرتبة 100 الأوائل لهم فرصة لتقسيم <em>{{rank.coins}}</em>عملة </div>
            <div class="conis" v-else> مستخدمون في مرتبة 100 الأوائل قد قسموا <em>{{rank.coins}}</em> عملة</div>
          </div>
        </div>
      </div>
      <a @click.prevent="onRefresh" href="" v-if="(tab == nowDay || mainTab == 1) && !isShare && activeityState===1" :style="{transform:'rotate('+rotatePx+'deg)'}" id="refresh"></a>
    </div>
    ...
    <ul v-if="mainTab==0" class="list day">
      <li v-for="(item,index) in rank.list" :key="index" :class="'rank'+item.rank" @click="goUser(item.uid)">
        ...
        <div class="score" :class="{ml:item.coins==0}">قيمة الاستدعاء: <em>{{item.score}}</em></div>
        <!-- -->
        <div class="coins" v-if="item.coins>0">{{item.coins}}عملة</div>
      </li>
    </ul>
    <!-- 总榜 -->
    <ul v-else-if="mainTab==1" class="list total">
      <li v-for="(item,index) in rank.list" :key="index" :class="'rank'+item.rank" @click="goUser(item.uid)">
        ...
        <div class="score">قيمة الاستدعاء: <em>{{item.score}}</em></div>
        <!-- v-if="item.coins>0" -->
    ...
    <div class="listTipsBox" v-if="rank.loading || mainTab==0 && tab > nowDay || rank.none && mainTab==0 && (rank.list.length == 0 && (tab <= nowDay || tab ==='total'))">
      <div v-if="rank.loading" class="scrollLoading">جار التمحيل...</div>
      <div v-if="rank.none && mainTab==0 && (rank.list.length == 0 && (tab <= nowDay || tab ==='total'))" class="scrollNone">
        لا أحدا في القائمة الآن</br>
        المرتبة العليا في انتظارك~
      </div>
      <div v-if="mainTab==0 && tab > nowDay" class="dengdai">يرجي الانتظار!</div>
    </div>
