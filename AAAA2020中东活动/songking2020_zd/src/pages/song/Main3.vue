<template>
  <div class="song">
    <div class="noSong" v-if="!isLength">
      <h5>ليس لديك أعمال يلبي شروط مشاركة في المسابقة حتى الآن</h5>
      <div class="sing" @click="goSing()">يغني</div>
    </div>
    <div class="haSong" v-else>
      <h5>اختر أعمالك الذي يعجبك أكثر لمشاركة في المسابقة</h5>
      <div class="songList">
        <ul>
          <li v-for="(item,index) in songList" :key="index">
            ...
            <span class="joinBtn" @click="choiceSong(item.sid,index)">اختر لمشاركة</span>
          </li>
        </ul>
      </div>
      <div class="goSing" :class="{border:isLength}">
        <p v-if="isLength">لا يوجد أي أعمال يعجبك؟</p>
        <div class="sing" @click="goSing()">يغني</div>
        <span v-if="isLength">لا تنس قيامة بتسجيل في المسابقة بعد الغناء</span>
      </div>
    </div>
    <div class="tipsBox">
      <p class="tips"><i>1.</i> يجب أن ترفع أعمالا عامة على إعلان النشاط بعد تسجيلك.</p>
      <p class="tips"><i>2.</i>يمكن أن ترفع أعمالا عديدة. لا تحصى بيانات الهدايا إلا بعد التسجيل.</p>
      <p class="tips"><i>3.</i>إذا تحذف عملك المشترك، فستحذف نقاط النمو من هذا العمل</p>
    </div>

    <div class="determinePup" v-if="showPup">
      <div class="determinCon">
        <p>هل أنت متأكد من تقديم هذا العمل للمسابقة؟</p>
        <div class="chang">
          <span class="no" @click="hdPup()">إلغاء</span>
          <span class="ok" @click="changed()">تأكيد</span>
        </div>
      </div>
    </div>
    <Loading></Loading>
