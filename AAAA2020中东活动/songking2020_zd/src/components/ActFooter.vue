    ...
    <div class="acrStatus" :class="{noStart:astState == 0,actEd:astState == 2,singUp:astState == 1}" @click="singUp()">
      <div class="actIng" v-if="astState === 3">
        <!-- <div class="total" v-if="tab != 'total'">
          ...
          <div class="commitSongBtn" @click="commitSong()">رفع الأعمال</div>
        </div> -->
        <div class="left" :class="'rank' +nowMsg.rank">
          ...
          <div class="score">
            <div class="lv">Lv.{{nowMsg.level}} <em class="lvScore"> نقاط التألق: {{nowMsg.score}}</em> </div>
            <div class="iconScore">
        ...
        if (this.astState == 1) {
          api.singUp().then(res => {
            if (res.data.response_status.code == 0) {
              ...
              if (top) {
                this.vxc('setToast', {
                  msg: `مبروك عليك ، من يناير إلى أكتوبر 2020   تكون مرتبة مجموعة العملات الذهبية لأعمالك التي حصلت عليها هي ${top[0]}، فستحصل على ${top[1]}  قيمة السحر في قائمة المطربين السحريين لهذا النشاط`,
                })
              }
            } else {
