  ...
  <div class="actList">
    ...
    <ul>
      <li @click="gokRoom(item.rid)" v-for="(item,index) in list" :key="index" :class="'list'+item.cover">
        ...
        <div class="title" v-if="item.is_official == 0">{{item.them}}</div>
        <div class="title" v-else>{{item.them}}<span class="titieTips">{{lang.officialAct}}</span> </div>
        <div class="actCon">
          ...
          <div class="userMsg">
            ...
            <div class="roomMsg">{{lang.kRoom}}{{item.rid}} {{lang.date}}{{getDate(item.stime)}}</div>
            <div class="userTop">
              <div class="topImg">
