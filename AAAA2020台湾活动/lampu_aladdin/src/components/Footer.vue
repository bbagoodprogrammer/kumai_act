<template>
  <div class="footer">
    ...
    <div class="list" v-if="showState ===3" :class="'rank'+myMsg.rank">
      ...
      <div class="userMsg">
        <span class="total" v-if="tab=='total'">累計召喚值：<em>{{myMsg.all_score}}</em> <strong>距離上一名還有: <em> {{myMsg.gap}}</em></strong></span>
        <span class="toDay" v-else>今日召喚值: <em>{{myMsg.today_score}}</em> <strong>距離上一名還有: <em> {{myMsg.gap}}</em></strong> </span>
      </div>
    </div>
  </div>
</template>
