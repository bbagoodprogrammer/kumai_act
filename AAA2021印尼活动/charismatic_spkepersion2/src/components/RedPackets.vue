<template>
  <div class="redPactets">
    ...
    <div class="lastOpenPacketPeople">
      <div class="title"> <span v-if="naming.uid" @click="peopleType = 1" :class="{act:peopleType == 2}">Penyanyi yg disponsori angpau di babak terakhir</span> <span @click="peopleType = 2"
          :class="{act:peopleType == 1}">Sponsorku</span>
      </div>
      ...
      <div class="my_vote" v-else>
        <div class="tips">Jumlah angpau yg saya sponsori:{{rate.num}} <span v-if="rate.rank">Rankingku:{{rate.rank}}</span> <span>Ditambah:{{rate.rate}}%</span></div>
        <div class="liner">
          ...
          <vue-seamless-scroll :class-option="optionHover" :data="redPacket.record" class="seamless-warp">
            <ul class="item">
              <li v-for="(item,index) in redPacket.record" :key="index">
                ...
                <strong> <em class="nick">{{item.nick}}</em>dptkan {{item.count}} <img :src="redPacket.img" />hadiah</strong>
              </li>
            </ul>
          </vue-seamless-scroll>
          <div class="share" v-if="packetType == 1" @click="shareAct()">Ingatkan teman ambil angpau</div>
        </div>
      </transition>
    </div>
    <!-- 开紅包结果 -->
    <div class="mask" v-show="packetRes">
      <transition name="slide">
        <div class="packetRes" v-if="packetRes">
          ...
          <div class="resHeader">
            <div class="title">{{coins?'Selamat ':'Lambat, angpaunya sudah habis'}}</div>
          ...
          <div class="resBottom">
            <div class="bar"></div>
            <div class="resBtn" @click="redPacketHistory()">Lihat detail</div>
          </div>
        </div>
      </transition>
    </div>
...
export default {
  ...
  methods: {
    ...
    shareAct () {
      ...
      var data = {
        "share_title": `Cepat! Ambil angpaulah!`,
        "share_content": "Klik masuk acara>>",
        "share_image": `https://activity.wekarapp.com/static_html/2020/charismatic_spkepersion/share.png?v=5`,
        "link": `https://activity.wekarapp.com/static_html/2020/charismatic_spkepersion/index.html?uid=#ID#&token=#TOKEN#`,
        "image": `https://activity.wekarapp.com/static_html/2020/charismatic_spkepersion/share.png?v=5`,
        "share_url": `https://activity.wekarapp.com/static_html/2020/charismatic_spkepersion/index.html?uid=#ID#&token=#TOKEN#`
      }
      if (ios) {
        if (window.share != undefined) {
          share(JSON.stringify(data))
        } else {
          location.href = `shareUserInfo://activity.17sing.tw/static_html/2020/charismatic_spkepersion/index.html?uid=#ID#&token=#TOKEN#&shareText=Klik masuk acara>>&userImg=https://activity.wekarapp.com/static_html/2020/charismatic_spkepersion/share.png?v=5&title=Cepat! Ambil angpaulah!`;
        }
      } else {
