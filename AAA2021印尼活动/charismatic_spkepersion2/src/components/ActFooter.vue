    ...
    <div class="acrStatus" :class="{noStart:astState == 0,actEd:astState == 2,singUp:astState == 1}" @click="singUp()">
      <div class="actIng" v-if="astState === 3">
        <div class="total" :class="'rank' + nowMsg.rank">
          ...
          <div class="songMsg">
            <div class="score" v-if="tab == 'total'">Nilai bintang top total:{{nowMsg.score}}</div>
            <div class="score" v-else> Nilai bintang {{act_day}}:{{nowMsg.score}}</div>
            <div class="add" v-if="nowMsg.rate > 0">Ditambah {{nowMsg.rate}}%</div>
          </div>
