...
function getDefault() {
    if (token) {
        return get(`/growing_way/init.php?token=${token}`);
    } else {
        return get(`/growing_way/shareinit.php`);
    }
}
//解锁礼包
function unlockPack(id) {
    return get(`/growing_way/openpacket.php?id=${id}&token=${token}`)
}
//领取任务
function completeTask(id) {
    return get(`/growing_way/finishtask.php?id=${id}&token=${token}`)
}
//注册
function singUp() {
    return get(`/growing_way/register.php?token=${token}`)
}
const httpConfig = {
    getDefault,
    unlockPack,
