<template>
  <div class="rankGroups">
    <!-- 日榜、总榜切换主Tabs -->
    <div class="mainTabs">
      <a @click.prevent="mainTabClick(0)" :class="{current:mainTab==0}" href="">紅包榜</a>
      <a @click.prevent="mainTabClick(1)" :class="{current:mainTab==1}" href="">搶紅包秘籍</a>
    ...
    <div v-if="mainTab==0" class="tabs">
      <a @click.prevent="tabClick(0)" :class="{current:tab==0}" href="">達人榜</a>
      <a @click.prevent="tabClick(1)" :class="{current:tab==1}" href="">手氣榜</a>
    </div>
    <div class="downTimebox" v-if="mainTab == 0">
      <p v-if="actStatus == 0">活動開始倒計時</p>
      <!-- 活动结束 -->
      <p v-else-if="actStatus == 1">活動結束倒計時</p>
      <p v-else> 活動已結束</p>
    ...
    <div class="listBox" v-if="mainTab==0">
      <div class="titleBox">
        <span>排名</span>
        <span>暱稱（UID） </span>
        <span v-if="tab == 0">發送紅包總金幣數</span>
        <span v-else>收到紅包個數</span>
      </div>
      <ul class="list day">
        ...
        <div v-if="rank.list.length==0" class="scrollNone">暫無數據</div>
      </ul>
    </div>
