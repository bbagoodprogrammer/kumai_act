  ...
  <div class="wardsListBox">
    <div class="listBox">
      ...
      <div class="list">
        ...
        <div v-if="wardsList.length == 0" class="noList">暫無數據</div>
        <p class="listMsg">上麥時獲得送禮人數前五名人氣值加成1%-5%</p>
      </div>
    ...
    <div class="listFooter" v-if="showUsermsg">
      <div class="top">
        ...
        <span>送禮人數:{{wardsListUserMsg.score}}</span>
      </div>
      <div class="bottom">
        <span>排名：{{wardsListUserMsg.rank}}</span>
        <span>人氣值加成：{{wardsListUserMsg.scaling*100+'%'}}</span>
      </div>
    </div>
  </div>
</template>
