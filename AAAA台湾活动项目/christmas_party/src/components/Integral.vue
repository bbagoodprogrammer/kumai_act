      ...
      <div class="receivePup" v-if="showGiftPup">
        <div class="receivePupCon">
          ...
          <div class="type" v-if="type == 1">
            ...
            <p class="giftName" v-else>隨機禮物double卡</br>當天前三檔禮物隨機一個*2</p>
            <p class="bean" v-if="giftIndex== 0">(已連續達成{{gift_list.continue_login_counts}}天)</p>
            <span class="lingqu" :class="{act:gift_list.gift_list[giftIndex].is_get == 0 && sucStep>=(giftIndex+1)}" @click="receive()">{{gift_list.gift_list[giftIndex].is_get==0?'領取':'已領取'}}</span>
          </div>
          ...
          <div class="type3" v-else-if="type == 3">
            ...
            <span class="succeTitle">領取成功!</span>
            <p class="sucTips">
              對應獎勵已派發到您的賬戶上，</br>
              請注意查收!
            </p>
