  ...
  <div class="box" :style="{backgroundImage: 'url(' + banner + ')', backgroundSize:'center',backgroundRepeat:'no-repeat'}">
    ...
    <div class="herder">
      <div class="spanBox" :class="{shareTop:isShare}">
        <span class="rules" @click="showRulesPup()">活動規則</span>
        <span class="history" @click="showBuyHistoryPup()">購買記錄</span>
      </div>
    </div>
    ...
    <transition name="slide">
      <div class="rulesPup" v-show="showRules">
        <div class="rulesCon">
          ...
          <p class="time">活動時間: {{stime}} ~ {{etime}}</p>
          <h6>活動規則：</h6>
          <p>1.用戶可用金幣或金豆購買商品，限購次數，購買商品的貨幣屬性具體以商品要求的條件爲準！</p>
          <p>2.活動分爲多場秒殺，商品售完即止！未開始的場次可先設置提醒，開搶後系統將自動發送提醒信息，不錯過每個爆款開售！</p>
          <p>3.秒殺到的禮物，禮券將會成功派發到你的背包上，請注意查收！IOS可前往app store 搜尋最新版本“高歌”，下載使用</p>
          <p>4.特效禮物請更新到最新版本使用，否則可能會錯過絢麗特效哦</p>
          <p class="tips">活動最終解釋權歸主辦方所有</p>
        </div>
      </div>
    </transition>
    <transition name="slide">
      <div class="buyHistoryPup" v-show="showBuyHistory">
        <div class="buyHistoryPupCon">
          ...
          <div class="title">購買記錄</div>
          <div class="listTitle">
            <span class="giftName">禮物</span>
            <span class="time">場次</span>
