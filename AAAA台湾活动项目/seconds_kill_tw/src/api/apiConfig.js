...
function getDefault() {
    if (token) {
        return get(`/seconds_kill/init.php?token=${token}`);
    } else {
        return get(`/seconds_kill/shareinit.php`);
    }
}

//预约
function make(gid) {
    return get(`/seconds_kill/order.php?token=${token}&gift_id=${gid}`)
}

//购买
function buyGift(id, count) {
    return get(`/seconds_kill/buy.php?token=${token}&gift_id=${id}&count=${count}`)
}

//购买记录
function buyrecord(qid, from) {
    return get(`/seconds_kill/buyrecord.php?token=${token}&qid=${qid}&from=${from}`)
}
const httpConfig = {
    getDefault,
    make,
