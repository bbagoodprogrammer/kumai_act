...
function getDefault(){
    if(token){
        return get(`/lucky_wednesday/init.php?token=${token}`);
    }else{
        return get(`/lucky_wednesday/init.php`);
    }
}
//购买
function buy(gid){
    return get(`/lucky_wednesday/buy.php?id=${gid}&token=${token}`)
}

function bugHistory(){
    return get(`/lucky_wednesday/history.php?token=${token}`)
}
const httpConfig = {
    getDefault,
    buy,
