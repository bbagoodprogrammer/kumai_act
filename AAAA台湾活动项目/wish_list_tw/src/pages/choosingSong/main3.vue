<template>
  <div class="ChoosingSong">
    <div class="top">
      <textarea maxlength="300" placeholder="請填寫4個字符以上的心願寄語，以便提升心願的完成率！" @input="descInput()" v-model="desc">
      </textarea>
      <span class="strLength">{{remnant}}/300</span>
    </div>
    <div class="bottom">
      <p class="title" v-if="!isChoosing">請選擇一首心願歌曲</p>
    ...
    <transition name="slide">
      <div class="choosingSongPup" v-show="showSongList">
        <div class="shear">
          <input type="text" placeholder="歌曲名/歌手名" v-model="sheraVal" />
          <span class="go" @click="goSearch()"></span>
        </div>
        <p v-if="songList.length == 0" class="noSong">暫無歌曲</p>
        <ul class="scrollable">
