<template>
  <div class="footerBox">
    <div class="footerBar">
      <div class="acrStatus" :class="{black:userMsgState.count<=0}">
        ...
        <span class="goAct" v-if="astState === 1" @click="showChoosingPup()">
          <div class="title">發佈點歌心願</div>
          <div class="shengyu">今日可發佈心願: {{userMsgState.count}}</div>
    ...
    <transition name="slide">
      <msg-toast :tostTitle="tostTitle" :msg="tastMsg" @closeToast="closeToast()" v-show="showT"></msg-toast>
    </transition>
  </div>
    ...
    showChoosingPup() {
      globalBus.$emit('commonEvent', () => {
        if (this.userMsgState.count <= 0) {
          this.tostTitle = `無法許願 `
          this.tastMsg = `今日許願次數已達到上限，無法許願，明天再來唷～`
          this.showT = true
        } else {
          let regstr = getString('token')
          location.href = `./index3.html?token=${regstr}&t=1`
        }
      })
    },
    showHistory() {
