<template>
  <div class="history3">
    <div class="userItemPup">
      <p v-if="history3.list && history3.list.length == 0" class="tips">暫無心願</p>
      <ul class="userCon scrollable" v-else>
        <li v-for="(item,index) in history3.list" :key="index">
          ...
          <p class="jiyu" v-html="getStr(item.wish)"></p>
          <div class="userSatisfied" v-if="item.status == 2">
            用戶滿意分:<em>{{item.score}}</em><i class="xinTip" :class="{red:item.score > 0}"></i>
          </div>
          <div class="userSatisfied" v-if="item.status == 1">
            <span class="goSing" @click="goSing()"></span>
          </div>
        </li>
        <p class="loadingTips" v-show="loading">加載中...</p>
      </ul>
    </div>
  </div>
</template>
          ...
          api.wishHistory('took', this.history3.list.length).then((res) => {
            ...
            if (res.data.response_status.code === 0) {
              ...
              if (list.length == 0) {
                ...
                this.$store.commit('addHistory3', list)
              }
            }
          })
        }
