<template>
  <div class="gradeGift">
    ...
    <div class="listTitle">
      <span class="grade">等級</span>
      <span class="score">目標分數</span>
      <span class="ward">獎勵</span>
      <span class="state">領取情況</span>
    </div>
    <ul>
      <li v-for="(item,index) in newLv" :key="index">
        <span class="grade">Lv.{{item.lv}}</span>
        ...
        <span class="state">
          <span v-if="item.is_get==1" class="noSuc">已領取</span>
          <span v-else-if="gradeObj.grade<item.lv" class="suo"></span>
          <span v-else-if="item.is_get == 0" class="suc" :class="{black:item.lv > gradeObj.grade}" @click="getGift(item,index)">領取</span>
        </span>
      </li>
    </ul>
    <Loading></Loading>
