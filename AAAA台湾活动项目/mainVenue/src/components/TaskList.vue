<template>
  <div class="taskList">
    <div class="rankListCon">
      ...
      <div class="rankTitle">
        <span class="day">每日聖誕任務</span>
        <span class="point">積分 <i></i></span>
        ...
        <li v-for="(item,index,i) in daily_tasks" :key="index" :class="{act:i %2 == 0}">
          ...
          <span class="du">
            <span class="state">
              ...
              <b v-if="item.task_list.length > 1" @click="showWhole(item.finish_count,item.task_list)">查看全部</b>
            </span>
          </span>
        </li>
      </ul>
  ...
  data() {
    return {
      rankName: {
        ...
        sendBean: "當天向作品或K房送出100金豆",
        charge: "當天累計儲值金幣達到",
        sendCoin: "當天累計向作品或K房送出禮物金達到",
        // praise: "給10個不同的作品點讚",
      },
      wholePup: false,
      nowTask_list: [],
