<template>
  <div class="rankGroups">
    ...
    <!-- 日榜、总榜切换主Tabs -->
    <div class="mainTabs">
      <a @click.prevent="changPre(0)" :class="{current:month==0}" href="">Top Mingguan</a>
      <a @click.prevent="changPre(1)" :class="{current:month ==1}" href="">Top Bulanan</a>
    </div>
    <div class="lastTabs">
      <span v-if="month== 0 && mainTab==2" @click="mainTabClick(0)">Minggu ini>></span>
      <span v-if="month== 0 && mainTab==0" @click="mainTabClick(2)">Minggu lalu>></span>
      <span v-if="month== 1 && mainTab==3" @click="mainTabClick(1)">Bulan lalu>></span>
      <span v-if="month== 1 && mainTab==1" @click="mainTabClick(3)">Bulan lalu>></span>
    </div>
    <p v-if="mainTab==0 || mainTab==2" class="rankTips">Perangkat didasarkan poin yg K-room dapatkan pd minggu {{mainTab==0?'ini':'lalu'}}, yg mencapai 3000 nilai atau masuk Top10 teratas akan dapatkan hadiahnya</p>
    <p v-else class="rankTips">Perangkat didasarkan poin yg K-room dapatkan pd bulan {{mainTab==1?'ini':'lalu'}}, yg mencapai 8000 nilai atau masuk Top10 teratas akan dapatkan hadiahnya</p>
    <ul class="list day" :class="{nodata: rank.list.length == 0}">
      <li v-for="(item,index) in rank.list" :key="index" @click="goUser(item.rid)">
        ...
        <div class="score" v-if="mainTab==0 || mainTab==2">Poin minggu ini {{item.score}}</div>
        <div class="score" v-else>Poin bulan ini {{item.score}}</div>
      </li>
    </ul>
    <!-- 总榜 -->
    ...
    <div v-if="rank.none" class="scrollNone">List kosong</div>
    <div class="footer" v-if="msg.rank != 0 && msg.rid">
      ...
      <div class="score" v-if="mainTab==0 || mainTab==2">Poin minggu ini {{msg.score}}</div>
      <div class="score" v-else>Poin bulan ini {{msg.score}}</div>
    </div>
  </div>
</template>
