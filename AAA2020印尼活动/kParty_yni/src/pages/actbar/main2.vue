<template>
  <div class="myParty">
    <p v-if="actList.length == 0" class="noData">Belum ada data! Ayo mendaftar ya </p>
    <ul>
      <li v-for="(item,index) in actList" :key="index" :class="[{max:item.apply_status != 0,black:item.status == 2},'list'+item.cover]">
        ...
        <div class=" title">{{item.them}} <span v-if="item.status == 2"> - acara udah selesai</span> </div>
        <div class="actCon">
...
export default {
  ...
  methods: {
    ...
    getDate(time) {
      ...
      if (partyTime == 0) {
        return `hari ini ${getDate(new Date(time * 1000), 1)}`
      } else {
