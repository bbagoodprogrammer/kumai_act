<template>
  <div class="giftBox">
    <div class="userLv">
      <div class="lv">Levelku: <strong>Lv.{{packets.level}}</strong></div>
      <div class="score">Nilai bersinar <strong>{{packets.score}}</strong> <i class="quest" @click="showTipsPup = true"></i></div>
    </div>
    <div class="liner">
    ...
    <div class="mask" v-show="showBoxGift">
      <transition name="slide">
        <div class="boxGifts" v-if="showBoxGift">
          ...
          <div class="getStatus" :class="{act:isOpen}" @click="getGift()">{{packets.status[showBoxIndex]?'Telah menerima':'Menerima'}}</div>
        </div>
      </transition>
    </div>
    ...
    <div class="mask" v-show="showTipsPup">
      <transition name="slide">
        <div class="boxGifts tipsPup" v-if="showTipsPup">
          ...
          <h3>Cara utk dapatkan nilai bersinar</h3>
          <div class="tipsCon">
            <p class="yel">Nilai bersinar = dapatkan jumlah klik like X10 + menerima nilai pesona hadiah koin emas dari nyanyian yg ikut acara</p>
...
export default {
  ...
  computed: {
    ...
    showBoxItem() {
      if (this.showBoxIndex == 5) {
        return this.packets.prizes[this.showBoxIndex].filter((item) => {
  ...
  methods: {
    ...
    getGift() {
      globalBus.$emit('commonEvent', () => {
        if (this.isOpen) {
          api.openPacket(this.showBoxIndex).then(res => {
            if (res.data.response_status.code == 0) {
              ...
              this.vxc('setToast', {
                ...
                msg: 'Hadiah telah dikirim ke akunmu,silakan cek!'
              })
            } else {
          ...
          this.vxc('setToast', {
            title: 'Gak bisa menerima',
            msg: `Level mencapai Lv.${this.showBoxIndex}(nilai bersinar mencapai ${this.packets.levels[this.showBoxIndex]})baru bisa menerima`
          })
        }
      })
    },
    getGiftName(item) {
      if (item.type == 'gift') {
        return 'Hadiah @ ( # koin emas )*$'.replace('@', item.name).replace('#', item.coin).replace('$', item.count)
      } else if (item.type == 'coupon') {
        return `${item.ratio}% kupon bonus top up`
      } else if (item.type == 'vip') {
        return `VIP(${item.day} hari)`
      } else if (item.type == 'bean') {
        return `${item.count}kacang emas`
      } else if (item.type == 'coin') {
        return `${item.count}koin emas`
      } else if (item.type == 'medal') {
        return `${item.name}(7 hari)`
      }
    }
  }
}
