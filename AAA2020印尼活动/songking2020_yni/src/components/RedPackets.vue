<template>
  <div class="redPactets">
    ...
    <div class="redTips">
      Nyanyian dgn label “ Kontes King Penyanyi Pesona” tiap kali menerima {{redPacket.step}} hadiah <i></i>,semua pengguna bisa buka angpau
    </div>
    ...
    <div class="lastOpenPacketPeople" v-if="naming.uid">
      <div class="title">Angpau putaran lalu disponsori oleh penyanyi berikut</div>
      <div class="people">
          ...
          <vue-seamless-scroll :class-option="optionHover" :data="redPacket.record" class="seamless-warp">
            <ul class="item">
              <li v-for="(item,index) in redPacket.record" :key="index">
                ...
                <strong> <em class="nick">{{item.nick}}</em> dapatkan {{item.count}} hadiah “mendukungmu”</strong>
              </li>
            </ul>
          </vue-seamless-scroll>
    ...
    <div class="mask" v-show="packetRes">
      <transition name="slide">
        <div class="packetRes" v-if="packetRes">
          ...
          <div class="resHeader">
            <div class="title">{{coins?'Selamat':'Tangan lambat,angpau habis'}}</div>
          ...
          <div class="resBottom">
            <div class="bar"></div>
            <div class="resBtn" @click="redPacketHistory()">Cek info menerima</div>
          </div>
        </div>
      </transition>
    </div>
...
export default {
  ...
  methods: {
    subscribe() {
      if (!this.reg) {
        this.vxc('setToast', {
          ...
          msg: 'Belum ikut acara,kamu gak bisa ambil angpau,buruan klik”Mendaftar”utk ikut acara'
        })
        return
      }
    ...
    shareAct() {
      ...
      var data = {
        "share_title": `Cepat masuk!Siap ambil angpau!`,
        "share_content": "Klik reservasi utk ambil angpau, dan Anda juga bisa menerima pengingat >>",
        "share_image": `https://activity.wekarapp.com/static_html/2020/songking2020/share.png?v=2`,
        "link": `https://activity.wekarapp.com/static_html/2020/songking2020/index.html?uid=#ID#&token=#TOKEN#`,
        "image": `https://activity.wekarapp.com/static_html/2020/songking2020/share.png?v=2`,
        "share_url": `https://activity.wekarapp.com/static_html/2020/songking2020/index.html?uid=#ID#&token=#TOKEN#`
      }
      if (ios) {
        if (window.share != undefined) {
          share(JSON.stringify(data))
        } else {
          location.href = `shareUserInfo://activity.17sing.tw/static_html/2020/songking2020/index.html?uid=#ID#&token=#TOKEN#&shareText=Klik reservasi utk ambil angpau, dan Anda juga bisa menerima pengingat >>&userImg=https://activity.wekarapp.com/static_html/2020/songking2020/share.png?v=2&title=Cepat masuk!Siap ambil angpau!`;
        }
      } else {
