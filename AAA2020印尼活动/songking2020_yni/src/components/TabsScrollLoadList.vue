  ...
  <div class="rankGroups">
    ...
    <div class="rankTips">
      <div class="giftTips" v-if="mainTab == 0">
        <p>Nilai bersinar = dapatkan jumlah klik like X10 + nilai pesona hadiah koin emas diterima dari nyanyian yg ikut acara <br />Nyanyian yg ikut acara dapatkan hadiah tertentu berikutnya, nilai pesona meningkat 5%</p>
        <div class="giftItem">
    ...
    <div class="rankList">
      <ul v-if="mainTab==0" class="list day">
        <li v-for="(item,index) in rank.list" :key="index" :class="'rank' +item.rank">
          ...
          <div class="msg">
            ...
            <div class="add" v-if="item.ratio > 0">Meningkat {{item.ratio}}%</div>
          </div>
          <div class="score">
            <div class="lv">Lv.{{item.level}} <em class="lvScore">Nilai: {{item.score}}</em> </div>
            <div class="iconScore">
      ...
      <div class="upTopRank" v-else-if="mainTab==1">
        <div class="topTabs">
          <span :class="{act:tab == 1}" @click="tabClick(1)">Diposting dalam 48 jam</span>
          <span :class="{act:tab == 2}" @click="tabClick(2)">Diposting sebelum 48 jam</span>
        </div>
        <div class="listHeader">
          <span class="name">Nama nyanyian</span>
          <span class="bar">Kemajuan dapatkan hadiah</span>
          <span class="status">Status saat ini</span>
        </div>
        <ul class="list total">
          <li v-for="(item,index) in rank.list" :key="index">
            ...
            <div class="status">
              <strong v-if="tab == 2">{{item.status == 1?'Udah melonjak':'Telah melebihi waktu'}}</strong>
              <strong v-else>{{item.status == 1?'Udah melonjak':'Tunggu melonjak'}}</strong>
              <span v-if="!item.status && tab == 1">Sisa:{{getDate(item.ltime,2)}}</span>
            </div>
          </li>
        </ul>
      </div>
  ...
  data() {
    return {
      ...
      giftArr: [
        ...
        'Akordeo',
        'Unicorn pelangi ',
        'Hadiah efek khusus Balon cinta'
      ]
    }
  },
  watch: {
