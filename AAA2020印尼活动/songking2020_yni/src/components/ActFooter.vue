    ...
    <div class="acrStatus" :class="{noStart:astState == 0,actEd:astState == 2,singUp:astState == 1}" @click="singUp()">
      <div class="actIng" v-if="astState === 3">
        <div class="total" v-if="tab != 'total'">
          ...
          <div class="songMsg">
            <div class="mySong">Jumlah laguku ikut acara:{{nowMsg.total}}</div>
            <div class="topNum">Jumlah lagu yg sudah melonjak:{{nowMsg.up}}</div>
          </div>
          <div class="commitSongBtn" @click="commitSong()">Upload nyanyian</div>
        </div>
        <div class="left" :class="'rank' +nowMsg.rank" v-else>
          ...
          <div class="score">
            <div class="lv">Lv.{{nowMsg.level}} <em class="lvScore">Nilai:{{nowMsg.score}}</em> </div>
            <div class="iconScore">
        ...
        if (this.astState == 1) {
          api.singUp().then(res => {
            if (res.data.response_status.code == 0) {
              ...
              if (top) {
                this.vxc('setToast', {
                  msg: `Selamat ,peringkat nilai pesona hadiah dgn koin emas dari nyanyianmu yg diupload pd tgl X Dec - tgl Y Dec adalah Top ${top[0]},dan kamu mendapatkan nilai pesona meningkat ${top[1]}% dari Top pesona acara ini.`,
                })
              }
            } else {
