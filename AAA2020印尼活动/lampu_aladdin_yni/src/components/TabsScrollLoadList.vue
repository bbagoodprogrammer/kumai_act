<template>
  <div class="rankGroups">
    ...
    <div class="mainTabs" :class="{maxH:mainTab!=2 && mainTab!=1}">
      <div class="tabItem">
        <span class="taskTab" @click.prevent="mainTabClick(2)" :class="{current:mainTab==2}" href="">Tugas</span>
        <div class="tabs">
      ...
      <div class="listBg" v-if="mainTab!=2 && mainTab!=1">
        ...
        <div class="dayScore" :class="{mt:tab<nowDay}">
          ...
          <div class="scoreMsg">
            <div class="total">Nilai panggilan dari lampu ajaib sekarang telah mencapai{{rank.amount}}</div>
            <div class="conis" v-if="tab>=nowDay">Top 100 user dari Daftar Harian akan bisa membagikan {{rank.coins}} koin emas hari ini</div>
            <div class="conis" v-else>Top 1-100 dari Daftar Harian sudah membagikan {{rank.coins}} koin emas</div>
          </div>
        </div>
      </div>
    ...
    <ul v-if="mainTab==0" class="list day">
      <li v-for="(item,index) in rank.list" :key="index" :class="'rank'+item.rank" @click="goUser(item.uid)">
        ...
        <div class="score" :class="{ml:item.coins==0}">Nilai panggilan<em>{{item.score}}</em></div>
        <!-- -->
        <div class="coins" v-if="item.coins>0">{{item.coins}}Koin emas</div>
      </li>
    </ul>
    <!-- 总榜 -->
    <ul v-else-if="mainTab==1" class="list total">
      <li v-for="(item,index) in rank.list" :key="index" :class="'rank'+item.rank" @click="goUser(item.uid)">
        ...
        <div class="score">Nilai panggilan <em>{{item.score}}</em></div>
        <!-- v-if="item.coins>0" -->
    ...
    <div class="listTipsBox" v-if="(rank.loading || rank.list.length == 0 && (tab <= nowDay || mainTab ==1) ||  mainTab==0 && tab > nowDay)&& mainTab!=2 ">
      <div v-if="rank.loading" class="scrollLoading">Loading...</div>
      <div v-if="rank.list.length == 0 && (tab <= nowDay || mainTab ==1)" class="scrollNone">
        Sekarang belum ada data!
      </div>
