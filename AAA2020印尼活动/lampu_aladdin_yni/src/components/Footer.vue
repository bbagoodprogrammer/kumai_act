<template>
  <div class="footer">
    ...
    <span class="singUp" @click="goSingUp()" v-if="showState===2">Segera mendaftar</span>
    <div class="list" v-if="showState ===3" :class="'rank'+myMsg.rank">
      <div class="rank" v-if="myMsg.rank > 0">{{myMsg.rank}}</div>
      <div class="noRank" v-else>Belum masuk daftar</div>
      <div class="uerImg">
        <span class="imgBg"></span>
        <img v-lazy="myMsg.avatar" alt="" class="imgItem">
      </div>
      <div class="userMsg">
        <span class="total" v-if="tab=='total'">
          <span> <strong>Total nilai panggilan</strong> <em>{{myMsg.all_score}}</em></span>
          <span><strong>Jarak dari yg sebelumnya</strong><em> {{myMsg.gap}}</em></span>
        </span>
        <span class="toDay" v-else>
          <span><strong>Nilai panggilan hari ini</strong><em>{{myMsg.today_score}}</em></span>
          <span><strong>Jarak dari yg sebelumnya</strong><em> {{myMsg.gap}}</em></span>
        </span>
      </div>
    </div>
