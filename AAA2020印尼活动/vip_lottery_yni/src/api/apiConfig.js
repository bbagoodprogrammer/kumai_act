...
function getDefault() {
    if (token) {
        return get(`/vip_lottery/init.php?token=${token}`);
    } else {
        return get(`/vip_lottery/init.php`);
    }
}

//報名
function singUp() {
    return get(`/vip_lottery/register.php?token=${token}`)
}

//刮卡
function lottery(posi) {
    return axios.get(`/vip_lottery/lottery.php?posi=${posi}&token=${token}`)
}

//驗證邀請碼
function verIfy(code) {
    return get(`/vip_lottery/verify.php?code=${code}&token=${token}`)
}

//領取詳情
//type -1全部，0神秘任务除外，8神秘任务
function detail(type, from, more) {
    if (more) {
        return axios.get(`/vip_lottery/detail.php?type=${type}&from=${from}&token=${token}`)
    } else {
        return get(`/vip_lottery/detail.php?type=${type}&from=${from}&token=${token}`)
    }
}
