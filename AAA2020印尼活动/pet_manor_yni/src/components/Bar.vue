<template>
  <div class="bar">
    <div class="barTips">Anda dapat membuka kunci dan mengumpulkan pet lucu yg sesuai setelah menerima hadiah di nyanyian atau kamar karaoke.<br /> <strong>Setiap Anda mengumpulkan 1 pet yg lucu, Anda dapat membuka 1 level hadiah</strong> </div>
    <div class="barItem">
      <div class="liner">
        <span class="store" :style="{left:getLeft(index)}" :class="{act:schule >= index}" v-for="(item,index) in petObj" :key="index">
          ...
          <em class="gift">
            ...
            <strong>{{item.gift.num?`x${item.gift.num}`:`${item.gift.day} hari`}}</strong>
          </em>
        </span>
