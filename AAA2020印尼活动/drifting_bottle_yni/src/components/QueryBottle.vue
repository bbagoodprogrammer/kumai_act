  ...
  <div class="querySong">
    ...
    <div class="query">
      ...
      <div class="firstCom" v-if="showType==1">
        ...
        <!-- 對不起，這首作品《{{querySong.name}}》已于{{getDate(querySong.free_time)}}被免費投瓶過一次了，可以選擇定製投瓶再進行投放~ -->
        <P v-if="querySong.free">Nyanyian ini telah dilemparkan secara gratis</P>
      ...
      <div class="defaultCom" v-else>
        <div class="pNumBox">
          <div class="pNum" :class="{act:item.pNum == pNum}" v-for="(item,index) in optionArr " :key="index" @click="selectChange(item)">
            ...
            <span class="price">{{item.price}} <em>{{lang.coins}}</em></span>
          </div>
        </div>
    ...
    <div class="mask" v-show="showQuery">
      <transition name="slide">
        <div class="queryPup" v-show="showQuery">
          ...
          <p>{{queryType==1?`Apakah Anda akan melempar botol lagu ini dengan ${price} koin emas?`:`Maaf, koin emasmu yg tersisa gak cukup utk melempar botol, silakan top up dulu~`}} </p>
  ...
  computed: {
    ...
    type() {
      if (this.first && this.showType == 1) {
        return 1
      } else if (this.pNum == 60 && this.showType == 2) {
        return 3
      } else if (this.pNum == 180 && this.showType == 2) {
        return 4
      } else if (this.pNum == 10 && this.showType == 2) {
        return 2
      }
    }
  },
