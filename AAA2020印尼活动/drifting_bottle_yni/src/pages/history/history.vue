<template>
  <div class="history">
    <div class="tabs">
      <span :class="{act:showType==1}" @click="tabClick(1)">Catatan lempar </span>
      <span :class="{act:showType==2}" @click="tabClick(2)">Catatan ambil</span>
    </div>
    <ul v-if="showType==1">
      <li v-for="(item,index) in list.list" :key="index" @click="goSid(item.sid)">
        ...
        <div class="userMsg">
          <div class="user">
            ...
            <em v-if="item.ban">(Diskualifikasi)</em>
          </div>
          <div class="dStatus">
          ...
          <div class="dScore">
            ...
            <strong>Tambah jumlah follower:<em>{{item.attension}}</em></strong>
            <strong v-if="item.like">Nilai popularitas:<em>{{item.like}}</em></strong>
          </div>
        </div>
      </li>
    </ul>
    <ul v-else>
      <li v-for="(item,index) in list.list" :key="index" @click="goSid(item.sid)">
        ...
        <div class="time">{{item.time}} ambil</div>
      </li>
    </ul>
    <p class="hLoading" v-if="list.loading">Loading...</p>
    <p class="hLoading" v-if="list.list.length == 0 && !list.loading">Saat belum ada data</p>
    <Loading />
  </div>
</template>
<script>
