...
function getGuildInfo() {
    return get(API_ROOT+`/index.php?signture=innerserver&action=Action/Anchor.getGuildInfo&uid=${uid}&token=${token}&lang=${lang}`);
}

function getDayInfo(ymd) {
    return get(API_ROOT+`/index.php?signture=innerserver&action=Action/Anchor.getDayAnchorInfo&uid=${uid}&token=${token}&lang=${lang}&page=1&ymd=${ymd}`);
}

function getMonthInfo(ym) {
    return get(API_ROOT+`/index.php?signture=innerserver&action=Action/Anchor.getMonthAnchorInfo&uid=${uid}&token=${token}&lang=${lang}&page=1&ym=${ym}`);
}
