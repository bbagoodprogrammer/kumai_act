...
function getInitInfo() {
    ...
    return get("/index.php?action=shootBalloons.init&uid={uid}&token={token}");
}

//领取大奖
function getPrize() {
    return get(
        "/index.php?action=shootBalloons.receive&uid={uid}&token={token}"
    );
}
...
function luck(type, count) {
    return get(
        `/index.php?action=shootBalloons.shoot&uid={uid}&token={token}&type=${type}&count=${count}`
    );
}
...
function lotteryRecord(from, more) {
    if (more) {
        return axios.get(
            `/index.php?action=shootBalloons.records&uid={uid}&token={token}&from=${from}`
        );
    }
    return get(
        `/index.php?action=shootBalloons.records&uid={uid}&token={token}&from=${from}`
...
export { get, post, loadData, getInitInfo, getPrize, luck, lotteryRecord };
