...
axios.interceptors.request.use(
    async config => {
        ...
        if (url) {
            ...
            url = url.replace('{rid}', euc(rid)).replace('{uid}', euc(uid)).replace('{token}', euc(token)).replace('{target_uid}', euc(target_uid));
        }
...
function init() {
    return get('/index.php?action=Action/StrangeFate.getProgress&uid={uid}&token={token}&rid={rid}');
}

//rule
function getPrizeList() {
    return get('/index.php?action=Action/StrangeFate.getPrizeList&uid={uid}&token={token}&rid={rid}');
}
