...
axios.interceptors.request.use(
    async config => {
        ...
        if (url) {
            ...
            const sid = getUrlString('sid') || '';
            const uid = getUrlString('uid') || '';
            const token = getUrlString('token') || '';
            const target_uid = getUrlString('target_uid') || '';
            url = url.replace('{sid}', euc(sid)).replace('{uid}', euc(uid)).replace('{token}', euc(token)).replace('{target_uid}', euc(target_uid));
        }
...
function getBillBoard() {
    return get('/index.php?action=luckyBox.getBillBoard&uid={uid}&token={token}&id={id}');
}


function getLuckyBoxDetail() {
    return get('/index.php?action=luckyBox.getLuckyBoxDetail&uid={uid}&token={token}');
}
