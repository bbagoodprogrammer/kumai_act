...
//     this.downTime = JSON.stringify(downTime(timeKey));
// } else {
//     setInterval(() => {
//         this.downTime = JSON.stringify(downTime(timeKey));
//     }, 1000);
...
// https://www.cnblogs.com/pannysp/p/3756623.html
    ...
    if (typeof value != "undefined") {
        ...
        if (isNaN(value)) {
            ...
            sessionStorage.setItem(
                key,
                JSON.stringify([value, Math.round(new Date().getTime() / 1000)])
            );
        }
    } else {
        //取出倒计时
                ...
                Math.round(
                    arr[0] -
                        (new Date().getTime() / 1000 -
                            new Date(arr[1] * 1000).getTime() / 1000)
                )
            );
