...
export default {
    ...
    methods: {
        load() {
            ...
            // var _page = parseInt(this.list.length/40)+1;
            // console.log(_page,this.list.length);
            // if(_page==1&&parseInt(this.list.length)<40&&parseInt(this.list.length)!=0){
            //     _page = 2;
            // }
            axios.get(this.url.replace('{uid}', uid).replace('{token}', token).replace('{page}', this.now_page).replace('{from}', this.list.length)).then(res => {
                this.loading = false;
                let arr = res.data;
                // console.log(arr);
