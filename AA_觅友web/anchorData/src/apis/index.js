...
function getDayInfo(ymd) {
    return get(
        API_ROOT +
            `/index.php?signture=innerserver&action=Action/Anchor.getDayInfo&uid=${uid}&token=${token}&lang=${lang}&rid=${rid}&ymd=${ymd}`
    );
}

function getMonthInfo(ym, startDate, endDate) {
    return get(
        API_ROOT +
            `/index.php?signture=innerserver&action=Action/Anchor.getMonthInfo&uid=${uid}&token=${token}&lang=${lang}&rid=${rid}&ym=${ym}&startDate=${startDate}&endDate=${endDate}`
    );
}

export { loadData, getDayInfo, getMonthInfo };
