...
class LocalFilesHashPlugin {
    apply(compiler) {
        compiler.hooks.afterEmit.tap("LocalFilesHashPlugin", compilation => {
            ...
            const history = fs.readFileSync("./dist/history.html");
            const tips = fs.readFileSync("./dist/tips.html");
            fs.writeFileSync(
                "./dist/index.html",
            ...
            fs.writeFileSync(
                "./dist/history.html",
                history.toString().replace("$hash$", JSON.stringify(obj))
            );
            fs.writeFileSync(
                "./dist/tips.html",
                tips.toString().replace("$hash$", JSON.stringify(obj))
            );
            if (files.length) {
                console.log(
                    "\n==================== 以下文件包含中文 ===================="
