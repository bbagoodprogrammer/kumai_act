...
function loadData(apiFunc, commitName, loadOnce = false, nums) {
...
function getInitInfo(is_pink) {
    // return testGet('getInitInfo');
    // return testGet(arguments.callee.name);
    return get(
        `/index.php?action=jewel.getMyJewelWallet&_login_uid={uid}&token={token}&lang={lang}&is_pink=${is_pink}`
    );
}

function getHistory(is_pink, year, from, more) {
    if (more) {
        if (!year) {
            return axios.get(
                `/index.php?action=jewel.getMyDailyJewelBill&_login_uid={uid}&token={token}&lang={lang}&is_pink=${is_pink}&from=${from}`
            );
        }
        return axios.get(
            `/index.php?action=jewel.getMyDailyJewelBill&_login_uid={uid}&token={token}&lang={lang}&is_pink=${is_pink}&from=${from}&year=${year}`
        );
    } else {
        if (!year) {
            return get(
...
function getGift(is_pink, gid) {
    // return testGet('getInitInfo');
    // return testGet(arguments.callee.name);
    return get(
        `/index.php?action=jewel.getJewelGoods&_login_uid={uid}&token={token}&lang={lang}&is_pink=${is_pink}&gid=${gid}`
    );
}

export { get, post, loadData, getInitInfo, getHistory, getGift };
