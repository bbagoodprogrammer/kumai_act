...
function getDayInfo(ymd) {
    return get(API_ROOT+`/index.php?signture=innerserver&action=Action/Anchor.getDayInfo&uid=${uid}&token=${token}&lang=${lang}&rid=${rid}&ymd=${ymd}`);
}

function getMonthInfo(ym) {
    return get(API_ROOT+`/index.php?signture=innerserver&action=Action/Anchor.getMonthInfo&uid=${uid}&token=${token}&lang=${lang}&rid=${rid}&ym=${ym}`);
}
