...
function getInitInfo() {
    ...
    return get(
        "/index.php?action=task.getTaskList2&uid={uid}&token={token}&lang={lang}"
    );
}

function getGift(id, deviceId) {
    return get(
        `/index.php?action=task.getThePrize&uid={uid}&token={token}&lang={lang}&id=${id}&deviceId=${deviceId}`
    );
}

export { get, post, loadData, getInitInfo, getGift };
