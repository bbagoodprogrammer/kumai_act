<template>
  <div class="bandit">
    ...
    <div class="footer">
      ...
      <p class="toastMsg">Top up 1 koin emas selama acara</br> Anda akan mendapatkan 1 koin harimau</br>
        Koin harimau dapat digunakan untuk menukar undian,</br> semakin banyak koin harimau yang dihabiskan, hadiah yang Anda dapatkan dalam mode undian akan semakin kaya, dan semakin besar kemungkinan utk mendapatkan hadiah yang berharga!
      </p>
    </div>
    <loading></loading>
  ...
  created() {
    ...
    api.getDefault().then(function (res) { //请求基础数据
      ...
      that.activeMsg.lotterylist.forEach(function (item) {
        that.textArr.push(`Selamat   ${item.uid}   mendapatkan   ${wards[item.prizeid]} × ${item.num}`)
      })
      //存入活动时间
      let timeObj = {
        stime: that.activeMsg.stime,
        etime: that.activeMsg.etime
      }
