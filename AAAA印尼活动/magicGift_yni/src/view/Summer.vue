<template>
  <div class="bigBox">
    ...
    <div class="listTab">
      <a href="" class="works" :class="{active:showComponent === 'works'}" @click.prevent="updateWorks('works')">Top Hegemon Nyanyian</a>
      <a href="" class="kRoom" :class="{active:showComponent === 'kRoom'}" @click.prevent="updateKroom('kRoom')">Top Hegemon Kamar Karaoke</a>
      <a href="" class="people" :class="{active:showComponent === 'people'}" @click.prevent="updatePeople('people')">Top Hegemon Popularitas</a>
    </div>
    <component :is="showComponent"></component>
    <div class="footer" v-if="!isShare">
      <a class="singUp" v-if="showFooter === 2" @click.prevent="singUp()">Segera mendaftar</a>
      <span v-if="showFooter === 0">Acara belum dimulai</span>
      <span v-if="showFooter === 1">Acara telah berakhir</span>
      <div class="myMsg" v-show="showComponent === 'works' && (showFooter === 3 || showFooter == 4)">
        <img :src="WardsMyMsg.avatar" alt="" class="ml">
        <div class="msgBox">
          <div class="msgTop">
            Nilai pesona dari hadiah nyanyian:：<span>{{WardsMyMsg.score}}</span>
          </div>
          <div class="msgBottom">
            Peringkat ：<span>{{WardsMyMsg.rank}}</span>
          </div>
        </div>
      ...
      <div class="myMsg" v-show="showComponent === 'kRoom' && (showFooter === 3 || showFooter == 4)">
        ...
        <div class="msgBox">
          <div class="msgTop">
            Nilai pesona dari hadiah kamar karaoke：<span>{{kRommMyMsg.score}}</span>
          </div>
          <div class="msgBottom">
            Peringkat ：<span>{{kRommMyMsg.rank}}</span>
          </div>
        </div>
      </div>
    </div>
...
export default {
  components: { KRoom, Works, Loading, msgToast, People },
  data() {
    return {
      isShare: false, //是否分享
      showComponent: "works",
