<template>
  <div class="rankGroups">
    <!-- 日榜、总榜切换主Tabs -->
    <div class="mainTabs">
      <span class="taskTab" @click.prevent="mainTabClick(2)" :class="{current:mainTab==2}" href="">Tugas poin</span>
      <div class="tabs">
        <a @click.prevent="mainTabClick(0)" :class="{current:mainTab==0}" href="">Daftar Harian Karnaval</a>
        <a @click.prevent="mainTabClick(1)" :class="{current:mainTab==1}" href="">Daftar Total Karnaval</a>
      </div>
    ...
    <p class="scoreTips">Nilai karnaval = nilai kumulatif poin + nilai pesona dari hadiah koin emas yang didapatkan dari kamar karaoke atau nyanyian</p>
    <div class="downTimebox" v-if="mainTab == 0">
      <p v-if="tab == nowDay">Hitung mundur akhir daftar harian ini</p>
      <p v-else-if="tab > nowDay">Hitung mundur dimulai daftar harian ini</p>
      ...
      <div class="noTime" v-else>Daftar harian ini telah berakhir</div>
    </div>
    <!-- 日榜 -->
    ...
    <ul v-if="mainTab==0" class="list day">
      <li v-for="(item,index) in rank.list" :key="index" :class="'rank'+item.rank" @click="goUser(item.uid)">
        ...
        <div class="userMsg">
          ...
          <div class="probability">Winning rate :<em>{{(item.scale*100).toFixed(1)+'%'}}</em></div>
        </div>
        <div class="score">Nilai karnaval: <em>{{item.score}}</em></div>
      </li>
    </ul>
    <!-- 总榜 -->
    ...
    <div v-if="rank.none && mainTab==0 && (rank.list.length == 0 && (tab <= nowDay || tab ==='total'))" class="scrollNone">
      Sekarang belum ada pemain masuk Top,menunggu Anda untuk menantang!
    </div>
