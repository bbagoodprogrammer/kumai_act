<template>
  <div class="history">
    <div class="title">Detail waktu yg mendapat hadiah</div>
    <div class="giftlist">
      <div class="listTitle">
        <span class="time">Waktu</span>
        <span class="gift">Hadiah yg didapatkan</span>
      </div>
      <ul class="scrollable">
      ...
      <p v-if="record.length == 0" class="noDate">Belum ada catatan hadiah</p>
    </div>
    <Loading></Loading>
  </div>
</template>
  ...
  methods: {
    getGiftName(item) {
      if (item.gift_id == 3) {
        ...
        return `${item.counts}  Kacang emas`
      } else {
