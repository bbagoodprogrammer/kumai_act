...
//获取活动基础信息
function getDefault(rank) {
    if (token) {
        return get(`/gifts_packet/init.php?token=${token}&rank=${rank}`);
    } else {
        return get(`/gifts_packet/init.php?rank=${rank}`);
    }
}
function singUp() {
    return get(`/gifts_packet/register.php?token=${token}`)
}
const httpConfig = {
    getDefault,
    singUp
