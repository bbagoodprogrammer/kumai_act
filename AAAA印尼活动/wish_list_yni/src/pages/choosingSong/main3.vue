<template>
  <div class="ChoosingSong">
    <div class="top">
      <textarea maxlength="300" placeholder="Silakan isi pesan keinginan dengan lebih dari 4 karakter untuk meningkatkan tingkat penyelesaian keinginan Anda!" @input="descInput()" v-model="desc">
      </textarea>
      <span class="strLength">{{remnant}}/300</span>
    </div>
    <div class="bottom">
      <p class="title" v-if="!isChoosing">Silakan pilih satu lagu keinginan</p>
      ...
      <p class="tips">
        Hanya dapat memposting keinginan Anda setelah Anda mengisi semua info
      </p>
    </div>
    <div class="footer" :class="{active:isChoosing&& desc!== ''}" @click="Release()">
      <span>Posting keinginan lagu</span>
    </div>
    <transition name="slide">
      <div class="choosingSongPup" v-show="showSongList">
        <div class="shear">
          <input type="text" placeholder="Nama lagu/Nama penyanyi" v-model="sheraVal" />
          <span class="go" @click="goSearch()"></span>
        </div>
        <p v-if="songList.length == 0" class="noSong">Sekarang belum ada lagu ini</p>
        <ul class="scrollable">
