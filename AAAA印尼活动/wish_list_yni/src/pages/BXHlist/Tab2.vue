<template>
  <div class="tab1">
    <ul class="scrollable">
      ...
      <p v-show="loading" class="tips">Loading...</p>
      <p v-if="rankList2.list && rankList2.list.length == 0" class="tips">Sekarang belum ada data</p>
    </ul>
  </div>
</template>
<script>
      ...
      if (scrollToBottom) { //滾動加載，沒有加載完成
        ...
        if (this.more) {
          ...
          api.hearList('2', this.rankList2.list.length).then(res => {
            ...
            this.$store.commit('addRanklist2', res.data.response_data.list)
          })
        }
      }
    },
