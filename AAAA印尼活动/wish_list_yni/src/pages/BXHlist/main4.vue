<template>
  <div class="BXHlist">
    <div class="tabs">
      <span class="tab1" @click="changTab('tab1')" :class="{actTab:showTab == 'tab1'}">Top kirim keinginan</span>
      <span class="tab2" @click="changTab('tab2')" :class="{actTab:showTab == 'tab2'}">Top selesaikan keinginan</span>
    </div>
    <p class="tips1" v-show="showTab == 'tab1'">
      Daftar ini hanya menampilkan Top 100 pengguna yang mengirim keinginan yang paling banyak!
    </p>
    <p class="tips1" v-show="showTab == 'tab2'">
      Daftar ini hanya menampilkan Top 100 pengguna yang menyelesaikan keinginan yang paling banyak!
    </p>
    <p class="tips2" v-show="showTab == 'tab1'">
      Nilai keinginan = jumlah nyanyian keinginan yang dikirim + skor nyanyian yang diberikan pemohon/orang yang buat keinginan
    </p>
    <p class="tips2" v-show="showTab == 'tab2'">
      Nilai keinginan = jumlah nyanyian keinginan yang diterima
    </p>
    ...
    <div class="footer">
      <div class="box">
        ...
        <div class="score" v-if="userMsg.bad && showTab == 'tab1'">
          Telah keluar dari kontes
        </div>
        <div class="score" v-else-if="showTab == 'tab1'">
          Memberi/Kirim: <em>{{userMsg.score}}</em><i></i>
        </div>
        <div class="score" v-else>
          Menerima: <em>{{userMsg2.score}}</em>
        </div>
      </div>
    </div>
  </div>
