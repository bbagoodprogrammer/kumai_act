  ...
  <div class="footerBox">
    <div class="footerBar">
      <div class="acrStatus" :class="{black:userMsgState.count<=0}">
        <span class="noAct" v-if="astState === 0"> Acara belum dimulai</span>
        <span class="noAct" v-if="astState === 2"> Acara telah berakhir</span>
        <span class="goAct" v-if="astState === 1" @click="showChoosingPup()">
          <div class="title"> Posting </br>keinginan lagu</div>
          <div class="shengyu">Hari ini bisa posting keinginan: {{userMsgState.count}}</div>
    ...
    showChoosingPup() {
      globalBus.$emit('commonEvent', () => {
        if (this.userMsgState.count <= 0) {
          this.tastMsg = `Jumlah kali membuat keinginan tidak cukup!`
          this.showT = true
        } else {
