...
function getDefault(day) {
    if (token) {
        return get(`/ktv_beginner/init.php?total=${day}&token=${token}`);
    } else {
        return get(`/ktv_beginner/init.php`);
    }
}
//注册或者刷新个人信息
function singUpMsg(day) {
    return get(`/ktv_beginner/register.php?day=${day}&token=${token}`)
}
//出道榜单
function getDebutList() {
    return get(`/ktv_beginner/volrank.php`)
}
//活动规则时间段
function getDaysTime() {
    return get(`/ktv_beginner/days.php`)
}
const httpConfig = {
    getDefault,
    singUpMsg,
