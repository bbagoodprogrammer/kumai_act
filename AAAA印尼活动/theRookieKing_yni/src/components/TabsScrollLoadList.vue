<template>
  <div class="rankGroups">
    <!-- 日榜、总榜切换主Tabs -->
    <div class="mainTabs">
      <a @click.prevent="mainTabClick(0)" :class="{current:mainTab==0}" href="">Daftar harian</a>
      <a @click.prevent="mainTabClick(1)" :class="{current:mainTab==1}" href="">Daftar total</a>
    ...
    <div class="downTimebox" v-if="showTimeState && tab == nowDay && mainTab == 0">
      <p><em class="timeLf"></em>Hitung mundur akhir acara ini<em class="timeRi"></em></p>
      <div class="timeDown">
        <div class="day">
    ...
    <div class="downTimebox" v-if="totalShowTotalTime && mainTab == 1">
      <p><em class="timeLf"></em>Hitung mundur akhir acara periode ini<em class="timeRi"></em></p>
      <div class="timeDown">
        <div class="day">
    ...
    <!-- 日榜 -->
    <DayTabs v-if="mainTab==0" @changeClick="tabClick" :tab="tab" :isList="isList"></DayTabs>
    ...
    <div v-if="rank.none || (rank.list.length == 0 && (tab == nowDay || tab ==='total'))" class="scrollNone">Sekarang belum ada data!</div>
...
export default {
  ...
  computed: {
    ...mapState(['rankGroups', "nowDay", "setInited", "isShare", "activeityState", "day_down_time", "total_down_time"]),
    rankKey() {
    ...
    rankApi() {
      if (this.isShare) {
        var dayApi = `/ktv_beginner/dayrank.php?day=${this.tab}&from={from}`;
        var totalApi = `/ktv_beginner/totalrank.php?from={from}`;
        var api = this.rankKey == 'total' ? totalApi : dayApi;
        return api.replace('{day}', this.tab)
      } else {
        var dayApi = `/ktv_beginner/dayrank.php?token={token}&day=${this.tab}&from={from}`;
        var totalApi = `/ktv_beginner/totalrank.php?token={token}&from={from}`;
        var api = this.rankKey == 'total' ? totalApi : dayApi;
    ...
    totalShowTotalTime() { // 总榜剩余时间
      if (this.total_down_time > 0) {
        this.totaldownTimeGo(this.total_down_time)
        return true
      } else {
        return false
      }
  ...
  methods: {
    ...
    onScroll() {
      // if (this.tab > this.nowDay) return
      if ((this.tab > this.nowDay && this.rankKey !== 'total') || this.setInited === 1) { //初始化是少一次請求,是日榜的时候和不是总榜的时候返回
    ...
    totaldownTimeGo(val) {
      downTime('time2', val);
      this.totalSurplusTime = downTime('time2');
      this.timer2 = setInterval(() => {
        this.totalSurplusTime = downTime('time2');
        if (this.totalSurplusTime.end) {
          clearInterval(this.timer2)
          this.$store.commit("changtotal_down_time", 0) //總榜剩餘時間
          this.$store.commit("changactiveityState", 2) //当前期活动结束
        }
      }, 1000)
    },
    goUser(kid, uid) { //跳转
