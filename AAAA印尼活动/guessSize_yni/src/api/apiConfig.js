...
function getDefault() {
    if (token) {
        return get(`/guess_dice/init.php?token=${token}`);
    } else {
        return get(`/guess_dice/shareinit.php`);
    }
}

//注册
function singUp() {
    return get(`/guess_dice/register.php?token=${token}`)
}
//任务详情
function tasksDetails() {
    return get(`/guess_dice/dailytask.php?token=${token}`)
}

//礼物列表
function giftList() {
    return get(`/guess_dice/giftlist.php?token=${token}`)
}

//领取礼物
function receiveGift(gid) {
    return get(`/guess_dice/getgift.php?token=${token}&gid=${gid}`)
}

//领礼物记录
function receiveGiftRecord(from) {
    return get(`/guess_dice/getgiftrecord.php?token=${token}&from=${from}`)
}

//投注
function betting(bets) {
    var newBets = JSON.stringify(bets)
    return get(`/guess_dice/bet.php?token=${token}&bets=${newBets}`)
}
//投注记录
function bettingRecord(from) {
    return get(`/guess_dice/betRecord.php?token=${token}&from=${from}`)
}
