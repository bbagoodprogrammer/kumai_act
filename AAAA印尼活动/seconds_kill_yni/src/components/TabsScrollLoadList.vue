  ...
  <div class="rankGroups">
    ...
    <div class="downTime" v-if="tab >= nowTab &&!surplusTime.end && rank.second!==0">
      <div class="title" v-if="tab == nowTab && rank.gift_status ==1">
        Hitung mundur akhir round ini
      </div>
      <div class="title" v-else-if="tab >= nowTab">
        Hitung mundur mulai round ini
      </div>
      <div class="timeItem">
    ...
    <ul v-if="mainTab==0" class="list day">
      <li v-for="(item,index) in rank.list" :key="index">
        ...
        <div class="gifMsg">
          ...
          <div class="tips" v-if="item.label != ''">{{item.label}}</div>
          <div class="quota" v-if="item.user_max != 0">Hanya bisa membelinya sebanyakk {{item.user_max}} buah</div>
          <div class="sold">
            <em v-if="((1-(item.stock / item.total_max))* 100)%1 == 0">Telah dijual {{((1-(item.stock*1 / item.total_max*1))* 100)+ '%'}}</em>
            <em v-else>Telah dijual {{((1-(item.stock*1 / item.total_max*1))* 100).toFixed(1) + '%'}}</em>
    ...
    <div v-if="rank.none" class="scrollNone">Sekarang belum ada data!</div>
...
export default {
  ...
  computed: {
    ...
    rank() {
      ...
      if (rankConf.second && rankConf.second != undefined && rankConf.second !== 0) {
  ...
  methods: {
    ...
    onRefresh() {
      this.surplusTime = {}
      this.downTimeGo('time' + this.rankKey, this.rank.second)
    ...
    add() {
      if (this.giftMsgItem.user_max == 0 && this.gifNum < this.giftMsgItem.total_max) { //不限购
        ...
        this.tastMsg = `Persediaan tidak cukup!`
        this.showT = true
        ...
        if ((this.gifNum * 1 + this.giftMsgItem.buy_counts * 1) < this.giftMsgItem.user_max) {
          ...
          this.tastMsg = `Hanya bisa membelinya sebanyak ${this.giftMsgItem.user_max} buah!`
          this.showT = true
        }
      }
    },
    queryBuy(id) { //确定购买
      if (this.giftMsgItem.pay_type == 1 && this.user_msg.coins * 1 < this.giftMsgItem.amount * 1) {  //金币不足
        this.tastMsg = `Koin emas yang tersisa tidak mencukupi!`
        this.showT = true
      } else if (this.giftMsgItem.pay_type == 2 && this.user_msg.beans * 1 < this.giftMsgItem.amount * 1) { //金豆不足
        this.tastMsg = `Kacang emas  tidak cukup!`
        this.showT = true
      } else if (this.gifNum * 1 > this.giftMsgItem.stock * 1) { //库存不足
        this.tastMsg = `Persediaan tidak cukup!`
        this.showT = true
      } else {
        api.buyGift(id, this.gifNum).then(res => {
          if (res.data.response_status.code === 0) {
            ...
            this.tastMsg = `Telah berhasil membeli!</br>Hadiah telah dikirimkan di ransel Anda</br>Silakan cek!`
            this.showT = true
          } else {
    ...
    showBuyItemPup(item, index) {
      globalBus.$emit('commonEvent', () => {
        if (this.rank.gift_status != 1) {
          this.tastMsg = `Tidak dalam waktu round saat ini!`
          this.showT = true
        } else if (item.stock == 0) { //库存不足
          this.tastMsg = `Persediaan tidak cukup!`
          this.showT = true
        } else if (item.user_max > 0 && item.user_max == item.buy_counts) { //礼物限购并且购买达到上限
          this.tastMsg = `Hanya bisa membelinya sebanyak ${item.user_max} buah!`
          this.showT = true
        } else {
        ...
        if (this.rank.gift_status == 0) {
          api.make(gid).then(res => {
            if (res.data.response_status.code == 0) {
              ...
              this.tastMsg = `Pengingat push yang berhasil dijadwalkan!`
              this.showT = true
            } else {
              this.tastMsg = res.data.response_status.error
              this.showT = true
