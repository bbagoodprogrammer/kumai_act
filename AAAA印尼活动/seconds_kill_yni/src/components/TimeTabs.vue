  ...
  <div>
    <div class="tabs">
      <div v-for="(item,index) in days" :key="index" @click.prevent="changeDay(item)" :class="{current:item==day}">
        ...
        <!-- 当前场次是否开始 -->
        <em v-if="item==nowTab && rankGroups[nowTab].gift_status == 1">Berlangsung</em>
        <em v-else-if="item < nowTab|| (item==nowTab && rankGroups[nowTab].gift_status == 2)">Telah berakhir</em>
        <em v-else>Akan mulai</em>
      </div>
    </div>
  </div>
