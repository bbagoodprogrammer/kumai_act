<template>
  <div class="page pageHistory">
    <div class="title"><i class="back" @click="$router.go(-1)"></i> {{lang.history_title}}</div>
      ...
      <template slot-scope="{list, loading, none}">
        <ul>
          <li v-for="(item,index) in list" :key="index" @click="goDetail(item)">
            <div class="name"><strong class="nick">{{item.creator_nick}}</strong> {{lang.history_startPeople}} <strong>{{item.prise_name}}{{item.prise_str}}</strong> {{lang.history_startPeople2}}</div>
            <div class="time">{{item.end_date}}{{lang.history_open}}</div>
            <i class="arr"></i>
          </li>
        </ul>
        <p class="lastTips">{{lang.history_threeDay}}</p>
        <div v-show="loading" class="scrollLoading">{{lang.common_loading}}</div>
        <div v-show="none" class="scrollNone">{{lang.common_noData}}</div>
      </template>
    </InnerScrollLoadList>
  </div>
</template>
