<template>
  <div>
    <div class="footerBar">
      <div class="acrStatus">
        <span class="goAct" v-if="astState === 1" @click="singUp()">Báo danh tham gia</span>
        <span class="actIng" v-if="astState === 3">
          <span class="type1" @click="showVuePopup(true)" v-if="owner_msg.bets && owner_msg.bets.length===0" :class="{gray:actStatus === 2}">Đặt</span>
          <span class="type2" v-else>Ván này đã đặt</span>
        </span>
      </div>
    </div>
    <transition name="slide">
      <div class="vue-popup" v-if="astState === 3" v-show="show2">
        <div class="popup-content">
          ...
          <div class="title">
            <div class="left">Điểm còn: <em>{{owner_msg.b}}</em></div>
            <div class="right">Đặt ít nhất 50,</br>
              mỗi lần + - tối thiểu 10</div>
          </div>
          <div class="pointMsg">
            ...
            <div v-if="actPoints2.key" class="point2">
              <p>
                Trúng 1 thưởng nhân 1, trúng 2 thưởng nhân 2,
            ...
            <p v-show="!actPoints1.key && !actPoints2.key && !actPoints3.key" class="noChange">Bạn chưa chọn cách đặt cược</p>
          </div>
          <p class="msg">
            Ván này đặt {{totalNum}}, sau khi xác nhận không thể thêm</br>
            hoặc đổi Thời gian mở thưởng: {{hours}}
          </p>
          <div class="changBtn">
            <span v-if="actPoints1.key || actPoints2.key || actPoints3.key" @click="betting()">Xác nhận</span>
            <span v-else @click="showVuePopup(false)">Mời đặt</span>
          </div>
        </div>
      </div>
    </transition>
  ...
  methods: {
    ...
    change1() {
      if (this.coins1 < 50) { //提示要大于五十
        this.coins1 = 50
        this.tastMsg = `Đặt ít nhất 50`
        this.showT = true
      } else if (this.coins1 % 10 !== 0) {
        this.coins1 = 50
        this.tastMsg = `mỗi lần + - tối thiểu 10`
        this.showT = true
      }
    },
    reduce2() {
