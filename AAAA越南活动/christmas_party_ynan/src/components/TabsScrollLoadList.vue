<template>
  <div class="rankGroups">
    <!-- 日榜、总榜切换主Tabs -->
    <div class="mainTabs">
      <span class="taskTab" @click.prevent="mainTabClick(2)" :class="{current:mainTab==2}" href="">Nhiệm vụ ngày</span>
      <div class="tabs">
    ...
    <p class="scoreTips">Điểm An Lành = điểm nhiệm vụ + mị lực từ quà xu bài hát/phòng kara</p>
    <div class="downTimebox" v-if="mainTab == 0">
      <p v-if="tab == nowDay">Đếm ngược kết thúc BXH hôm nay</p>
      <p v-else-if="tab > nowDay">Đếm ngược mờ BXH ngày này</p>
    ...
    <ul v-if="mainTab==0" class="list day">
      <li v-for="(item,index) in rank.list" :key="index" :class="'rank'+item.rank" @click="goUser(item.uid)">
        ...
        <div class="userMsg">
          ...
          <div class="probability">Tỷ lệ trúng: <em>{{(item.scale*100).toFixed(2)+'%'}}</em></div>
        </div>
        <div class="score">Điểm An Lành: <em>{{item.score}}</em></div>
      </li>
    </ul>
    <!-- 总榜 -->
