<template>
  <div class="footer">
    ...
    <div class="singUpBox" v-if="showState ===2 || isShare">
      <span class="singUp" @click="goSingUp()">Báo danh</span>
    </div>
    <div class="list" v-if="showState ===3 ">
      ...
      <div class="userMsg">
        <div class="name">
          <span v-if="tab == 'total'" class="score">Điểm: <em>{{myMsg.score}}</em></span>
          <span v-else>{{`${getday(dateArr[this.tab-1])}  Điểm:`}} <em>{{myMsg.score}}</em></span>
        </div>
        ...
        <div class="probability" v-else> <i class="tips" v-if="myMsg.is_lucky == 1"></i> Tỷ lệ trúng: <em>{{(myMsg.scale*100).toFixed(1)+'%'}}</em></div>
      </div>
      <div class="upDateSong" @click="updateSong()">Đăng bài</div>
    </div>
