      ...
      <div class="receivePup" v-if="showGiftPup">
        <div class="receivePupCon">
          ...
          <div class="type" v-if="type == 1">
            ...
            <p class="giftName" v-else>Quà ngẫu nhiên X 2</p>
            <p class="bean" v-if="giftIndex== 0">(Đạt liên tiếp {{gift_list.continue_login_counts}} ngày)</p>
            <span class="lingqu" :class="{act:gift_list.gift_list[giftIndex].is_get == 0 && sucStep>=(giftIndex+1)}" @click="receive()">{{gift_list.gift_list[giftIndex].is_get==0?'Nhận':'Đã nhận'}}</span>
          </div>
          <div class="type2" v-else-if="type == 2">
            ...
            <p class="sucTips">
              Phần thưởng đã gửi vào tài khoản, mời kiểm tra
            </p>
          </div>
          <div class="type3" v-else-if="type == 3">
            ...
            <span class="succeTitle">Đã nhận!</span>
            <p class="sucTips">
              Phần thưởng đã gửi vào tài khoản, mời kiểm tra
            </p>
          </div>
...
export default {
  ...
  computed: {
    ...
    lineWidth() {
      // if (this.integral > 70) {
      //   return (this.integral - 70) / 80 * 100 + '%'
      // }
      // return 0
      if (this.integral >= 150) {
  ...
  methods: {
    getName(name) {
      return name.replace('XB', this.gift_list.gift_list ? this.gift_list.gift_list[0].counts : 0)
    },
    getNameTwo(name) {
      let giftTwoName = gift[this.nowDay][this.gift_name].name
      return giftTwoName.replace('XB', this.gift_list.gift_list ? this.gift_list.gift_list[0].counts : 0).replace('*1', '') + '*2'
    },
    showGift(index, item) {
    ...
    getDengji() {
      return lang.todayp.replace('XB', lang.todayGift[this.giftIndex].num)
    },
    getPupImg() {
...
.integral {
  ...
  .intergralCon {
    ...
    .line {
      ...
      margin: 0.23rem 0 0.12rem 0.45rem;
      position: relative;
      .actLine {
        height: 0.12rem;
        position: absolute;
