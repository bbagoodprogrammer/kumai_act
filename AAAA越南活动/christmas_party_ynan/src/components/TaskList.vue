<template>
  <div class="taskList">
    <div class="rankListCon">
      ...
      <div class="rankTitle">
        <span class="day">Nhiệm vụ Giáng Sinh</span>
        <span class="point">Điểm <i></i></span>
        <span class="du">Tiến độ</span>
      </div>
      <ul>
        <li v-for="(item,index,i) in daily_tasks" :key="index" :class="{act:i %2 == 0}">
          ...
          <span class="du">
            <span class="state">
              ...
              <span v-show="item.finish_count == item.task_count" class="suc">Đã xong</span>
              <b v-if="item.task_list.length > 1" @click="showWhole(item.finish_count,item.task_list)">Xem thêm</b>
            </span>
          </span>
        </li>
      </ul>
    ...
    <transition name="slide">
      <div class="whole" v-if="wholePup">
        <div class="wholeCon">
          ...
          <div class="title">Mốc hiện tại</div>
          <div class="intrgral">
            <span>0</span>
  ...
  data() {
    return {
      rankName: {
        ...
        last_publicWork: "Không xoá bài hát công khai đã đăng hôm trước ",
        praise: "Nhấn like cho 10 bài hát ",
        share: "Chia sẻ 1 tác phẩm ra mạng xã hội",
        online: "Vào phòng Kara bất kỳ 15 phút trở lên",
        kmic_start: "Lên mic tại phòng Kara bất kỳ 1 lần",
        sendBean: "Tặng 100 đậu cho bài hát hoặc trong phong Kara trong ngày ",
        charge: "Tổng xu nạp trong ngày đạt",
        sendCoin: "Tổng xu quà tặng bài hát hoặc phòng kara trong ngày đạt",
        // praise: "Nghe 10 bài",
      },
      wholePup: false,
      nowTask_list: [],
