  ...
  <div class="rankGroups">
    ...
    <div class="downTime" v-if="tab >= nowTab && !surplusTime.end && rank.second!==0">
      <div class="title" v-if="tab == nowTab && rank.gift_status ==1">
        Đếm ngược kết thúc đợt này
      </div>
      <div class="title" v-else-if="tab >= nowTab">
        Đếm ngược mở bán đợt này
      </div>
      <div class="timeItem">
    ...
    <ul v-if="mainTab==0" class="list day">
      <li v-for="(item,index) in rank.list" :key="index">
        ...
        <div class="gifMsg">
          ...
          <div class="quota" v-if="item.user_max != 0">Mỗi đợt chỉ được mua {{item.user_max}} món</div>
          <div class="sold">
            <em v-if="((1-(item.stock / item.total_max))* 100)%1 == 0">Đã bán {{((1-(item.stock*1 / item.total_max*1))* 100)+ '%'}}</em>
            <em v-else>Đã bán {{((1-(item.stock*1 / item.total_max*1))* 100).toFixed(1) + '%'}}</em>
    ...
    <div v-if="rank.none" class="scrollNone">Danh sách trống!</div>
    ...
    add() {
      if (this.giftMsgItem.user_max == 0 && this.gifNum < this.giftMsgItem.total_max) { //不限购
        ...
        if ((this.gifNum * 1 + this.giftMsgItem.buy_counts * 1) < this.giftMsgItem.user_max) {
          ...
          this.tastMsg = `Đợt này chỉ được mua tối đa ${this.giftMsgItem.user_max} món!`
          this.showT = true
        }
      }
    },
      ...
      if (this.giftMsgItem.pay_type == 1 && this.user_msg.coins * 1 < this.giftMsgItem.amount * 1) {  //金币不足
        ...
        api.buyGift(id, this.gifNum).then(res => {
          if (res.data.response_status.code === 0) {
            ...
            this.tastMsg = `Đã mua!</br>Quà đã gửi vào Túi của bạn</br>Mời kiểm tra!`
            this.showT = true
          } else {
    ...
    showBuyItemPup(item, index) {
      globalBus.$emit('commonEvent', () => {
        if (this.rank.gift_status != 1) {
          this.tastMsg = `Không trong thời gian mở bán!`
          this.showT = true
        } else if (item.stock == 0) { //库存不足
          this.tastMsg = `Đã hết hàng!`
          this.showT = true
        } else if (item.user_max > 0 && item.user_max == item.buy_counts) { //礼物限购并且购买达到上限
          this.tastMsg = `Đợt này chỉ được mua tối đa ${item.user_max} món!`
          this.showT = true
        } else {
        ...
        if (this.rank.gift_status == 0) {
          api.make(gid).then(res => {
            if (res.data.response_status.code == 0) {
              ...
              this.tastMsg = `Đã mở thông báo nhắc nhở!`
              this.showT = true
            }else {
              this.tastMsg = res.data.response_status.error
              this.showT = true
