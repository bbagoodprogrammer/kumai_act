<template>
  <div class="rankGroups">
    <!-- 日榜、总榜切换主Tabs -->
    <div class="mainTabs">
      <a @click.prevent="mainTabClick(0)" :class="{current:mainTab==0}" href="">BXH Lì Xì</a>
      <a @click.prevent="mainTabClick(1)" :class="{current:mainTab==1}" href="">Bí kíp Lì Xì</a>
    ...
    <div v-if="mainTab==0" class="tabs">
      <a @click.prevent="tabClick(0)" :class="{current:tab==0}" href="">BXH Chuyên Gia</a>
      <a @click.prevent="tabClick(1)" :class="{current:tab==1}" href="">BXH May Mắn</a>
    </div>
    <div class="downTimebox" v-if="mainTab == 0">
      <p v-if="actStatus == 0">Đếm ngược mở sự kiện</p>
      <!-- 活动结束 -->
      <p v-else-if="actStatus == 1">Đếm ngược kết thúc sự kiện</p>
      <p v-else> Sự kiện đã kết thúc！</p>
