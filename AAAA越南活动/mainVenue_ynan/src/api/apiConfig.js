...
function getDefault() {
    if (token) {
        return get(`/ye201900/init.php?token=${token}`);
    } else {
        return get(`/ye201900/shareinit.php`);
    }
}
//榜單
function rank(act_id) {
    if (token) {
        return get(`/ye201900/gettotalrank.php?token=${token}&act_id=${act_id}`);
    } else {
        return get(`/ye201900/sharegettotalrank.php?act_id=${act_id}`)
    }
}
//總榜
function getTotelRank() {
    if (token) {
        return get(`/ye201900/alltotalrank.php?token=${token}`);
    } else {
        return get(`/ye201900/sharealltotalrank.php`)
    }
}

//上報
function upadateAct(tid) {
    return axios.get(`/ye201900/actrecord.php?token=${token}&&act_id=${tid}`)
}
const httpConfig = {
    getDefault,
    rank,
