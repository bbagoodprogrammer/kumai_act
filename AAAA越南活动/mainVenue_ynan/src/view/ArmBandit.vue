<template>
  <div class="box">
    ...
    <div class="header">
      <span class="ruleTips" :class="{top:isShare}" @click="goRule()">Thể lệ</span>
      <div class="bigBg"></div>
      ...
      <div class="title">
        <div class="actTime">13/01/2020 – 14/02/2020</div>
      </div>
      <div class="titleLate">
        <div class="slide"></div>
      </div>
    ...
    <div class="main">
      <div class="tabs">
        <a :class="{act:showCom == 'Match'}" @click="tabClick('Match')">Cuộc thi cuối năm</a>
        <a :class="{act:showCom == 'People'}" @click="tabClick('People')">BXH Ngôi Sao Của Năm</a>
      </div>
      <component :is="showCom"></component>
    </div>
    <p class="tips"> Quyết định của Karaoke Now là quyết định cuối cùng</p>
    <loading></loading>
...
export default {
  components: { Loading, MsgToast, Match, People },
  data() {
    return {
      activite: 1,    //活动状态
      showT: false,  //提示弹窗显示
    ...
    getDefaultData(val) { //初始化
      api.getDefault().then(res => {
        ...
        if (response_status.code == 0) {
          ...
          let act_list = response_data.act_list
          let newObj = act_list.map((item, index) => {
            return { ...item, ...this.actTimeArr[index] };
          });
          this.$store.commit('changActList', newObj)  //活动列表
