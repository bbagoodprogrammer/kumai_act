<template>
  <div class="footer">
    ...
    <div class="end" v-if="showState ===4">
      Bạn đã là ca sĩ, không thể tham gia sự kiện
    </div>
    <div class="singUpBox" v-if="showState ===2 || isShare">
      <span class="singUp" @click="goSingUp()">Báo danh tham gia</span>
    </div>
    ...
    goSingUp() {
      ...
      api.singUpMsg(listChang).then((res) => {
        ...
        if (response_status.code === 20007) {  //歌手等级大于四级
          this.tastMsg = "Cấp ca sĩ ≤4 mới được tham gia nhé!"
          this.showT = true
        } else if (response_status.code === 20005) { //已经报名
          this.tastMsg = "Đã báo danh!"
          this.showT = true
        } else if (response_status.code === 0) {  //报名成功
          this.tastMsg = "Đã báo danh! </br>Mau vào phòng Kara lên mic nhé~"
          this.showT = true
          //当前天数排行榜添加用户数据
