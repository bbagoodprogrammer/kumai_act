  ...
  <div class="rankGroups">
    ...
    <div class="downTime" v-if="tab >= nowTab && !surplusTime.end && rank.second!==0">
      <div class="title" v-if="tab == nowTab && rank.gift_status ==1">
        العد التنازلي لانتهاء الجولة
      </div>
      <div class="title" v-else-if="tab >= nowTab">
        العد التنازلي لبدء الجولة
      </div>
      <div class="timeItem">
    ...
    <ul v-if="mainTab==0" class="list day">
      <li v-for="(item,index) in rank.list" :key="index">
        ...
        <div class="gifMsg">
          ...
          <div class="quota" v-if="item.user_max != 0">يمكن شراء {{item.user_max}} فقط لكل حساب</div>
          <div class="sold">
            <em v-if="((1-(item.stock / item.total_max))* 100)%1 == 0">يبيع {{'%'+((1-(item.stock*1 / item.total_max*1))* 100)}}</em>
            <em v-else>يبيع {{'%'+((1-(item.stock*1 / item.total_max*1))* 100).toFixed(1)}}</em>
    ...
    <div v-if="rank.none" class="scrollNone">لا بيانات الآن!</div>
...
export default {
  ...
  data() {
    return {
      ...
      totalDay: 0, //总得场次
      giftType: [
        `تنجح في الشراء! تصل هذه الهدية إليك، تفضل البتحقق!`,
        ``,
        `تنجح في الشراء! تصل هذه التذكرة إليك، تفضل البتحقق!`,
        `تنجح في الشراء! يصل VIP إليك، تفضل البتحقق!`,
        `تنجح في الشراء! تصل هذه السيارة إليك، تفضل البتحقق!`
      ],
    };
  },
  computed: {
  ...
  methods: {
    ...
    add() {
      if (this.giftMsgItem.user_max == 0 && this.gifNum < this.giftMsgItem.total_max) { //不限购
        ...
        this.tastMsg = `البضائع ليست كافية!`
        this.showT = true
        ...
        if ((this.gifNum * 1 + this.giftMsgItem.buy_counts * 1) < this.giftMsgItem.user_max) {
          ...
          this.tastMsg = `يمكن شراء ${this.giftMsgItem.user_max} فقط لكل حساب!`
          this.showT = true
        }
      }
    },
    queryBuy(id) { //确定购买
      if (this.giftMsgItem.pay_type == 1 && this.user_msg.coins * 1 < this.giftMsgItem.amount * 1) {  //金币不足
        this.tastMsg = `فشل في الشراء! عملاتك ليست كافية، تفضل بالشحن!`
        this.showT = true
      } else if (this.giftMsgItem.pay_type == 2 && this.user_msg.beans * 1 < this.giftMsgItem.amount * 1) { //金豆不足
        this.tastMsg = `فشل في الشراء! فولاتك ليست كافية، تفضل بإكمال المهام اليومية لكسب الفول!`
        this.showT = true
        ...
        if (this.rank.gift_status == 0) {
          api.make(gid).then(res => {
            if (res.data.response_status.code == 0) {
              ...
              this.tastMsg = `تنجح في ضرب الموعد لرسائل الانتباه!`
              this.showT = true
            } else {
              this.tastMsg = res.data.response_status.error
              this.showT = true
          ...
          this.tastMsg = `ليس في وقت ضرب الموعد!`
          this.showT = true
        }
      })
    },
