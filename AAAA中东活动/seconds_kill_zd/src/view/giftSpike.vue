  ...
  <div class="box" :style="{backgroundImage: 'url(' + banner + ')', backgroundSize:'center',backgroundRepeat:'no-repeat'}">
    ...
    <div class="herder">
      <div class="spanBox" :class="{shareTop:isShare}">
        <span class="rules" @click="showRulesPup()">القواعد</span>
        <span class="history" @click="showBuyHistoryPup()">تفاصيل الشراء</span>
      </div>
    </div>
    ...
    <transition name="slide">
      <div class="rulesPup" v-show="showRules">
        <div class="rulesCon">
          ...
          <div class="title">القواعد</div>
          <p class="time">الوقت: <em>{{stime}} ~ {{etime}}</em> </p>
          <h6>القواعد：</h6>
          <p>1.يمكن أن يشتري المستخدمون البضائع بالعملات أو الفولات. كمية الشراء المحدودة تعتمد على شروط البضائع</p>
          <p>2.يشتمل هذا النشاط على مختلف الجولات، ولا يمكن شراء البضائع بعد بيع كلها! يمكن أن تضرب الموعد لرسائل الانتباه(لا بد أن تفتح صلاحية رسائل الانتباه)، بعد فتحها، سينتبهك النظام تلقائيا، فلا تفوتك أي الجولة!</p>
          <p>3.البضائع التي تشتريها في هذا النشاط ستصل إلى حقيبتك</p>
          <p class="red">4.وقت الجولة كله بتوقيت دبي، تفضل بانتباه الوقت، لا تفوتك الجولة!</p>
          <p class="tips">التفسير النهائي لهذا النشاط ينتمي إلى المنظم</p>
        </div>
      </div>
    </transition>
    <transition name="slide">
      <div class="buyHistoryPup" v-show="showBuyHistory">
        <div class="buyHistoryPupCon">
          ...
          <div class="title">تفاصيل الشراء</div>
          <div class="listTitle">
            <span class="giftName">بضائع</span>
            <span class="time">الوقت</span>
            <span class="num">كمية</span>
          </div>
          <ul class="scrollable">
