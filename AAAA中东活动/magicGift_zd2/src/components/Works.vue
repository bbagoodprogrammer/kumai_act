<template>
  <div class="words" :class="{active:activateState === 0}">
    <div class="wardsSample">
      <!-- <p>按報名后參賽作品收到以下禮物魅力值排名</p> -->
      <div class="wards">
        <div class="wardsImg">
          <p>الترتيب حسب السحر لأعمال من الهدايا التالية</p>
          <span class="water">
            <em>طوف من الخيزران</em>
          </span>
          <span class="cun">
            <em>كشك الهاتف</em>
          </span>
          <span class="car">
            <em>روبوت القط</em>
          </span>
        </div>
      </div>
      <!-- <p class="fansong">向參賽作品送以上禮物，送禮者凌晨返金幣數的5%金幣</p> -->
    </div>
    <div class="wordsList" v-if="activateState !== 0">
      <div class="listTop">
        <div class="listItem" :class="'top'+ (index+1)" v-for="(item,index) in top" @click="goPersonalMsg(item.uid)" :key=index>
          <span>{{item.rank}}</span>
          <div class="userBox">
            <div class="imgBox">
              <img src="../assets/img/sunmmer.png" alt="" class="img1" v-if="index ===0">
              <img v-lazy="item.avatar" alt="" class="img2">
            </div>
            <div class="userMsg">
              <p class="name">{{item.nick}}</p>
              <!-- <p class="id">UID:{{item.uid}}</p> -->
              <p class="charm">{{item.score}}</p>
            </div>
          </div>
          <div class="wardsMsg">
            <!-- <span v-for="(item2,index) in item.gifts" :style="{'backgroundImage': 'url('+item2.img+')',backgroundSize:'contain'}" :key="index"> X {{item2.total}}</span> -->
            <span v-for="(item2,index) in item.gifts" :key=index>
              <em :style="{'backgroundImage': 'url('+item2.img+')',backgroundSize:'contain',backgroundRepeat:'no-repeat'}"></em>{{item2.total}}<i>X</i>
            </span>
          </div>
        </div>
      </div>
      <div class="listBottom">
        <div class="listItem" v-for="(item,index) in bottom" @click="goPersonalMsg(item.uid)" :key=index>
          <span>{{item.rank}}</span>
          <div class="userBox">
            <div class="imgBox">
              <img v-lazy="item.avatar" alt="">
            </div>
            <div class="userMsg">
              <p class="name">{{item.nick}}</p>
              <!-- <p class="id">UID:{{item.uid}}</p> -->
              <p class="charm">{{item.score}}</p>
            </div>
          </div>
          <div class="wardsMsg">
            <span v-for="(item2,index) in item.gifts" :key=index>
              <em :style="{'backgroundImage': 'url('+item2.img+')',backgroundSize:'contain',backgroundRepeat:'no-repeat'}"></em>{{item2.total}}<i>X</i>
            </span>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import { mapState } from "vuex"
export default {
  computed: {
    ...mapState(["worksArr", "activateState"]),
    top() {
      if (this.worksArr !== []) {
        return this.worksArr.slice(0, 3)
      }
    },
    bottom() {
      if (this.worksArr !== []) {
        return this.worksArr.slice(3)
      }
    }
  },
  methods: {
    goPersonalMsg(id) {
      location.href = `uid:${id}`
    }
  }
}
</script>

<style lang="scss" scope>
.words {
  padding-top: 0.13rem;
  &.active {
    padding-bottom: 1.8rem;
  }
  .wardsSample {
    width: 7.5rem;
    // height: 3rem;
    // background: url(../assets/img/shanhu.png);
    padding-bottom: 0.2rem;
    background-size: 100% 100%;
    p {
      height: 0.5rem;
      text-align: center;
      color: #c3bfff;
    }
    .wards {
      // height: 1.72rem;
      .wardsImg {
        background: url(../assets/img/ward1.png) no-repeat;
        background-size: 100% 100%;
        margin: 0.2rem auto 0;
        width: 5rem;
        height: 2.36rem;
        position: relative;
        > p {
          width: 100%;
          position: absolute;
          font-size: 75%;
          text-align: center;
          line-height: 0.5rem;
        }
        > span {
          display: inline-block;
          width: 33%;
          font-size: 80%;
          position: absolute;
          top: 1.9rem;
          color: #c3bfff;
          img {
            display: block;
            width: 1.26rem;
            height: 1.23rem;
            margin: 0 auto;
          }
          em {
            display: block;
            width: 100%;
            text-align: center;
            white-space: nowrap;
            font-size: 80%;
            // margin-top: 0.1rem;
          }
          &.water {
            // left: -0.4rem;
          }
          &.cun {
            left: 1.68rem;
          }
          &.car {
            right: 3.35rem;
          }
        }
      }
    }
    .fansong {
      font-size: 80%;
      color: #6ffff3;
      text-align: center;
      line-height: 0.5rem;
    }
  }
  .wordsList {
    margin: 0.05rem 0.15rem 0;
    padding-bottom: 1.6rem;
    .listTop {
      .userBox {
        .userMsg {
          margin-left: 0;
        }
      }
      .listItem {
        > span {
          display: block;
          margin: 0 0 0 0.1rem;
          width: 0.84rem;
          height: 0.73rem;
          text-indent: -999rem;
        }
      }
      .top1 {
        > span {
          background: url(../assets/img/top1.png);
          background-size: 100% 100%;
        }
        .imgBox {
          img {
            border: none !important;
          }
        }
      }
      .top2 {
        > span {
          background: url(../assets/img/top2.png);
          background-size: 100% 100%;
        }
      }
      .top3 {
        > span {
          background: url(../assets/img/top3.png);
          background-size: 100% 100%;
        }
      }
    }
    .listBottom {
    }
    .listItem {
      height: 1.24rem;
      // background: url(../assets/img/listItem.png);
      // background-size: 100% 100%;
      border-bottom: 0.02rem solid #4f32cd;
      display: flex;
      align-items: center;
      margin-top: 0.05rem;
      position: relative;
      > span {
        width: 0.5rem;
        margin: 0 0.2rem 0 0.25rem;
        text-align: center;
        color: #ffbb8d;
        font-size: 120%;
        font-weight: bold;
      }
      .userBox {
        width: 3rem;
        height: 1rem;
        display: flex;
        align-items: center;
        .imgBox {
          width: 1.2rem;
          height: 1.1rem;
          // background: url(../assets/img/sunmmer.png);
          // background-size: 100% 100%;
          z-index: 50;
          position: relative;
          img {
            margin: 0.1rem 0 0 0;
            width: 0.86rem;
            height: 0.86rem;
            border-radius: 50%;
            border: 0.02rem solid rgba(255, 238, 120, 1);
          }
          .img1 {
            width: 0.93rem;
            height: 1.04rem;
            z-index: 999;
            position: absolute;
            margin: 0;
            border: none;
          }
          .img2 {
            display: inline-block;
            width: 0.86rem;
            height: 0.86rem;
            position: absolute;
            top: 0.15rem;
            right: 0.05rem;
            margin: 0;
          }
        }
        .userMsg {
          position: relative;
          height: 1rem;
          .name {
            position: absolute;
            font-size: 85%;
            width: 4.3rem;
            max-width: 4.3rem;
            overflow: hidden;
            text-overflow: ellipsis; //超出部分以省略号显示
            white-space: nowrap;
            color: #fffcef;
          }
          .id {
            color: #86edf1;
            font-size: 75%;
            height: 0.3rem;
          }
          .charm {
            height: 0.33rem;
            // padding-left: 0.35rem;
            // background: url(../assets/img/charm.png) no-repeat;
            // background-size: 0.28rem 100%;
            font-size: 80%;
            color: #29fff7;
            line-height: 0.33rem;
            margin-top: 0.55rem;
          }
        }
      }
      .wardsMsg {
        width: 3.8rem;
        height: 0.4rem;
        display: flex;
        // justify-content: space-between;
        // padding-right:.5rem;
        color: #6ffff3;
        font-size: 80%;
        line-height: 0.4rem;
        margin-top: 0.5rem;
        position: absolute;
        top: 0.15rem;
        left: 0rem;
        > span {
          display: inline-block;
          margin-right: 0.1rem;
          font-size: 80%;
          color: #fff8ab;
          width: 1.2rem;
          em {
            display: inline-block;
            width: 0.39rem;
            height: 0.39rem;
            float: right;
            margin-left: 0.05rem;
          }
          i {
            font-size: 80%;
            margin-left: 0.05rem;
          }
        }
        > span:first-child {
          // width: 0.95rem;
          width: 1.3rem;
          margin-right: 0;
        }
        > span:nth-child(2) {
          // width: 0.75rem;
        }
        > span:nth-child(3) {
        }
      }
    }
  }
}
</style>
