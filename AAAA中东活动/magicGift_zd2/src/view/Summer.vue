  ...
  <div class="bigBox">
    ...
    <div class="footer" v-if="!isShare">
      ...
      <div class="myMsg" v-show="showComponent === 'works' && (showFooter === 3 || showFooter == 4)">
        <div class="rank"><span>ترتيب الأعمال:</span><em>{{WardsMyMsg.rank}}</em></div>
        <img :src="WardsMyMsg.avatar" alt="">
        <div class="msgBox">
          <div class="msgTop"><span>{{WardsMyMsg.nick}}</span>
          </div>
          <div class="msgBottom">
            قيمة السحر لأعمالك:</br><span>{{WardsMyMsg.score}}</span>
          </div>
        </div>
        <div class="unpload" @click="uploadSong()">رفع أعمالك</div>
      </div>
      <div class="myMsg" v-show="showComponent === 'kRoom' && (showFooter === 3 || showFooter == 4)">
        <div class="rank"><span>ترتيب الغرف:</span><em>{{kRommMyMsg.rank}}</em></div>
