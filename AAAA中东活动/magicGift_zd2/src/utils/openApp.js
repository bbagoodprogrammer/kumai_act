...
export default function openApp(schemaUrl, androidDownUrl, iosDownUrl, iosSchemaUrl) {
    schemaUrl = schemaUrl || 'hsing://';
    androidDownUrl = androidDownUrl || 'http://17sing.tw';
    iosDownUrl = iosDownUrl || 'http://17sing.tw/goto_appstore';
    // iosSchemaUrl = iosSchemaUrl || schemaUrl;
    iosSchemaUrl = iosSchemaUrl || 'encore://';
    if (ifChrome) {
        if (ifAndroid) {
            setTimeout(function () {
