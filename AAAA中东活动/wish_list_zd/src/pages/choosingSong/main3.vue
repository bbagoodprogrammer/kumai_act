<template>
  <div class="ChoosingSong">
    <div class="top">
      <textarea maxlength="300" placeholder="تفضل بكتابة  رغبة كلمات أكثر من أربعة ألفاظ لرفع نسبة الإكمال لرغبتك!" @input="descInput()" v-model="desc">
      </textarea>
      <span class="strLength">{{remnant}}/300</span>
    </div>
    <div class="bottom">
      <p class="title" v-if="!isChoosing">تفضل باختيار أغنية الرغبة</p>
      ...
      <p class="tips">
        يمكن نشر رغبتك بعد الكتابة الكاملة
      </p>
    </div>
    <div class="footer" :class="{active:isChoosing&& desc!== ''}" @click="Release()">
      <span>نشر طلب الأغنية</span>
    </div>
    <transition name="slide">
      <div class="choosingSongPup" v-show="showSongList">
        <div class="shear">
          <input type="text" placeholder="اسم الأغنية\اسم المطرب" v-model="sheraVal" />
          <span class="go" @click="goSearch()"></span>
        </div>
        <p v-if="songList.length == 0" class="noSong">لا توجد الأغنية الآن</p>
        <ul class="scrollable">
