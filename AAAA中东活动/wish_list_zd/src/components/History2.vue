<template>
  <div class="history2">
    <p v-if="history2.list && history2.list.length == 0" class="tips">لا توجد الرغبات الآن</p>
    <ul class="scrollable" v-else>
      <li v-for="(item,index) in history2.list" :key="index">
        ...
        <div class="userBox">
          ...
          <div class="getUid">
            <span v-if="!item.fuser"> لم تُقطَف هذه الرغبة</span>
            <span v-else>قطف UID{{item.fuser.uid}} هذه الرغبة</span>
          </div>
        </div>
        <p class="time">وقت النشر: {{getTimeStr(item.ptime)}}</p>
      </li>
      <p class="loadingTips" v-show="loading">في التحميل...</p>
    </ul>
