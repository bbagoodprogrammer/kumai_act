    ...
    <div class="footerBar">
      <div class="acrStatus" :class="{black:userMsgState.count<=0}">
        ...
        <span class="goAct" v-if="astState === 1" @click="showChoosingPup()">
          <div class="title">نشر طلب الأغنية</div>
          <div class="shengyu">الرغبة الباقية اليوم:{{userMsgState.count}}</div>
    ...
    showChoosingPup() {
      globalBus.$emit('commonEvent', () => {
        if (this.userMsgState.count <= 0) {
          this.tastMsg = `ليس لديك عدد الرغبة الكافي، فلا يمكن نشر الرغبة. فتفضل بذهاب هنا غدا`
          this.showT = true
        } else {
