<template>
  <div class="share">
    <div class="shareCon">
      ...
      <div class="shareBtn">
        <input type="number" name="" id="" placeholder="ابحث عن UID أصدقائك" v-model="uid" @focus="focus()">
        <span class=" go" @click="shareUid()">{{lang.Determine}}</span>
      </div>
      <p class="noPeople" v-if="peopleList.length == 0">{{lang.noFriend}}</p>
      <ul class="rankList">
        <li v-for="(item,index) in peopleList" :key="index" :class="'rank'+item.rank" @click="gpPeople(item.uid)">
          ...
          <div class="uerMsg">
            ...
            <div class="uid">
              <em class="score">{{lang.PlayValue}}{{item.score}}</em>
            </div>
          </div>
          <div class="scoreBox">
            <span v-if="item.owner_praises > 0" class="type1">{{lang.ComeOned}}</span>
            <span v-else-if="item.receiver_praise == 3" class="type2 ">{{lang.fireeComeon}}</span>
            <span v-else-if="item.owner_praises == 0" class="type3" @click.stop="getPoint(item.uid,index)">{{lang.CommonTips}}</span>
          </div>
        </li>
      </ul>
    </div>
  ...
  methods: {
        ...
        if (this.uid != this.userMsg.uid) {
          api.chaUid(this.uid).then(res => {
            ...
            if (newRank.length > 0) {
              ...
              this.tastMsg = lang.queryNodata
              this.showT = true
            }
          })
        } else {
          this.tastMsg = lang.queryUs
          this.showT = true
        }
      }
    ...
    getPoint(uid, index) {  //点赞加积分
      if (this.userMsg.send_praise >= 3) { //當天點讚次數不足
        this.tastMsg = lang.CommeonFail
        this.showT = true
      } else {
        api.praise(uid).then(res => {
          if (res.data.response_status.code == 0) {
            this.$store.commit('addB', 5)
            this.tastMsg = lang.CommeonSuc
            this.showT = true
