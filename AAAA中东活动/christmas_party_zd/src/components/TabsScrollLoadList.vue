<template>
  <div class="rankGroups">
    <!-- 日榜、总榜切换主Tabs -->
    <div class="mainTabs">
      <span class="taskTab" @click.prevent="mainTabClick(2)" :class="{current:mainTab==2}" href="">مهام النقاط</span>
      <div class="tabs">
        <a @click.prevent="mainTabClick(0)" :class="{current:mainTab==0}" href="">قائمة يومية</a>
        <a @click.prevent="mainTabClick(1)" :class="{current:mainTab==1}" href="">قائمة إجمالية</a>
      </div>
    ...
    <p class="scoreTips">قيمة المهرجان=نقاط+قيمة السحر من هدايا تتلقاها في الأعمال أو غرفة الغناء</p>
    <div class="downTimebox" v-if="mainTab == 0">
      <p v-if="tab == nowDay">العد التنازلي لانتهاء القائمة اليومية</p>
      <p v-else-if="tab > nowDay">العد التنازلي لبدء القائمة اليومية</p>
      ...
      <div class="noTime" v-else>انتهى هذه القائمة اليومية</div>
    </div>
    <!-- 日榜 -->
    ...
    <ul v-if="mainTab==0" class="list day">
      <li v-for="(item,index) in rank.list" :key="index" :class="'rank'+item.rank" @click="goUser(item.uid)">
        ...
        <div class="userMsg">
          ...
          <div class="probability">نسبة المكافأة：<em>{{'%'+(item.scale*100).toFixed(1)}}</em></div>
        </div>
        <div class="score">قيمة المهرجان: <em>{{item.score}}</em></div>
      </li>
    </ul>
    <!-- 总榜 -->
    ...
    <div v-if="rank.loading" class="scrollLoading">في تحميل...</div>
    <div v-if="rank.none && mainTab==0 && (rank.list.length == 0 && (tab <= nowDay || tab ==='total'))" class="scrollNone">
      لا يوجد مستخدمون يدخولون هذه القائمة</br>
      هذه القائمة تنتظرك!
    </div>
    <div v-if="mainTab==0 && tab > nowDay" class="dengdai">تفضل بالتطلع إليه!</div>
  </div>
</template>
