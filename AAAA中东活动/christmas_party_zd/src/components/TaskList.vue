<template>
  <div class="taskList">
    <div class="rankListCon">
      ...
      <div class="rankTitle">
        <span class="day">مهام عيد الميلاد اليومية</span>
        <span class="point">نقاط <i></i></span>
        <span class="du">مرحلة</span>
      </div>
      <ul>
        <li v-for="(item,index,i) in daily_tasks" :key="index" :class="{act:i %2 == 0}">
          ...
          <span class="du">
            <span class="state">
              ...
              <span v-show="item.finish_count == item.task_count" class="suc">إكمال </span>
              <b v-if="item.task_list.length > 1" @click="showWhole(item.finish_count,item.task_list)">نظر الكل</b>
            </span>
          </span>
        </li>
      </ul>
    ...
    <transition name="slide">
      <div class="whole" v-if="wholePup">
        <div class="wholeCon">
          ...
          <div class="title">مرحلة حاليا</div>
          <div class="intrgral">
            <span>0</span>
  ...
  data() {
    return {
      rankName: {
        ...
        praise: "قم بالإعجاب لعشر أعمال مختلفة",
        ...
        sendCoin: "تصل قيمة الهدايا التي تهديها لأعمال أو غرفة الغناء إلى",
        // praise: "給10個不同的作品點讚",
      },
      wholePup: false,
      nowTask_list: [],
