<template>
    <div class="box">
        ...
        <avtivity-contion v-else></avtivity-contion>
...
export default {
    components:{avtivityContion,ShareComponent,ActivityFooter,MsgToast,Loading},
    data(){
        return{
            isShare:false, //是否分享
        ...
        getDefaultData(val){
             api.getDefault(0).then((res)=>{
                if(res.data.response_status.code == 20000){  //活动未开始
                    ...
                    //     }
                    // }
                    const {second,activity_status,red_rnvelope,user_rank,owner_msg,order} = res.data.response_data
