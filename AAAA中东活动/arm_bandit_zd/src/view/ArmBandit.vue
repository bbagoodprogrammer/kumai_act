<template>
  <div class="bandit">
    ...
    <div class="footer">
      ...
      <p class="toastMsg">ستحصل على نقد الفتحة الواحدة بعد شحن عملة واحدة لاشتراك في اليانصيب. الجوائز في 400 نقد الفتحة و2000 نقد الفتحة و5000 نقد الفتحة مختلفة، تنفق نقود أكثر، وتكون الجوائز أوفر. ونسبة كسب الجائزة الفريدة أكبر!</p>
    </div>
    <loading></loading>
  ...
  created() {
    ...
    api.getDefault().then(function (res) { //请求基础数据
      ...
      that.activeMsg.lotterylist.forEach(function (item) {
        that.textArr.push(`مبروك! <em>${item.uid}</em> يكسب <em>${wards[item.prizeid]}</em> × <em>${item.num}</em>`)
      })
