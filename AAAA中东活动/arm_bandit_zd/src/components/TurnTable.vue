<template>
  <div class="turntable">
    ...
    <p class="coinsbox"> عدد نقد الفتحة الباقية:<span class="coins">{{coins}}</span></p>
  ...
  watch: {
    stime() {
      ...
      if (that.actState == 1) { //开始
        that.timer = getDate(new Date(Number(that.stime * 1000)), "rule") + ' ~ ' + getDate(new Date(Number(that.etime * 1000)), "rule")
      } else if (that.actState == 0) {//未开始
        that.timer = `لم يبدأ النشاط`
      } else if (that.actState == 2) { //一结束
        that.timer = `انتهى النشاط`
      }
