...
//获取活动基础信息
function getDefault() {
    return get(`/index.php?action=signInTask.getMedalTask&uid=${uid}&token=${token}&lang=${lang_str}`);

}
//補簽
function sateSingIn() {
    return get(`/index.php?action=signInTask.compensateSignIn&uid=${uid}&token=${token}&lang=${lang_str}`)
}

//獲取KEY
function gerKey() {
    return get(`/index.php?action=signInTask.inviteFriends&uid=${uid}&token=${token}&lang=${lang_str}`)
}

//查詢邀請信息
function queryShre(inviteCode) {
    return get(`/index.php?action=signInTask.getInvitedFriends&inviteCode=${inviteCode}&lang=${lang_str}`) //&uid=${uid}&token=${token}
}
