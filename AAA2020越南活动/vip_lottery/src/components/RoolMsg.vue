      ...
      <div class="roolBox">
        <ul class="lb" :class="{marquee_top:animate}">
          <li v-for="(item, index) in list" :key="index">
            <em>{{item.uid}} {{lang.luckEd}} {{getGiftName(item.pname)}}</em>
          </li>
        </ul>
      </div>
    </div>
...
export default {
  ...
  methods: {
    getWardName(item) {
      if (item.name) {
        return wards[item.etype].replace('X', item.num).replace('%', item.name)
      } else {
        return wards[item.etype].replace('X', item.num)
      }
    },
    // getNick(nick) {
    //   if (nick.length > 6) {
