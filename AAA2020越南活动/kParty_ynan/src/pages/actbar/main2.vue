  ...
  <div class="myParty">
    ...
    <ul>
      <li v-for="(item,index) in actList" :key="index" :class="[{max:item.apply_status != 0,black:item.status == 2},'list'+item.cover]">
        ...
        <div class="title">{{item.them}} <span v-if="item.status == 2"> - Tiệc đã kết thúc</span> </div>
        <div class="actCon">
        ...
        <span class="singMsg" @click="showPeople(item.id)" v-if="item.apply_status == 2 && item.is_reg != 1">{{lang.singUpDetil}}</span>
      </li>
    </ul>
