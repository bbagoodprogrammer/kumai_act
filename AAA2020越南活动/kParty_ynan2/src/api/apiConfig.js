...
//檢查是否房主貨管理員
function adminOrOwner(rid) {
    return get(`${actName}/adminOrowner.php?token=${token}&rid=${rid}`)
}

//申請活動
function commitAct(rid, them, nums, cover, desc, stime, etime, prize, is_reg, setType, host_uid) {
    let data = {
        token,
        rid,
        them,
...
function getPeopleList(id, from, isMore) {
    if (isMore) {
        return axios.get(`${actName}/regList.php?token=${token}&id=${id}&from=${from}`)
    }
