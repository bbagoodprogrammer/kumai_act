    ...
    <div class="acrStatus" :class="{noStart:astState == 0,actEd:astState == 2,singUp:astState == 1}" @click="singUp()">
      <div class="actIng" v-if="astState === 3">
        <div class="total" v-if="tab != 'total'">
          ...
          <div class="songMsg">
            <div class="mySong">Số bài hát dự thi:{{nowMsg.total}}</div>
            <div class="topNum">Bài hát tăng tốc:{{nowMsg.up}}</div>
          </div>
          <div class="commitSongBtn" @click="commitSong()">Đăng bài hát</div>
        </div>
        <div class="left" :class="'rank' +nowMsg.rank" v-else>
          ...
          <div class="score">
            <div class="lv">Lv.{{nowMsg.level}} <em class="lvScore">Điểm Rực Rỡ:{{nowMsg.score}}</em> </div>
            <div class="iconScore">
