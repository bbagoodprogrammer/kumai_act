<template>
  <div class="giftBox">
    <div class="userLv">
      <div class="lv">Cấp của tôi: <strong>Lv.{{packets.level}}</strong></div>
      <div class="score">Điểm Rực Rỡ: <strong>{{packets.score}}</strong> <i class="quest" @click="showTipsPup = true"></i></div>
    </div>
    <div class="liner">
    ...
    <div class="mask" v-show="showBoxGift">
      <transition name="slide">
        <div class="boxGifts" v-if="showBoxGift">
          ...
          <strong class="needLv">(tức đạt cấp Lv.{{showBoxIndex}})</strong>
          <div class="giftList">
          ...
          <div class="getStatus" :class="{act:isOpen}" @click="getGift()">{{packets.status[showBoxIndex]?'Đã nhận':'Nhận'}}</div>
        </div>
      </transition>
    </div>
    ...
    <div class="mask" v-show="showTipsPup">
      <transition name="slide">
        <div class="boxGifts tipsPup" v-if="showTipsPup">
          ...
          <h3>Cách nhận điểm Rực Rỡ</h3>
          <p class="yel">Điểm Rực Rỡ=lượt thích bài hát x10 + mị lực quà xu của bài hát</p>
          <p><span class="yel">Số lượt thích:</span> chỉ ghi nhận 30 lượt thích đầu tiên</p>
          <p><span class="yel">Quà đặc biệt:</span>bài hát dự thi nếu nhận được 4 quà đặc biệt sau: Ủng Hộ Bạn (10 xu), Hoa Hồng (118 xu), Kỳ Lân 7 Màu (399 xu), quà từ hộp quà cao Cạn Ly Bia (110 xu), sẽ tăng 5% mị lực nhận được.</p>
          <p><span class="yel">Thời gian tăng điểm: </span>trong thời gian sự kiện 21:00 – 21:10 mỗi ngày nếu bài hát nhận quà xu sẽ tăng thêm 10% điểm mị lực.</p>
          <span class="ok" @click="showTipsPup = !showTipsPup">Biết rồi</span>
        </div>
      </transition>
    </div>
  </div>
  ...
  methods: {
        ...
        if (this.isOpen) {
          api.openPacket(this.showBoxIndex).then(res => {
            if (res.data.response_status.code == 0) {
              ...
              this.vxc('setToast', {
                title: 'Nhận xong',
                msg: 'Phần thưởng đã được gửi vào tài khoản, mời bạn kiểm tra!'
              })
            } else {
          ...
          this.vxc('setToast', {
            ...
            msg: `Đạt cấp Lv.${this.showBoxIndex} (${this.packets.levels[this.showBoxIndex]} điểm Rực Rỡ mới được nhận)`
          })
        }
      })
    },
    getGiftName(item) {
      if (item.type == 'gift') {
        ...
        return `vip ${item.day} ngày`
      } else if (item.type == 'bean') {
        return `${item.count}đậu`
