<template>
  <div class="redPactets">
    ...
    <div class="lastOpenPacketPeople" v-if="naming.uid">
      <div class="title">Lì xì vừa rồi do những bạn sau tài trợ</div>
      <div class="people">
    ...
    <div class="mask" v-show="redPacket_downTime">
      <transition name="slide">
        <div class="downTimePacket" v-if="redPacket_downTime" :class="{open:packetType == 2}">
          ...
          <h5 v-if="packetType == 1">Đếm ngược lì xì</h5>
          <div class="timeDown" v-if="packetType == 1">Lì xì của bạn xuất hiện sau <em>{{downTime}}</em> giây!</div>
          ...
          <vue-seamless-scroll :class-option="optionHover" :data="redPacket.record" class="seamless-warp">
            <ul class="item">
              <li v-for="(item,index) in redPacket.record" :key="index">
                ...
                <strong> <em class="nick">{{item.nick}}</em> nhận {{item.count}} “Ủng Hộ Bạn”</strong>
              </li>
            </ul>
          </vue-seamless-scroll>
    ...
    <div class="mask" v-show="packetRes">
      <transition name="slide">
        <div class="packetRes" v-if="packetRes">
          ...
          <div class="resHeader">
            <div class="title">{{coins?'Chúc mừng nhận':'Chậm quá, lì xì bị giật hết rồi'}}</div>
          ...
          <div class="resBottom">
            <div class="bar"></div>
            <div class="resBtn" @click="redPacketHistory()">Chi tiết nhận</div>
          </div>
        </div>
      </transition>
    </div>
        ...
        <div class="packetRecord" v-show="packetRecore">
          ...
          <div class="resHeader">
            ...
            <div class="tips" v-else>
              ...
              <span>tổng <em>{{recoreData.gotCoins}}/{{recoreData.coins}}</em> xu</span>
            </div>
          </div>
          <ul class="recoreList">
  ...
  methods: {
    subscribe() {
      if (!this.reg) {
        this.vxc('setToast', {
          title: 'Hẹn thất bại',
          msg: 'Chưa báo danh sự kiện, không thể giật lì xì, mau báo danh nào!'
        })
        return
      }
    ...
    shareAct() {
      ...
      var data = {
        "share_title": `Mau lên! Sắp nổ lì xì rồi!`,
        "share_content": "Nhận hẹn giật lì xì, nhận ngay thông báo >>",
        "share_image": `http://activity.singnowapp.com/static_html/2020/songking2020/share.png?v=2`,
        "link": `http://activity.singnowapp.com/static_html/2020/songking2020/index.html?uid=#ID#&token=#TOKEN#`,
        "image": `http://activity.singnowapp.com/static_html/2020/songking2020/share.png?v=2`,
        "share_url": `http://activity.singnowapp.com/static_html/2020/songking2020/index.html?uid=#ID#&token=#TOKEN#`
      }
      if (ios) {
        if (window.share != undefined) {
          share(JSON.stringify(data))
        } else {
          location.href = `shareUserInfo://activity.17sing.tw/static_html/2020/songking2020/index.html?uid=#ID#&token=#TOKEN#&shareText=Nhận hẹn giật lì xì, nhận ngay thông báo >>&userImg=http://activity.singnowapp.com/static_html/2020/songking2020/share.png?v=2&title=Mau lên! Sắp nổ lì xì rồi!`;
        }
      } else {
