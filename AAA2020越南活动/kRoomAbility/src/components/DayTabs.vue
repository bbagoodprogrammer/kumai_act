  ...
  computed: {
    ...
    days() { //日榜tab计算 11 44 24 34 26 28 
      if (this.day < 4 && this.totalDay > 4) {  //总共的天数大于4，返回前四
        ...
        return [this.day - 3, this.day - 2, this.day - 1, this.day, this.day + 1]
      } else if (this.day == this.totalDay) { // 等于总天数放最后一位
        return [this.day - 4, this.day - 3, this.day - 2, this.day - 1, this.day]
      }
    }
  },
  updated() {
  ...
  .days {
    ...
    a {
      ...
      em {
        ...
        height: 100%;
        text-align: center;
        background: url(../assets/img/dayBg2.png);
        background-size: 100% 100%;
        line-height: 0.63rem;
        color: #ffd48f;
        font-size: 80%;
