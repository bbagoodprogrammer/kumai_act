<template>
  <div class="prizeBox">
    ...
    <div class="gift" v-if="nowPrize.type == 1">
      ...
      <strong>Chúc mừng bạn nhận túi quà {{nowPrize.name}} (7 ngày) đã gửi vào Túi, mời kiểm tra~</strong>
    </div>
    <div class="noThing" v-else-if="nowPrize.type == 4">
      <div class="imgBox">
        <img src="../assets/img/noThing.png" alt="">
      </div>
      <strong>Còn thiếu chút vận may, nhặt được mảnh lọ</strong>
    </div>
    <div class="song" v-else-if="nowPrize.type == 2 || nowPrize.type == 3">
      <div class="userMsg">
        <div class="avBox">
          <img v-lazy="nowPrize.avatar" alt="">
          <i @click="follow(nowPrize.attension,index,nowPrize.suid)" :class="{black:nowPrize.attension}">{{nowPrize.attension?'Đã quan tân':'+Quan tâm'}}</i>
        </div>
      ...
      <p v-if="!showLuckTips" class="luckTips">Chúc mừng bạn nhận {{nowPrize.type==2?'lọ thường':' lọ cao cấp'}}，<br />nghe hết {{luckTime}} giây có bất ngờ~</p>
      <p v-else class="luckTips">Chúc mừng bạn nhận {{luckGift}}，phần thưởng đã gửi vào Ví, mời kiểm tra~</p>
    </div>
    <span class="next" @click="next()"> {{isOver?"Xong":"Tiếp theo"}}</span>
  </div>
</template>
<script>
