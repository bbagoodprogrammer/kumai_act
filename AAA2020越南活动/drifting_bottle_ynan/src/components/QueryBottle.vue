  ...
  <div class="querySong">
    ...
    <div class="query">
      ...
      <div class="firstCom" v-if="showType==1">
        ...
        <P v-if="querySong.free"> Xin lỗi, bài hát 《{{querySong.name}}》 đã thả lọ miễn phí 1 lần vào ngày {{getDate(querySong.free_time)}}, mỗi bài chỉ được miễn phí thả 1 lần, mời chọn bài khác~</P>
    ...
    <div class="mask" v-show="showQuery">
      <transition name="slide">
        <div class="queryPup" v-show="showQuery">
          ...
          <p>{{queryType==1?`Bạn sẽ dùng ${price} xu đề cử bài hát này`:`Xin lỗi, bạn không đủ xu, mời nạp thêm`}} </p>
  ...
  computed: {
    ...
    type() {
      if (this.first && this.showType == 1) {
        return 1
      } else if (this.pNum == 100 && this.showType == 2) {
        return 3
      } else if (this.pNum == 300 && this.showType == 2) {
        return 4
      } else if (this.pNum == 15 && this.showType == 2) {
        return 2
      }
    }
  },
