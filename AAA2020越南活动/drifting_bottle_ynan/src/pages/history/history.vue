<template>
  <div class="history">
    <div class="tabs">
      <span :class="{act:showType==1}" @click="tabClick(1)">Lịch sử thả</span>
      <span :class="{act:showType==2}" @click="tabClick(2)">Lịch sử nhặt</span>
    </div>
    <ul v-if="showType==1">
      <li v-for="(item,index) in list.list" :key="index" @click="goSid(item.sid)">
        ...
        <div class="userMsg">
          <div class="user">
            ...
            <em v-if="item.ban">(Đã ngưng thả)</em>
          </div>
          <div class="dStatus">
            <strong>Trạng thái:<em>{{item.step==1?'đang thả':'Kết thúc thả'}}</em></strong>
            <strong>Thời gian thả:<em>{{item.time}}</em></strong>
          </div>
          <div class="dScore">
            <strong>Nghe:<em>{{item.hear}}</em></strong>
            <strong>Quan tâm:<em>{{item.attension}}</em></strong>
            <strong v-if="item.like">Điểm phổ biến:<em>{{item.like}}</em></strong>
          </div>
        </div>
      </li>
    </ul>
    <ul v-else>
      <li v-for="(item,index) in list.list" :key="index" @click="goSid(item.sid)">
        ...
        <div class="time">{{item.time}} nhặt</div>
      </li>
    </ul>
    <p class="hLoading" v-if="list.loading">Đang tải...</p>
    <p class="hLoading" v-if="list.list.length == 0 && !list.loading">Chưa có dữ liệu</p>
    <Loading />
  </div>
</template>
<script>
