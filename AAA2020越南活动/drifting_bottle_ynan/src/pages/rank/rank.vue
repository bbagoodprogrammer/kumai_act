<template>
  <div class="rank">
    ...
    <p class="tips">Xếp hạng theo tổng lượt nghe, thời lượng nghe, số quan tâm mới của bài hát <em>(BXH được công bố sau khi kết thúc sự kiện)</em> </p>
    <p class="noData" v-if="list.length == 0">Chưa có dữ liệu</p>
    <ul>
      <li v-for="(item,index) in list " :key="index" :class="'rank' + item.rank" @click="goSid(item.sid)">
        ...
        <div class="score">Điểm phổ biến:<strong>{{item.score}}</strong> </div>
      </li>
    </ul>
    <loading />
  </div>
