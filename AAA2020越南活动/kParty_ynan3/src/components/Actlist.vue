    ...
    <ul>
      <li @click="gokRoom(item)" v-for="(item,index) in list" :key="index" :class="'list'+item.cover">
        ...
        <div class="userTop">
          ...
          <span :class="{ml:item.user.length <=2}" v-if="item.status == 1">{{item.nums}}{{lang.peopleTips}} <i></i></span>
          <span :class="{ml:item.user.length <=2}" v-else>{{item.nums}} theo dõi</span>
        </div>
        <div class="actBtm">
          <span @click.stop="showSingUpPup(item.id)" v-if="item.is_reg == 2"> {{lang.singUp}}</span>
          <!-- <span>分享</span> -->
  ...
  methods: {
    ...
    getDate (time) {
      ...
      if (partyTime == 0) {
        ...
        return `${getDate(new Date(time * 1000), 1)} Ngày mai `
      } else {
    ...
    getImgArr (item) {
      ...
      if (item.cover != 5) {
        ...
        console.log(Object.values(item.user))

        return Object.values(item.user).slice(0, 3)
      }
    }
  }
}
...
.actList {
  ...
  .coverImg {
    ...
    height: 2.09rem;
    padding: 0.02rem;
    background: url(../assets/img/coverBg.png);
    background-size: 100% 100%;
    position: absolute;
    top: 1.06rem;
    right: 0.26rem;
