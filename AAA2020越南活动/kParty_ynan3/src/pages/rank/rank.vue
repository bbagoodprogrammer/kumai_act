<template>
  <div class="rankGroups">
    ...
    <!-- 日榜、总榜切换主Tabs -->
    <div class="mainTabs">
      <a @click.prevent="changPre(0)" :class="{current:month==0}" href="">BXH Tuần</a>
      <a @click.prevent="changPre(1)" :class="{current:month ==1}" href="">BXH Tháng</a>
    </div>
    <div class="lastTabs">
      <span v-if="month== 0 && mainTab==2" @click="mainTabClick(0)">BXH tuần này>></span>
      <span v-if="month== 0 && mainTab==0" @click="mainTabClick(2)">BXH tuần trước>></span>
      <span v-if="month== 1 && mainTab==3" @click="mainTabClick(1)">BXH tháng này>></span>
      <span v-if="month== 1 && mainTab==1" @click="mainTabClick(3)">BXH tháng trước>></span>
    </div>
    ...
    <ul class="list day" :class="{nodata: rank.list.length == 0}">
      <li v-for="(item,index) in rank.list" :key="index" @click="goUser(item.rid)">
        ...
        <div class="score" v-if="mainTab==0 || mainTab==2">Điểm tuần này {{item.score}}</div>
        <div class="score" v-else>Điểm tháng này {{item.score}}</div>
      </li>
    </ul>
    <!-- 总榜 -->
    ...
    <div v-if="rank.none" class="scrollNone">Chưa có số liệu!</div>
    <div class="footer" v-if="msg.rank != 0 && msg.rid">
      ...
      <div class="score" v-if="mainTab==0 || mainTab==2">Điểm tuần này {{msg.score}}</div>
      <div class="score" v-else>Điểm tháng này {{msg.score}}</div>
    </div>
  </div>
</template>
