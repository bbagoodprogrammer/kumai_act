<template>
  <div class="myParty">
    <p v-if="actList.length == 0" class="noData"> Chưa có lịch sử, nhanh đi đăng ký tiệc nào!</p>
    <ul>
      <li v-for="(item,index) in actList" :key="index" :class="[{max:item.apply_status != 0,black:item.status == 2},'list'+item.cover]">
        ...
        <div class="title">{{item.them}} <span v-if="item.status == 2">- Đã kết thúc</span> </div>
        <div class="actCon">
        ...
        <div class="actScore" v-if="item.status == 2" @click="showScore(item.id,item.final_score)">{{item.final_score}} điểm</div>
        <div class="setBtn" v-else-if="item.apply_status != 2 && item.fail_nums !=2" @click="changeMsg(item.id)">{{lang.actBarChang}}</div>
        <span class="singMsg" @click="showPeople(item.id)" v-else-if="item.apply_status == 2 && item.is_reg != 1">{{lang.singUpDetil}}</span>
      </li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    getDate(time) {
      ...
      if (partyTime == 0) {
        return `hôm nay ${getDate(new Date(time * 1000), 1)}`
      } else {
    ...
    showScore(id, score) {
      ...
      api.actScore(id).then(res => {
        ...
        if (record) {
          ...
          this.toast('Tiệc này không có điểm!')
        }
      })
    },
    closeActScorePup() {
