<template>
  <div class="rankGroups">
    ...
    <div class="mainTabs" :class="{maxH:mainTab!=2 && mainTab!=1}">
      <div class="tabItem">
        <span class="taskTab" @click.prevent="mainTabClick(2)" :class="{current:mainTab==2}" href="">Nhiệm vụ</span>
        <div class="tabs">
      ...
      <div class="listBg" v-if="mainTab!=2 && mainTab!=1">
        ...
        <div class="dayScore" :class="{mt:tab<nowDay}">
          ...
          <div class="scoreMsg">
            <div class="total">Pháp lực gọi Thần Đèn đạt {{rank.amount}}</div>
            <div class="conis" v-if="tab>=nowDay">Top 100 BXH ngày Hôm nay có thể chia <em>{{rank.coins}}</em>xu </div>
            <div class="conis" v-else>Hiện tại Top 100 đã chia <em>{{rank.coins}}</em> xu.</div>
          </div>
        </div>
      </div>
    ...
    <ul v-if="mainTab==0" class="list day">
      <li v-for="(item,index) in rank.list" :key="index" :class="'rank'+item.rank" @click="goUser(item.uid)">
        ...
        <div class="score" :class="{ml:item.coins==0}">Pháp lực: <em>{{item.score}}</em></div>
        <!-- -->
        <div class="coins" v-if="item.coins>0">{{item.coins}} xu</div>
      </li>
    </ul>
    <!-- 总榜 -->
    <ul v-else-if="mainTab==1" class="list total">
      <li v-for="(item,index) in rank.list" :key="index" :class="'rank'+item.rank" @click="goUser(item.uid)">
        ...
        <div class="score">Pháp lực: <em>{{item.score}}</em></div>
        <!-- v-if="item.coins>0" -->
