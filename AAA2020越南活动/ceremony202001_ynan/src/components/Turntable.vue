      ...
      <transition name="slide">
        <div class="getGiftCon" :class="{five:getGift_list.length == 5}" v-if="showGetGift">
          ...
          <div class="giftItemBox">
            ...
            <p class="luckGiftTips" v-if="getGift_list.length == 1">{{lang.luckTips1}} <em>{{getWardName(getGift_list[0])}}</em> </p>
    ...
    <div class="mask" v-show="showHistory">
      <transition name="slide">
        <div class="historyPup" v-show="showHistory">
          ...
          <div class="title"></div>
          <div class="tabHeader">
            <span>{{lang.luckTips2}}</span>
            <span>{{lang.luckTips3}}</span>
          </div>
          <p class="noData" v-if="!historyList.length">{{lang.luckTips4}}</p>
          <ul class="scrollable">
  ...
  methods: {
    luckGO(val) {
      globalBus.$emit('commonEvent', callback => {
        ...
        if (!this.registered) {
          this.vxc('setToast', {
            title: this.lang.luckTips5,
            msg: this.lang.luckTips6
          })
        } else {
    ...
    getGiftTips(gitfList) {
      if (gitfList.length == 1) {
        ...
        arr.forEach(element => {
          if (element == 4) {          //   欢卡
            strArr.push(this.lang.luckTips7)
