<template>
  <div class="playing2">
    <div class="boxList">
      <div class="userScore">
        <div class="lv">{{lang.mvpTips1}}{{level}}</div>
        <div class="score">{{lang.mvpTips2}}{{socre}}</div>
      </div>
      ...
      <div class="rank1" :style="{height:listHeight+'rem'}">
        ...
        <ul class="list">
          <li v-for="(item,index) in list" :key="index" :class="'top' + item.rank">
            ...
            <div class="score">{{lang.mvpTips2}} <em>{{item.score}}</em></div>
          </li>
        </ul>
      </div>
    </div>
    ...
    <div class="mask" v-show="showBoxGift">
      <transition name="slide">
        <div class="boxGifts" v-if="showBoxGift">
          ...
          <h3 v-html="lang.mvpTips4.replace('X',showBoxItem.limit)"></h3>
          <strong class="needLv">{{lang.mvpTips5.replace('X',showBoxItem.level)}}</strong>
          <div class="giftBox">
      ...
      if (!item.receive) {
        api.getGift(item.level).then(res => {
          if (res.data.response_status.code == 0) {
            ...
            this.vxc('setToast', {
              title: this.lang.mvpTips6,
              msg: this.lang.mvpTips7
            })
          } else {
