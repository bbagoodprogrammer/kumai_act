<template>
  <div class="rankGroups">
    <a @click.prevent="onRefresh" :style="{transform:'rotate('+rotatePx+'deg)'}" id="refresh" v-if="actStatus == 1"></a>
    <!-- 三個類別 -->
    <div class="typeMainTabs">
      <i class="liner"></i>
      <!-- <em>{{getDate(stime1,1)}}-{{getDate(etime1,1)}}</em> -->
      <span class="tab1" @click="setShowType(1)" :class="{act:showType == 1}"><strong>Bảo Hộ</strong> </span>
      <span class="tab2" @click="setShowType(2)" :class="{act:showType == 2}"><strong>PK Gia Tộc</strong> </span>
      <span class="tab3" @click="setShowType(3)" :class="{act:showType == 3}"><strong>Tranh Bá</strong></span>
    </div>
    <!-- 倒計時 -->
    <div class="downTimebox">
      <div class="timeTips">
        <i class="left"></i>
        <p v-if="showType == nowTab ">Đếm ngược hết vòng thi</p>
        <p v-else-if="showType > nowTab">Đếm ngược bắt đầu vòng thi</p>
        <p v-else>Vòng thi đã kết thúc</p>
        <i class="right"></i>
      </div>
      <!-- surplusTime.day &&-->
    ...
    <div class="mainTabs" v-if="showType == 1">
      <div class="tabs">
        <a @click.prevent="mainTabClick(0)" :class="{current:mainTab==0}" href="">Nhiệm vụ gia tộc</a>
        <a @click.prevent="mainTabClick(1)" :class="{current:mainTab==1}" href="">Hạng gia tộc</a>
      </div>
    </div>
    ...
    <div class="tasks" v-if="showType ==1 && mainTab == 0">
      <div class="taskHeader">
        <span class="name">Nhiệm vụ</span>
        <span class="score">Điểm bảo hộ</span>
      ...
      <ul class="taskList">
        <li v-for="(item,index) in task" :key="index">
          ...
          <span class="bar">{{item.schule >= item.limit?'Đã đạt':`${item.schule}/${item.limit}`}}</span>
        </li>
      </ul>
    </div>
    ...
    <div class="tipsBox" v-if="!(showType == 1 && mainTab== 0)">
      ...
      <div v-if="mainTab==0 && showType > nowTab" class="dengdai">Hãy vui lòng chờ đón!</div>
    </div>
    <!--  -->
...
export default {
  ...
  data() {
    return {
      ...
      cardsMsg: [],
      taskName: {
        login: 'Vào trang sự kiện điểm danh gia tộc 1 lần',
        mic: 'Lên mic tại phòng Kara gia tộc 60 giây/ngày',
        work: 'Đăng 1 bài hát công khai/ngày',
        praise: 'Thích 2 bài của thành viên gia tộc/ngày',
        share: 'Chia sẻ 2 bài của thành viên gia tộc/ngày',
        sendGift: 'Tặng 2 quà gia tộc/ngày',
        mark: ''
      },
      loaded: false,
      more: true,
