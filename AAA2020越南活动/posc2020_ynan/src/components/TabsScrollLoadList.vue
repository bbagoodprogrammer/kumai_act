<template>
  <div class="rankGroups">
    ...
    <div class="actTips">
      <p>Tặng quà trên cho thí sinh thì họ sẽ nhận thêm 10% điểm sôi nổi! </p>
      <p v-if="nowTab != 4"><span>Thời khắc vĩ đại:</span>tặng quà trong 21:00:00~21:10:00 hàng ngày sẽ giúp thí sinh nhận thêm 10% sôi nổi. </p>
    </div>
      ...
      <li v-for="(item,index) in rank.list" :key="index" :class="'rank'+item.rank" @click="goGroup(item.tid)">
        <div class="grouplist">
          ...
          <div class="groupScore">
            ...
            <div class="groupT">Top {{item.count}} vào vòng sau</div>
          </div>
        </div>
    ...
    <div class="stage4List" v-else>
      <div class="listTop">
        <div class="kRoom">
          ...
          <div class="roomTips">
            <div class="goBtn" @click="goRoom(nowInitData.room.rid)">Vào phòng kara chung kết</div>
            <div class="micPeople" @click="showMicPeople()">Thứ tự mic >></div>
          </div>
        </div>
    ...
    <div class="listTipsBox">
      ...
      <div v-else-if="nowTab > showType && actStatus == 1" class="dengdai">Xin vui lòng chờ!</div>
      <div v-else-if="rank.none && rank.list.length == 0 " class="scrollNone">
        BXH tạm đang trống</br>
        Đang trống, mau đến nhé!
      </div>
    ...
    <!-- 排麦列表 -->
    <div class="mask" v-show="showMicUser">
      <transition name="slide">
        <div class="micList" v-show="showMicUser">
          <i class="close" @click="showMicUser = false"></i>
          <div class="tips">
            <h3>Thứ tự mic phòng</h3>
            <p>Thứ tự BXH vòng chung kết vào 00:00 24/12 sẽ quyết định thứ tự mic live trong phòng kara chung kết vào 20:00 25/12. Thí sinh xếp hạng cao hơn thì thứ tự mic sẽ được mặc định xếp cuối. Nếu thí sinh không tham gia live tại vòng chong kết, thì thứ tự mic của thí sinh này sẽ được tạm hoãn. Cụ thể sẽ quyết định bởi sự trao đổi và thỏa thuận giữa cộng tác viên và thí sinh.</p>
          </div>
          <div class="mList">
            <div class="micListheader">
              <span>Xếp hạng</span>
              <span>Thứ tự mic</span>
            </div>
            <ul>
              <li v-for="(item,index) in micList" :key="index">
...
export default {
  ...
  data() {
    return {
      ...
      timeTipsArr: {
        1: {
          0: 'Đếm ngược bắt đầu Vòng Loại',
          1: 'Đếm ngược kết thúc Vòng Loại',
          2: 'Vòng Loại đã kết thúc'
        },
        2: {
          0: 'Đếm ngược bắt đầu 1 VS 1',
          1: 'Đếm ngược kết thúc 1 VS 1',
          2: '1 VS 1 đã kết thúc'
        },
        3: {
          0: 'Đếm ngược bắt đầu Đấu Đội',
          1: 'Đếm ngược kết thúc Đấu Đội',
          2: 'Đấu Đội đã kết thúc'
        },
        4: {
          0: 'Đếm ngược bắt đầu Vòng Chung Kết',
          1: 'Đếm ngược kết thúc Vòng Chung Kết',
          2: 'Vòng Chung Kết đã kết thúc'
        }
      },
      first: true,
      showRankTips: false,
  ...
  methods: {
    ...
    showMicPeople() {
      api.micList().then(res => {
        this.micList = res.data.response_data.list
        this.showMicUser = true
      })
    }
  },
}
