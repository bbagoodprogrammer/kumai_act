  ...
  <div class="footerBar">
    <div class="acrStatus">
      ...
      <div class="goAct" :class="{type1:showType == 1}" v-if="astState === 1">
        <p v-if="showType == 1">Sự kiện này chỉ dành cho người dùng đã ký hợp đồng KOL. Mau ký hợp đồng trở thành KOL! </p>
        <p v-else>Sự kiện này chỉ dành cho người dùng đã ký hợp đồng KOL. Mau đi trở thành fan và bình chọn cho thí sinh nhé!</p>
        <div class="signUp" v-if="showType == 1" @click="goSingNing()">Ký ngay</div>
      </div>
      <div class="user_1" :class="{up:nowUserScore.up}" v-if="astState == 3 || astState == 5 || astState == 6">
        <p v-if="nowUserScore.up && astState == 3">Chúc mừng bạn đã vào vòng sau! Mau chuẩn bị vòng tiếp theo nhé!~ </p>
        <p v-else-if="nowUserScore.up && astState == 5">Chúc mừng bạn đại diện đội của mình vào vòng sau! Mau chuẩn bị quyết chiến vòng chung kết nhé!~</p>
        <p v-else-if="nowUserScore.up && astState == 6">Chúc mừng bạn đã vượt qua vô số thử thách, giành được hạng {{nowUserScore.rank}} tại Đại Chiến Gương Mặt Đại Diện!</p>
        <div class="user_1_msg" :class="[{icon:astState == 6},'rank' + nowUserScore.rank]">
          ...
          <div class="userMsg">
            ...
            <div class="group" v-if="astState == 5">Đội:{{groupName[nowUserScore.team]}}</div>
      ...
      <div class="user_2" :class="{up:nowUserScore.up}" v-if="astState == 4" @click="shouCards()">
        <p v-if="nowUserScore.up">Chúc mừng bạn đã vào vòng sau! Mau chuẩn bị vòng tiếp theo nhé!~ </p>
        <div class="pkMsg">
...
export default {
  ...
  methods: {
    goSingNing() {
      ...
      location.href = `http://act.singnowapp.com/signing/index.html?uid=${uid}&token=${token}`
    },
    goUser(uid) {
