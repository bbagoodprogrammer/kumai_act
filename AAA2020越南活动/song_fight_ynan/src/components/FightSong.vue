  ...
  <div class="fSongList">
    <ul :class="{isDown:isDown}" :style="{height:useTotalTop+'rem'}">
      <li v-for="(item,index) in list" :style="{zIndex:-index+100,top:item.top +'rem'}" :key="item.name">
        <div class="songName"><em>Bài hát võ đài:</em> <strong>{{item.name}}</strong> </div>
        <div class="pkUser">
          <div class="user">
        ...
        <div class="pkTips" v-if="item.status == 0 && set">Đang ghép cặp đấu...</div>
        <div class="pkTips" v-else-if="item.status == 1 && item.result == 0">Đang thi đấu...</div>
        <div class="peopleNum" v-else-if="item.result != 0 && item.judges.length">
          ...
          <u @click="goVotePeople(item.id,item.judge_nums)">{{item.judge_nums}} người ủng hộ >></u>
        </div>
      </li>
    </ul>
    <div class="listBtn" v-if="list.length>1"></div>
    <span @click="setDownStatus()" class="setBtn" v-if="list.length>1">{{isDown?'Thu lại':'Mở rộng'}} <i :class="{upper:isDown}"></i></span>
  </div>
</template>
<script>
