  ...
  <div class="myVote">
    ...
    <div class="preResTime" v-for="(item,index) in list " :key="index">
      <div class="timeTips">
        ...
        <p v-else><span class="time">{{getDate(item.stime,1)}}-{{getDate(item.etime,1)}}</span> Chọn {{item.counts}} nhóm， tỷ lệ trúng {{item.right}}%<span v-if="item.title!=0">，nhận danh hiệu {{err[item.title]}}</span> <br /> <strong v-if="item.eight >=2">{{item.eight}} lần chọn trúng liên tiếp 8 nhóm</strong> </p>
      </div>
