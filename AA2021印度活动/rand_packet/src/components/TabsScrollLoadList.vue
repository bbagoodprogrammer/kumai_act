<template>
  <div class="rankGroups">
      ...
      <div class="top3">
        <div v-for="(item,index) in top3" :key="index" :class="'rank'+item.rank" class="topItem">
          <div class="imgBox" @click="goUser(item.uid)">
            <img :src="require(`../img/top${item.rank}.png`)" alt="" class="championTips">
            <img v-lazy="item.avatar" alt="" class="userImg">
          </div>
          <div class="userMsg">
            <div class="name">{{item.nick}}</div>
            <div class="score"><i></i><em>{{item.score}}</em></div>
            <div class="ticket" v-if="mainTab == 0">{{item.rebate}}{{lang.ticket}}</div>
          </div>
        </div>
      </div>
      <ul class="list day">
        <li v-for="(item,index) in surpluslist" :key="index" :class="'rank'+item.rank" @click="goUser(item.uid)">
          ...
          <div class="uerImg">
            ...
            <img v-if="item.noble != 0" :src="require(`../img/noble/${item.noble}.png`)" alt="" class="noble">
    ...
    <div v-if="rank.loading" class="scrollLoading">{{lang.rank_loading}}</div>
    <div v-if="rank.list.length == 0 && tab <= nowDay" class="scrollNone">
      {{lang.rank_nodata}}
    </div>
    <div v-if="mainTab==0 && tab > nowDay" class="dengdai"> {{lang.rank_noStar}}</div>
  </div>
</template>
...
.rankGroups {
  ...
  .downTimebox {
    ...
    .timeDown {
      ...
      width: 5.14rem;
      height: 0.76rem;
      background: url(../img/timeDown.png);
      background-size: 100% 100%;
      display: flex;
  ...
  .list {
    ...
    li {
      ...
      background: url(../img/topItemBg.png);
      background-size: 100% 100%;
      display: flex;
      align-items: center;
      .rank {
      ...
      .userMsg {
        ...
        .probability {
          color: rgba(255, 178, 198, 1);
          font-size: 0.22rem;
          em {
            font-size: 0.22rem;
            margin-left: 0.14rem;

            background: linear-gradient(0deg, #fff587 0%, #ffb659 100%);
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
          }
        }
