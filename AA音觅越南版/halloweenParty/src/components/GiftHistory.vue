<template>
  <div class="giftHistory">
    ...
    <div class="noData" v-if="list.length == 0 && !loading">Không có dữ liệu</div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    getTips(data) {
      let type = data.type
      if (type == 1) {
        return `Hoàn thành ${this.day_task_name[data.task_id]}, nhận kẹo ma lực *${data.nums}`
      } else if (type == 2) {
        return `Hoàn thành ${this.new_task_name[data.task_id]},nhận kẹo ma lực *${data.nums}`
      } else if (type == 3) {
        return `Đổi 3ngày`
      } else if (type == 4) {
        return `Tặng kẹo ma lực *${data.nums}`
      } else if (type == 5) {
        return `Nhận được ${data.nick} tặng kẹo ma lực *${data.nums}`
      }
    },
    getTime(tm, type) {
