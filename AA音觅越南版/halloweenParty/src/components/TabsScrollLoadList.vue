<template>
  <div class="rankGroups">
    ...
    <div class="listBog">
      <div class="mainTabs">
        <a @click.prevent="mainTabClick(0)" :class="{current:mainTab==0}" class="tabL">BXH Ngày</a>
        <a @click.prevent="mainTabClick(1)" :class="{current:mainTab==1}" class="tabR">BXH tổng</a>
      ...
      <!-- 搜索框 -->
      <div class="shareBtn">
        <input type="number" v-model="searchUid" placeholder="nhập UID tìm quỷ thần" @keyup.13="search()" id="searchInput">
        <i @click="search()"></i>
      </div>
      <!-- 日榜 -->
      <div class="list day" v-if="!showitemList">
        <li v-for="(item,index) in rank.list" :key="index" :class="'list'+item.rank">
          ...
          <div class="userScore">
            <div class="total">tổng điểm {{item.sugar_score*1 + item.gift_score*1}}</div>
            <div class="score1">Điểm Kẹo ma lực {{item.sugar_score}}</div>
            <div class="score1">Điểm chơi phòng {{item.gift_score}}</div>
          </div>
          <div class="peopleList" v-if="item.rank <=3">
            <div class="userList">
              <span>Top 3 bảng cống hiến</span>
              <div class="items">
      ...
      <div class="searchList" :class="'list'+itemMsg.rank" v-else>
        ...
        <div class="userScore">
          <div class="total">tổng điểm {{itemMsg.sugar_score*1 + itemMsg.gift_score*1}}</div>
          <div class="score1">Điểm Kẹo ma lực {{itemMsg.sugar_score}}</div>
          <div class="score1">Điểm chơi phòng {{itemMsg.gift_score}}</div>
        </div>
      ...
      <div v-if="rank.none" class="scrollNone">
        Tạm thời chưa có bạn bè lên BXH
      </div>
    </div>
    <!-- 守護榜 -->
    <div class="mask" v-show="showPlist">
      <transition name="slide">
        <div class="peopleListPup" v-show="showPlist">
          ...
          <h5 class="pupTitle">BXH cống hiến của <strong>{{pupMsg.info.nick}}</strong></h5>
          <div class="peopleItem">
            <div class="listHeader">
              <span class="rank"></span>
              <span class="user"></span>
              <span class="score">Cống hiến</span>
            </div>
            <p class="noData" v-if="pupMsg.list.length == 0">Không có dữ liệu</p>
            <ul class="scrollable">
            ...
            <p class="listRankTips">
              *Tổng điểm là fan cống hiến cho một ngườif, BXH chỉ hiển thị Top 20
            </p>
            <div class="goBtn" @click="goUser(pupMsg.info.uid)">Thông tin</div>
          </div>
        </div>
      </transition>
    </div>
    ...
    <div class="mask" v-show="scoreTips">
      <transition name="slide">
        <div class="scoreTips" v-show="scoreTips">
          <div class="title">Điểm quỷ thần</div>
          <h6>Điểm quỷ thần = điểm kẹo ma lực + điểm chơi phòng</h6>
          <p>điểm kẹo mà lực= số kẹo nhận đươc *100<br />
            Điểm chơi phòng = Tổng xu nhận được từ quà chỉ định *12
          </p>
          <div class="ok" @click="scorePupClick()">Bết rồi</div>
        </div>
      </transition>
    </div>
  </div>
