<template>
  <div class="box">
    ...
    <div class="header">
      <span class="ruleTips" @click="showRulePup()"></span>
      ...
      <transition name="slide">
        <div class="rules" v-if="showRule">
          ...
          <div class="msg">
            ...
            <h6>Giải thưởng:</h6>
            <p>Giải nhất: Chứng nhận Vua báu vật (15 ngày) </br>
              Giải nhì: Chứng nhận Nhà sưu tầm báu vật (15 ngày)</br>
              Giải ba: Chứng nhận Nhà sưu tầm báu vật (7 ngày)
            </p>
            <h6>Chú ý:</h6>
            <p class="lastTips">1, Tìm được báu vật có giá trị xu sẽ được tính theo giá trị xu trên banner</p>
            <p class="lastTips">2, Tìm được báu vật, miếng ghép đổi báu vật sẽ được gửi luôn vào túi quà, mời vào túi quà kiểm tra, khung ảnh Thanh lịch mời bạn vào siêu thị kiểm tra.</p>
            <p class="lastTips">3, Chìa khóa và miếng ghép chỉ được sử dụng trong thời gian diễn ra sự kiện, hết hạn sẽ không được tính.</p>
            <p class="lastTips">4, Trong thời gian diễn ra sự kiện, nhận được hoặc tặng đi quà Tiệc đêm đều được nhận Khung ảnh Thanh lịch (3 ngày).</p>
    ...
    <div class="mask" v-show="showRank">
      <transition name="slide">
        <div class="rank" v-show="showRank">
          ...
          <p class="noData" v-if="rank.length == 0">Không có dữ liệu</p>
          <ul class="scrollable">
    ...
    <div class="mask" v-show="showHistory">
      <transition name="slide">
        <div class="history" v-show="showHistory">
          ...
          <div class="hList">
            <p class="noData" v-if="HshowRank.length == 0">Không có dữ liệu</p>
            <ul class="scrollable2">
              <li v-for="(item,index) in HshowRank" :key="index">
                <div class="giftNick">
                  <strong v-if="item.name && item.type!='frame'" :class="'lv' + item.level">{{item.name}} {{item.day?`(${item.day}ngày)`:`*${item.count}`}}</strong>
  ...
  methods: {
    ...
    getName(item) {
      if (item.type == 'coin') {
        ...
        return `${item.name}頭像框(${item.days}ngày) x1`
