<template>
  <div class="giftBox" :class="{type2:type==2}">
    ...
    <div class="keys">
      <i></i>
      <strong>Còn: <em>{{keyNum}}</em></strong>
    ...
    <div class="mask" v-show="buyKeysPup">
      <transition name="slide">
        <div class="buyKeysPup" v-if="buyKeysPup">
          ...
          <div class="title"></div>
          <p class="tips"> Mua khung ảnh xanh kinh điển ({{buyKeyNum*1}} ngày), nhận chìa khóa *{{buyKeyNum*2}}</p>
          <div class="keyGift">
            <span class="frame">
              <strong>
                <em>({{buyKeyNum}}ngày)</em>
                <i>Khung ảnh mèo lười</i>
              </strong>
            </span>
            <span class="key">
              <em>x{{buyKeyNum*2}}</em>
          ...
          <div class="buyBtn" @click="buyKeys()">Xu {{buyKeyNum*200}} mua</div>
        </div>
      </transition>
    </div>
        ...
        <div class="prizes" v-if="prizesPup">
          ...
          <div class="prizesBox" :class="{one:prizes.length ==1}">
            <span v-for="(item,index) in prizes" :key="index">
              ...
              <strong v-if="item.name && item.type!='frame'" :class="'lv' + item.level"><i v-if="item.level!=0">{{item.level==1?'Qùa hiếm':'Cấp cao '}}</i> {{item.name}} {{item.day?`(${item.day}ngày)`:`*${item.count}`}}</strong>
              <strong v-else :class="'lv' + item.level"><i v-if="item.level!=0">{{item.level==1?'Qùa hiếm':'Cấp cao '}}</i> {{getName(item)}}</strong>
            </span>
          </div>
        </div>
      </transition>
  ...
  methods: {
    ...
    luck(num) {
      ...
      if (this.keyNum >= needKeyNum) {
        ...
        this.vxc('setToast', {
          type: 4,
          title: 'Không đủ chìa khóa',
          msg: 'ấn nhận chìa khóa',
          cb: this.showBuyKeys
        })
        this.luckIng = false
      }
      ...
      api.buyKeys(this.buyKeyNum).then(res => {
        if (res.data.response_status.code == 0) {
          this.vxc('setToast', {
            ...
            msg: ` Mời bạn vào Shop kiểm tra khung ảnh, nhận thành công chìa khóa *${this.buyKeyNum * 2}`,
          })
