...
export default {
  ...
  methods: {
    ...
    useProp (id) {
      if (this.propMissions.tools[0].tool_count > 0) {
        if (this.isHas.length) {
          ...
          this.vxc('setToast', {
            msg: `Không có thức ăn phù hợp`,
            title: require('../assets/img/prop/title2.png')
          })
        }
      } else {
        this.vxc('setToast', {
          msg: `<span>Đã dùng hết đạo cụ,</span> cần hoàn thành nhiệm vụ để đổi`,
        })
      }
    },
    useQuickProp (id) {
    ...
    downAllTime () {
      ...
      this.vxc('setToast', {
        msg: `Đã dùng! Rút ngắn thời gian trồng trọt 3 phút`,
