      ...
      <div class="taskMsgList">
        ...
        <ul class="taskItem">
          <li v-for="(item,index) in dayTask.tasks" :key="index">
            ...
            <div class="status">
              <em class='ed' v-if="item.got">đã hoàn thành</em>
              <em class="get" v-else-if="(item.current == item.target) && !item.got" @click="getDice(index,item)">Nhận xúc xắc</em>
              <em class="doTask" v-else @click="doTask(index,item)">làm nhiệm vụ</em>
        ...
        <ul class="priceList">
          <li v-for="(item,index) in dayTask.goods" :key="index">
            ...
            <div class="buy" @click="buyDice(index)">Mua</div>
          </li>
        </ul>
      </div>
    ...
    <div class="mask" v-show="showShare">
      <transition name="slide">
        <div class="sharePup" v-if="showShare">
          ...
          <div class="title"><i></i></div>
          <p class="giftTips"> Mời bạn mới tham gia, mời thành công mỗi người <i></i>*3</p>
          <p v-if="invitedList.length == 0" class="noData">Không có dữ liệu</p>
          <ul class="peopleList">
            <li v-for="(item,index) in invitedList" :key="index">
              <div class="userRank" :class="{noRank:item.rank == 0}">{{item.rank == 0?'Chưa lên BXH':item.rank}}</div>
              <div class="imgBox">
                <span class="avBg" v-if="item.rank<=3"></span>
                <img v-lazy="item.avatar" alt="">
              </div>
              <div class="nick">{{item.nick}}</div>
              <div class="score"><em>{{item.score}}bước</em></div>
            </li>
          </ul>
          <div class="inivitBtn" @click="showPelple()">Mời bạn bè mới</div>
        </div>
      </transition>
    </div>
          ...
          <ul class="pList">
            <li v-for="(item,index) in peopleListHas" :key="index">
              ...
              <div v-if="item.status!=2" class="lsitItem">
                ...
                <div class="nick">
                  <div>{{item.nick}}</div>
                  <div class="tips">Chưa tham gia sự kiện</div>
                </div>
                <div class="shareBtn" :class="{share2:item.status == 1}">
                  <em v-if="item.status == 1">Đã mời</em>
                  <em v-else @click="shareAct(item,index)">Mời</em>
                </div>
              </div>
            </li>
          </ul>
...
export default {
  data() {
    return {
      taskName: {
        coin: 'Tặng 80 xu',
        invite: 'Mời bạn chưa tham gia sự kiện（A/5）',
        mic: 'Lên ghế tại phòng công khai 30 phút(A/1)',
        owner: 'Điểm thu hút tăng 500（A/1）',
        room: 'Điểm sôi nổi tăng 5000'
      },
      showShare: false,
      showPeople: false,
      invitedList: [],
  ...
  methods: {
    showRoomTips() {
      this.vxc('setToast', {
        title: 'room',
        msg: 'Mỗi người online 1 phút nhận 10 điểm sôi nổi <br/> tặng 1 xu nhận 1 điểm sôi nổi'
      })
    },
    showSharePup() {
    ...
    doTask(key, item, rid) {
      ...
      if (key == 'mic' || key == 'coin' || key == 'gift' || key == 'owner') {  //跳轉活躍度前十房間
        ...
        try {
          share({
            from: '2',
            url: `http://activity.alochatapp.com/static_html/2020/richMan/index.html?v=2`,
            title: `Tôi đang chơi Đại phú ông alochat, mau cùng tham gia nhận xu, khung ảnh, xe!`,
            desc: `Tôi đang chơi Đại phú ông alochat, mau cùng tham gia nhận xu, khung ảnh, xe!`,
            image: 'http://activity.alochatapp.com/static_html/2020/richMan/share.jpg?v=2'
          })
        } catch (e) { }
