      ...
      <transition name="slide">
        <div class="giftPup" :class="{mHeight:showGift.type == 'packet'}" v-if="showGiftPup">
          ...
          <p class="luckTips" v-if="showType== 2">
            Hôm nay thật may mắn<br />
            Nhận {{getGigtName(showGift)}}
          </p>
    ...
    <div class="mask" v-show="showTask">
      <transition name="slide">
        <div class="task" v-if="showTask">
          ...
          <div class="downTime">Thời gian kết thúc:{{surplusTime.minute}}:{{surplusTime.second}}</div>
          <div class="taskTitle" v-if="showGift.task == 'share'">Chia sẻ sự kiện tới bạn bè</div>
          <div class="taskTitle" v-else>Tặng quà xu 1 lần</div>
          <div class="giftBox">
            <div class="giftItem">
              <div class="imgBox">
                <img src="../assets/img/dice.png">
              </div>
              <div class="nick">Xúc xắc*1</div>
            </div>
          </div>
          <div class="btnBox">
            <span class="out" @click="closeTaskPup()">Bỏ qua nhiệm vụ</span>
            <span class="share" v-if="showGift.task == 'share'" @click="share()">Chia sẻ sự kiện tới bạn bè</span>
            <span class="share" v-else @click="goGift()">Tặng quà</span>
          </div>
        </div>
      </transition>
    </div>
  ...
  methods: {
    ...
    getGigtName(gift) {
      if (gift.type == 'gift') {
        ...
        return `${gift.name} khung ảnh *${gift.day}ngày`
      } else if (gift.type == 'car') {
        return `${gift.name} *${gift.day}ngày`
      } else if (gift.type == 'coin') {
        return `${gift.count} Xu`
