    ...
    mutations: {
        ...
        resetLandSteps(state, exceptionId) {
            for (let i = 0; i < state.land_info.length; i++) {
                ...
                const newObj = Object.assign({}, state.land_info[i], { empty: 1 });
        ...
        addRadish(state, newRadishCount) {
            if (state.owner_msg) {
                ...
                state.owner_msg = Object.assign({}, state.owner_msg, { radish });
            }
        },
        updateNormalSeed(state, num = -1) {
            if (state.owner_msg) {
                const free_seed = state.owner_msg.free_seed + num;
                state.owner_msg = Object.assign({}, state.owner_msg, { free_seed });
            }
        },
        updateCrazySeed(state, num = -1) {
            if (state.owner_msg) {
                const crazy_seed = state.owner_msg.crazy_seed + num;
                state.owner_msg = Object.assign({}, state.owner_msg, { crazy_seed });
            }
        },
    },
    actions: {
