...
//打开礼盒成功分享上报
function shareSuc() {
    return get(`/index.php?action=sweetHouse.give&signture=innerserver&uid=${uid}&token=${token}`)
}

function shareCb() {
    return get(`/index.php?action=sweetHouse.shareCb&signture=innerserver&uid=${uid}&token=${token}`)
}
