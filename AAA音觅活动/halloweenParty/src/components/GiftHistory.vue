...
export default {
  ...
  methods: {
    ...
    getTips(data) {
      let type = data.type
      if (type == 1) {
        return `完成${this.day_task_name[data.task_id]},獲得糖果*${data.nums}`
      } else if (type == 2) {
        return `完成${this.new_task_name[data.task_id]},獲得糖果*${data.nums}`
      } else if (type == 3) {
        return `兌換【小惡魔】 3天`
      } else if (type == 4) {
        return `送出糖果*${data.nums}`
      } else if (type == 5) {
        return `收到${data.nick}贈送的糖果*${data.nums}`
      }
    },
    getTime(tm, type) {
