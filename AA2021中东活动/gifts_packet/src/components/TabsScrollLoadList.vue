<template>
  <div class="rankGroups">
    <!-- 日榜、总榜切换主Tabs -->
    <div class="mainTabs">
      <a @click.prevent="mainTabClick(0)" :class="{current:mainTab==0}" href="">قائمة الحزمة الحمراء</a>
      <a @click.prevent="mainTabClick(1)" :class="{current:mainTab==1}" href="">وصف ميزة الحزمة الحمراء</a>
    ...
    <div v-if="mainTab==0" class="tabs">
      <a @click.prevent="tabClick(0)" :class="{current:tab==0}" href="">قائمة الأغنياء</a>
      <a @click.prevent="tabClick(1)" :class="{current:tab==1}" href="">القائمة المحظوظة</a>
    </div>
    <div class="downTimebox" v-if="mainTab == 0">
      <p v-if="actStatus == 0">العد التنازلي لبداية النشاط</p>
      <!-- 活动结束 -->
      <p v-else-if="actStatus == 1">العد التنازلي لنهاية النشاط</p>
      <p v-else> قد انتهى النشاط</p>
      ...
      <!-- <div class="noTime" v-else>该日排行榜已结束</div> -->
    </div>
    <div class="listBox" v-if="mainTab==0">
      <div class="titleBox">
        <span>المرتبة</span>
        <span>الاسم(UID) </span>
        <span v-if="tab == 0">عملات مرسلة</span>
        <span v-else>الحزمة الحمراء محصولة</span>
      </div>
      <ul class="list day">
        <li v-for="(item,index) in rank.list" :key="index" :class="'rank'+item.rank" @click="goUser(item.uid)">
          ...
          <div class="score">
            <i :class="{coins:tab == 0}"></i> {{item.score}}
          </div>
        </li>
        <!-- 日榜和总榜共用Loading（如果需要细化加载提示文案，可以把以下标签复制到不同的榜单后面） -->
        <div v-if="rank.loading" class="scrollLoading">جار التحميل...</div>
        <div v-if="rank.list.length==0" class="scrollNone">لا توجد المعطيات</div>
      </ul>
    </div>
