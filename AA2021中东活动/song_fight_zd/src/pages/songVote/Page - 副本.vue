<template>
  <div class="songVote">
    ...
    <div class="chioneChange">
      ...
      <p class="chioneTips">
        1、評委每天評分前30組歌每組歌曲將獲得2金豆<br />
        2、當期每連投8組投中，可隨機獲得一個音樂盒背包禮物（360金豆）/棉花糖背包禮物（1金幣）/2天VIP/赤鷹戰機座駕（2天）
      </p>
    </div>
    <loading />
  </div>
...
export default {
  ...
  methods: {
    ...
    chione(val) {
      if (this.howl[0].listenTime < 10 && this.howl[1].listenTime < 10) {
        this.toast(`每位選手聽夠10S 才能投票，2位選手都還未聽夠10S喔`)
      } else if (this.howl[0].listenTime < 10) {
        this.toast(`每位選手聽夠10S 才能投票，左邊的選手還未聽夠10S喔 `)
      } else if (this.howl[1].listenTime < 10) {
    ...
    sconedTime() {
      ...
      this.giftTimer = setInterval(() => {
        ...
        if (sing && lastSeek != seek) {
          this.howl[this.nowIndex].listenTime += 0.2
        }
      }, 200)
    },
