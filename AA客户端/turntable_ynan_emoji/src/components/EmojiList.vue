<template>
  <div class="emojiList" :class="{overTurn:!this.type}" :style="{maxHeight:!this.type?pupMaxHeight+'px':'none'}">
    ...
    <div class="getBtn" @click="showPup()"><i></i>{{listItem.pay_num}} mảnh đổi</div>
...
export default {
  ...
  data() {
    return {
      ...
      showGiftPup: false,
      lateImg: '',
      giftPupName: 'Chúc mừng bạn đổi Gói biểu cảm, dùng tại phòng Kara!',
      dataArr: [],
      loading: false,
      user_wallet: {},
      tastMsg: '',
    ...
    api.getGiftList(13).then(res => {
      ...
      api.getSvga(bid).then(res => {
        ...
        if (response_data.msg == 'ok') {
          ...
          this.tastMsg = 'Tải gói biểu cảm thất bại!'
          this.showT = true
        }
      })
    })
  ...
  methods: {
    ...
    showPup() {
      if (this.user_wallet.icon_puzzle >= this.listItem.pay_num) {
        ...
        this.tastMsg = 'Mảnh gói biểu cảm không đủ~ Hãy nhận thêm mảnh!'
        this.showT = true
      }
    },
    closePup() {
