    ...
    <div class="mask" :style="{height:mainHeight+'px'}" v-show="showGiftPup">
      <transition name="slide">
        <div class="giftPupCon" v-show="showGiftPup">
          ...
          <p>{{pupType==1?'Chúc mừng nhận':pupTitle}}</p>
          <div class="imgBox">
            <img :src="imgSrc" alt="">
            <em>{{giftPupName}}</em>
          </div>
          <p v-if="showVerTips && !Version" class="verTips">Phần thưởng này phải cập nhật bản mới nhất để dùng</p>
          <div class="querBtn" @click="closePup()">{{pupType==1?'Xác nhận':'Mở rương'}}</div>
        </div>
      </transition>
    </div>
...
export default {
  ...
  mounted() {
    ...
    this.player = new Player('#canvas') // #canvas is HTMLCanvasElement
      ...
      ; (async () => {
        const fileData = await downloader.get(`http://fstatic.cat1314.com/uc/svga/bdbc190e742cb484d4fa2b422eccabf3_1605011330.svga?${Math.random()}`)
  ...
  methods: {
    openBox() {
      if (!this.isDown) {
        this.tastMsg = 'Chưa tải hết tài nguyên, mời thử lại sau!'
        this.showT = true
        return
      }
    ...
    showClickGift(item) {
      ...
      this.pupType = 2
      this.pupTitle = `Lần ${item.must_get} chắc chắn trúng`
      this.showGiftPup = true  //禮物彈窗
    },
    // goCun() {
