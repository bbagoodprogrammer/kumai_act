  ...
  <div class="lackBox">
    <div class="boxCon">
      <div class="luckBox" :style="{'background-position-Y':imgPos[imgindex]}">
        <i class="luckTips" @click="showBoxGift()">Thưởng</i>
      </div>
      <div class="open" :class="{act:clueOpen}" @click="openBox()"></div>
    </div>
    <p class="tips">
      Nạp thêm {{userMsg.diff_charge}} xu nhận một chìa khoá miễn phí</br>
      <em>(Tính từ 20h ngày 28/4)</em>
    </p>
    <span class="goCun" @click="goCun()">Nạp</span>
    ...
    <div class="mask" v-show="showBoxGiftItem">
      <transition name="slide">
        <div class="boxGiftCon" v-show="showBoxGiftItem">
          <div class="title">Thưởng <span class="close" @click="closeGiftItem"></span></div>
          <div class="con">
            <p class="giftTips">Mở rương sẽ có cơ hội nhận một trong những quà như sau</p>
          </div>
          <div class="giftItem" :class="{flex:isloading||noGift,maxH:gift_list2.length>3}">
            <p v-if="isloading" class="loadingTips">Đang tải...</p>
            <p class="noGift" v-if="noGift">Chưa có quà nhé~</p>
  ...
  methods: {
    ...
    openBox() {
      ...
      if (this.userMsg.box_key > 0 && this.userMsg.lock_box > 0) {
        ...
        this.tastMsg = 'Không đủ rương báu nè, mau đi kiếm rương nào!'
        this.showT = true
      } else if (this.userMsg.box_key <= 0) {
        this.tastMsg = 'Không đủ chìa khoá không thể mở rương!'
        this.showT = true
      }
    },
    showBoxGift() {
