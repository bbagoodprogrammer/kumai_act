  ...
  <div class="chione">
    <div class="header">
      <div class="time">
        ...
        <strong v-if="voteData.status == 2">Vote udh berakhir</strong>
        <strong v-else-if="voteData.status == 1 && voteData.ttl != 0">Vote berakhir mundur hitung: {{surplusTime.minute}}:{{surplusTime.second}}</strong>
      </div>
      <span class="overVote" v-if="host == 1 && voteData.status == 1" @click="endVote()">Berakhir vote</span>
    </div>
    ...
    <ul>
      <li v-for="(item,index) in voteData.options " :key="index" @click="setIndex(index)">
        ...
        <div class="msg">
          ...
          <span class="ticket" v-if="voteData.results">{{voteData.results[index]}} tiket</span>
        </div>
      </li>
    </ul>
    <!-- </div> -->
    <span class="commitBtn" :class="{act:cIndex!=null && voteState==3}" v-if="voteData.status == 2 ||voteState!=4" @click="commitChione()">{{voteData.status == 2?'Udh berakhir':'Vote'}}</span>
    <Loading />
  </div>
</template>
<script>
