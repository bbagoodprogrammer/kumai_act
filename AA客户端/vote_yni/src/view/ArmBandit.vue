<template>
  <div class="box">
    ...
    <div v-if="!optionType">
      ...
      <div class="addOptions">
        <div class="add" @click="addOptions()">
          ...
          <span v-else>Tambahkan pilihan</span>
        </div>
        <p>Maksimum 20 pilihan, Maksimum 20 kata per pilihan</p>
      </div>
    </div>
    ...
    <h3>Pengaturan vote</h3>
    <div class="pelpleType">
      <span class="title">Yg siapa bisa vote</span>
      <span class="total" @click="setPeopleType(1)"><i :class="{act:peopleType == 1}"></i><strong>Semua</strong></span>
      <span class="other" @click="setPeopleType(2)"><i :class="{act: peopleType == 2}"></i><strong>User yg ada di antrian</strong></span>
    </div>
    <div class="setLv">
      <div class="lvBox">
        <span>Persyaratan Level</span>
        <div id="lv">
          <span v-if="choiceLv != ''">Level</span>
      ...
      <p>Pilih level berapa atau atas yg bisa vote (tidak berisi tiada batasan)</p>
    </div>
    <div class="setLv">
      <div class="lvBox">
        ...
        <div id="lv">
          ...
          <span v-if="choiceTime != ''">menit</span>
        </div>
      </div>
      <p>Pilih berapa menit utk berakhir (tidak berisi tiada batasan)</p>
    </div>
    <div class="commitBtn" :class="{act:couldCommit}" @click="commit()">Mulai</div>
...
export default {
  ...
  methods: {
    ...
    getp(index) {
      return `Pilihan ${index + 1}`
    },
    commit() {
      let option_type = !this.optionType ? 1 : 2
      // let options = 
      if (this.describe == '') {
        this.toast("Silakan berisi deskripsi vote!")
      } else if (this.options.length <= 1) {
        this.toast("Silakan berisi pilihan vote yg cukup!")
      } else {
        ...
        api.commitVote(this.describe, option_type, this.options, this.peopleType, this.choiceLv, this.choiceTime).then(res => {
          ...
          if (res.data.response_data) {
            this.toast("Berhasil memulai Vote")
            setTimeout(() => {
              this.closeWeb()
            }, 300)
          } else {
