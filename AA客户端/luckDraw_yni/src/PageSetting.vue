  ...
  <div id="app">
    <div class="page pageSetting" :style="{minHeight:viewHeight+'px'}">
      ...
      <div class="settIngCon">
        <div class="tipsBox">
          <h3 class="tipsTitle">Permainan undian</h3>
          <p class="tips" v-if="luckType">
            Anda memulai undian dan membayar sejumlah koin emas untuk membeli VIP atau hadiah K-room sebagai hadiah undian. Selama waktu hitung mundur, pengguna dalam kamar akan memberi hadiah kualifikasi undian yang Anda tentukan untuk mendapatkan kesempatan undian. Pengundian akan segera dibukasetelah hitung mundur berakhir, dan salah satu dari mereka akan dipilih sebagai pemang untuk mendapatkan
            <br />Jika tidak ada yang berpartisipasi, hadiah akan milik pemulai undian.
          </p>
          <p class="tips" v-else>
            1. Anda dapat memilih hadiah undian berikut dan membayar koin emas yang sesuai untuk memulai undian<br />
            2. Selama waktu hitung mundur, pengguna dalam kamar tersebut sawer hadiah kualifikasi undian yang ditentukan akan bisa mendapatkan kesempatan undian. Pengundian akan segera dibuka setelah penghitungan mundur berakhir, dan salah satu peserta akan dipilih sebagai pemenang dan mendapatkan hadiah Anda.<br />
            3. Jika tidak ada yang berpartisipasi, hadiah akan milik pemulai undian.
          </p>
        </div>
        <div class="from" v-if="luckType">
          <div class="fromItem">
            <span>Kualifikasi undian</span>
            <div class="itemCon" @click="changGift()">
              ...
              <span v-else class="noSet">Blm atur</span>
              <i class="arr"></i>
            </div>
          </div>
          <div class="fromItem">
            <span>Mundur hitung</span>
            <div class="itemCon" @click="setTime()">
              <span v-if="timeIndex != null">{{timeArr[timeIndex]}}menit</span>
              <span v-else class="noSet">Blm atur</span>
              <i class="arr"></i>
            </div>
          </div>
          <div class="fromItem">
            <span>Pilihan hadiah</span>
            <!-- <div class="itemCon"> -->
          ...
          <div class="setpeopleType">
            <span>Yg siapa bisa ikut</span>
            <div class="peopleType">
              <span @click="setPeopleType(1)"><i :class="{act:peopleType == 1}"></i><strong>Semua</strong></span>
              <span @click="setPeopleType(2)"><i :class="{act:peopleType == 2}"></i><strong>User yg ada di antrian</strong></span>
            </div>
          </div>
        </div>
        <div class="setGift" v-else>
          <h3 class="tipsTitle">Silakan pilih hadiah undian</h3>
          <div class="fiexList">
            <ul>
            ...
            <div class="sGift">
              <span> Cara ikut:</span>kirim <img :src="fixeActDraw.ticket_gpic" alt="">x{{fixeActDraw.ticket_amount}} ({{fixeActDraw.ticket_price}}{{giftType[fixeActDraw.ticket_currency]}}) utk ikut undian
            </div>
            <div class="fTime"><span>Mundur hitung:</span> {{fixeActDraw.time_limit}}menit</div>
          </div>
          <div class="setpeopleType">
            <span>Yg siapa bisa ikut</span>
            <div class="peopleType">
        ...
        <div class="commitBtn" :class="{act:isCommit || (!luckType &&peopleType)}" @click="showPricePup()">Mulai undian</div>
      </div>
      ...
      <van-popup v-model="show" position="bottom" :round="true" :style="{background:'none'}" overlay-class="bgopt">
        <div class="giftList">
          ...
          <p v-if="tabLoading" class="tabLoading">Sedang muat...</p>
          ...
          <div class="queryBtn" @click="queryGift()">Ya</div>
        </div>
      </van-popup>
      ...
      <van-popup v-model="showSgiftPup" position="bottom" :round="true" :style="{background:'none'}" overlay-class="bgopt">
        <div class="giftList">
          ...
          <p v-if="tabGift[tabIndex] && tabGift[tabIndex].loading" class="tabLoading">Sedang muat...</p>
          <div class="vipCon" v-show="tabIndex == 1">
            <p class="vipTips">Pilih jumlah bulan VIP (Jika udh menjadi VIP, dptkn hadiahnya maka waktunya ditumpukkan)</p>
            <div class="vipBox">
              <div class="vipItem">
                <div class="item" :class="{act:vipType == index}" @click="vipClick(index,item)" v-for="(item,index) in nowTabGift[0]" :key="index">
                  <span class="tm">{{item.prise_days}}hari</span>
          ...
          <div class="queryBtn" @click="querySgift()">Ya</div>
        </div>
      </van-popup>
      ...
      <div class="mask" v-if="showCommitPup">
        <transition name="slise">
          <div class="commitQueryPup" v-show="showCommitPup">
            ...
            <p class="msg">Konfirmasi pembayaran {{price}} utk membeli hadiah dan memulai undian?
            </p>
            <div class="btnBox">
              <span class="cancel" @click="closeCommitPup()">Batal</span>
              <span class="query" @click="commitLuck()">Ya</span>
            </div>
          </div>
        </transition>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      showSgiftPup: false,
      tabIndex: 1,
      tabArr: ["VIP", "Hadiah", "Mount", "Bingkai kursi", null, " Kartu nama", "Tema"],
      tabGift: {},   //tab對應的禮物
      balance: {},
      tabLoading: false,
      vipType: 0,
  ...
  methods: {
    ...
    queryGift() {
      if (this.actGiftT.id) {
        ...
        toast(`Tolong pilih kualifikasi undian!`)
      }
    },
    querySgift() {
