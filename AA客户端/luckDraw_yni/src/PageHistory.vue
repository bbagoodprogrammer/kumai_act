<template>
  <div class="page pageHistory">
    <div class="title"><i class="back" @click="$router.go(-1)"></i> Catatan undian</div>
      ...
      <template slot-scope="{list, loading, none}">
        <ul>
          <li v-for="(item,index) in list" :key="index" @click="goDetail(item)">
            <div class="name"><strong class="nick">{{item.creator_nick}}</strong> memulai undian <strong>{{item.prise_str}}</strong></div>
            <div class="time">{{item.end_date}} Buka undian</div>
            <i class="arr"></i>
          </li>
        </ul>
        <p class="lastTips">Cuma tampilkan vote yg dimulai dr 3 hari akhir-akhir ini</p>
