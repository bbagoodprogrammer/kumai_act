    ...
    <div class="page pageSetting" :style="{minHeight:viewHeight+'px'}">
      ...
      <van-popup v-model="show" position="bottom" :round="true" :style="{background:'none'}" overlay-class="bgopt">
        <div class="giftList">
          ...
          <p v-if="tabLoading" class="tabLoading">Đang tải...</p>
      ...
      <div class="mask" v-if="showCommitPup">
        <transition name="slise">
          <div class="commitQueryPup" v-show="showCommitPup">
            <div class="title">Nhắc nhở</div>
            <p class="msg">Xác nhận dùng {{price}} mua phần thưởng <br />
              và mở rút thưởng?
            </p>
            <div class="btnBox">
              <span class="cancel" @click="closeCommitPup()">Huỷ</span>
              <span class="query" @click="commitLuck()">Xác nhận</span>
            </div>
          </div>
        </transition>
      </div>
