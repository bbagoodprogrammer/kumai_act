<template>
  <div class="page pageHistoryDetail">
    ...
    <div class="sGift">
      <span class="gift">Cách rút thưởng: <img :src="actItem.ticket_gpic" alt=""> x1</span>
      <span>Người mở: {{actItem.creator_nick}}</span>
    </div>
    <div class="list">
      <div class="total">Người dự: {{total?total:0}}</div>
      <ul class="scrollable">
        <li v-for="(item,index) in list " :key="index">
          <img :src="item.avatar" alt="">
          <div class="msg">
            <div class="nick">{{item.nick}}</div>
            <div class="tips">Tặng {{item.ticket_count}} quà, nhận {{item.ticket_count}} lần rút thưởng</div>
          </div>
        </li>
      </ul>
    </div>
