<template>
  <div class="box">
    ...
    <div class="describeBox">
      <textarea name="" v-model="describe" id="describe" maxlength="300" placeholder="أدخل وصفًا لمساعدة المستخدمين في الغرفة على فهم الغرض من التصويت"></textarea>
      <span class="describeNum">{{describe.length}}/300</span>
    </div>
    <h3 class="optionTitle">خيارات التصويت <u class="optionType" @click="setOptionType()">{{optionType?'خيارات مخصصة':'مستخدم على المايك'}}</u></h3>
    <div v-if="!optionType">
      ...
      <div class="addOptions">
        <div class="add" @click="addOptions()">
          ...
          <span v-else>إضافة خيار</span>
        </div>
        <p>ما يصل إلى 20 خيارًا ، كل خيار يصل إلى 20 كلمة</p>
      </div>
    </div>
    ...
    <h3>إعدادات التصويت</h3>
    <div class="pelpleType">
      <span class="title">من يمكنه التصويت</span>
      <span class="total" @click="setPeopleType(1)"><i :class="{act:peopleType == 1}"></i><strong>كل واحد</strong></span>
      <span class="other" @click="setPeopleType(2)"><i :class="{act: peopleType == 2}"></i><strong>مستخدم على المايك</strong></span>
    </div>
    <div class="setLv">
      <div class="lvBox">
        <span>متطلبات المستوى</span>
        <div id="lv">
          ...
          <span v-if="choiceLv != ''">مستوى</span>
        </div>
      </div>
      <p>اختر عدد المستويات أو أعلى يمكنه التصويت (لم يتم ملء أي حد)</p>
    </div>
    <div class="setLv">
      <div class="lvBox">
        <span>وقت التصويت</span>
        <div id="lv">
          ...
          <span v-if="choiceTime != ''">دقيقة</span>
        </div>
      </div>
      <p>اختر عدد الدقائق التي ستنتهي بها (بلا حدود إذا لم يتم ملؤها)</p>
    </div>
    <div class="commitBtn" :class="{act:couldCommit}" @click="commit()">إبدأ </div>
...
export default {
  ...
  methods: {
    ...
    commit() {
      ...
      // let options = 
      if (this.describe == '') {
        this.toast("يرجى ملء وصف التصويت!")
      } else if (this.options.length <= 1) {
        this.toast("يرجى ملء خيارات التصويت الكافية!")
      } else {
        ...
        api.commitVote(this.describe, option_type, this.options, this.peopleType, this.choiceLv, this.choiceTime).then(res => {
          ...
          if (res.data.response_data) {
            this.toast("تم بدء التصويت بنجاح")
            setTimeout(() => {
              this.closeWeb()
            }, 300)
          } else {
