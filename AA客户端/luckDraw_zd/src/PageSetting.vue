  ...
  <div id="app">
    <div class="page pageSetting" :style="{minHeight:viewHeight+'px'}">
      ...
      <div class="settIngCon">
        ...
        <div class="from" v-if="luckType">
          ...
          <div class="fromItem">
            ...
            <div class="itemCon" @click="setTime()">
              <span v-if="timeIndex != null">{{timeArr[timeIndex]}}دقيقة</span>
              <span v-else class="noSet">غير مضبوط</span>
              <i class="arr"></i>
            </div>
          </div>
          ...
          <div class="setpeopleType">
            <span>من يمكنه المشاركة</span>
            <div class="peopleType">
              <span @click="setPeopleType(1)"><i :class="{act:peopleType == 1}"></i><strong>كل واحد</strong></span>
              <span @click="setPeopleType(2)"><i :class="{act:peopleType == 2}"></i><strong>مستخدم على الميكروفون</strong></span>
            </div>
          </div>
        </div>
        <div class="setGift" v-else>
          <h3 class="tipsTitle">اختر الجوائز</h3>
          <div class="fiexList">
            <ul>
        ...
        <div class="commitBtn" :class="{act:isCommit || (!luckType &&peopleType)}" @click="showPricePup()">ابدا اليانصيب</div>
      </div>
      ...
      <van-popup v-model="show" position="bottom" :round="true" :style="{background:'none'}" overlay-class="bgopt">
        <div class="giftList">
          ...
          <p v-if="tabLoading" class="tabLoading">جار التحميل...</p>
          ...
          <div class="queryBtn" @click="queryGift()">تحديد</div>
        </div>
      </van-popup>
      ...
      <van-popup v-model="showSgiftPup" position="bottom" :round="true" :style="{background:'none'}" overlay-class="bgopt">
        <div class="giftList">
          ...
          <p v-if="tabGift[tabIndex] && tabGift[tabIndex].loading" class="tabLoading">جار التحميل...</p>
          <div class="vipCon" v-show="tabIndex == 1">
            <p class="vipTips">حدد عدد أشهر VIP (بالنسبة للأعضاء الذين فازوا بجوائز بالفعل ، سيتم إضافة الوقت)</p>
            <div class="vipBox">
              <div class="vipItem">
                <div class="item" :class="{act:vipType == index}" @click="vipClick(index,item)" v-for="(item,index) in nowTabGift[0]" :key="index">
                  <span class="tm">{{item.prise_days}}يوم</span>
      ...
      <van-popup v-model="showTimePup" position="bottom" :round="true">
        <ul class="timeList">
          <li v-for="(item,index) in timeArr" :key="index" :class="{act:timeIndex == index}" @click="timeQuery(index)">{{item}} دقيقة</li>
        </ul>
      </van-popup>
      <div class="mask" v-if="showCommitPup">
        <transition name="slise">
          <div class="commitQueryPup" v-show="showCommitPup">
            <div class="title">تذكير</div>
            <p class="msg"> هل تريد تأكيد دفع {{price}} لشراء الجوائز وبدء يانصيب؟</p>
            <div class="btnBox">
              <span class="cancel" @click="closeCommitPup()">إلغاء</span>
              <span class="query" @click="commitLuck()">تحديد</span>
            </div>
          </div>
        </transition>
      </div>
