<template>
  <div class="page pageHistory">
    <div class="title"><i class="back" @click="$router.go(-1)"></i> سجل اليانصيب</div>
      ...
      <template slot-scope="{list, loading, none}">
        <ul>
          <li v-for="(item,index) in list" :key="index" @click="goDetail(item)">
            <div class="name">
              <strong class="nick">{{item.creator_nick}}</strong> بدأ <strong>{{item.prise_str}}</strong> اليانصيب
            </div>
            <div class="time">{{item.end_date}}فتح الجوائز</div>
            <i class="arr"></i>
          </li>
        </ul>
        <p class="lastTips"> عرض التصويت التي بدأت في آخر 3 أيام فقط</p>
