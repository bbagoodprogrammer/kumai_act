<template>
  <div class="page pageMain">
    ...
    <div class="turnBox">
      ...
      <div class="luckMsg">
        <div class="noLuck" v-if="!luckMsg || (luckMsg&&!luckMsg.id)">لا يوجد اليانصيب حاليا</div>
        <div class="luckIng" v-else>
          <div class="msg">
            <span v-if="luckIng">رسم اليانصيب...</span>
            <span v-else-if="luckMsg.status == 1">العد التنازلي <em>{{surplusTime.minute}}:{{surplusTime.second}}</em></span>
            <span v-else class="nick">تهانينا:<em>{{luckMsg.winner_nick}}</em> </span>
          </div>
          <div class="gift">جائزة:{{luckMsg.prise_str}}</div>
        </div>
      </div>
      <div class="luckBtn" :class="{luckIng:luckStatus == 3,luck:luckStatus==2,again:luckStatus == 1}" @click="goSetting()"></div>
      <div class="sGiftTips" v-if="luckMsg&&luckMsg.status == 1">أرسل <img :src="luckMsg.ticket_gpic" alt="">x1 للدخول في السحب</div>
