<template>
  <div class="lackBox">
    ...
    <p class="tips">
      استمر في الشحن {{userMsg.diff_charge}} عملة ذهبية لحصول على مفتاح</br>
      <em>(محسوبة من 0:00 يوم 22 فبراير)</em>
    </p>
    <Tabs />
    ...
    <div class="mask" :style="{height:mainHeight+'px'}" v-show="showGiftPup">
      <transition name="slide">
        <div class="giftPupCon" v-show="showGiftPup">
          ...
          <p>{{pupType==1?'مبروك على الحصول على':pupTitle}}</p>
          <div class="imgBox">
            <img :src="imgSrc" alt="">
            <em>{{giftPupName}}</em>
          </div>
...
export default {
  ...
  methods: {
    openBox() {
      if (!this.isDown) {
        this.tastMsg = 'لم يتم تحميل المورد ، يرجى المحاولة مرة أخرى في وقت لاحق!'
        this.showT = true
        return
      }
      ...
      if (this.userMsg.box_key > 0 && this.userMsg.lock_box > 0) {
        ...
        this.tastMsg = 'صندوق الكنز الخاص بك غير كاف ، اذهب واحصل على صندوق الكنز!'
        this.showT = true
      } else if (this.userMsg.box_key <= 0) {
        this.tastMsg = 'ليس لديك مفاتيح كافية لفتح صندوق الكنز!'
        this.showT = true
      }
    },
    showBoxGift() {
    ...
    showClickGift(item) {
      ...
      this.pupType = 2
      this.pupTitle = ` يجب الفوز بـ ${item.must_get}`
      this.showGiftPup = true  //禮物彈窗
    },
    // goCun() {
