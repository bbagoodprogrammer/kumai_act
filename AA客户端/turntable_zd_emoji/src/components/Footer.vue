    ...
    <div class="tips1" :style="{height:pupHeight+'px'}" v-show="showTips">
      <transition name="slide">
        <div class="tipsCon" v-show="showTips">
          <div class="title"> قواعد اللعبة<i class="close" @click="closeTips1()"></i></div>
          <div class="con">
            <p>1.مقابل كل 100 قطعة نقدية ذهبية يتم إنفاقها في المجموع ، يمكن تحويل السحب</p>
            <p>2.يمكنك الحصول على مفتاح مفاجأة لكل قيمة متراكمة من 300 قطعة ذهبية.يمكن استخدام مفتاح المفاجأة لفتح صندوق الكنز المفاجئ</p>
            <p>3.يمكن استبدال شظايا الهدايا بهدايا نادرة في صندوق الهدايا ، والهدايا النادرة هي هدايا غرفة الغناء</p>
            <p>4.يمكن استخدام أجزاء الملصقات لاسترداد مجموعة الملصقات بالكامل. فترة صلاحية الملصقات التي تم استرداد قيمتها دائمة. يمكن لكل لاعب استرداد كل نوع من الملصقات مرة واحدة فقط. إذا كان يمتلك بالفعل الملصقات المقابلة ، فلا يمكنه استرداد قيمتها</p>
            <p>5.للمرة الثالثة والثامنة من صندوق الكنز المحظوظ ، سيتم ربح العناصر النادرة والعملات الذهبية عالية القيمة</p>
            <p>6.سيتم استبدال العناصر النادرة في بداية كل شهر ، وستتم إعادة تعيين التقدم</p>
            <p>7.يمكن تجميع عناصر انتهاء الصلاحية وتكديسها</p>
          </div>
        </div>
      </transition>
    </div>
    <div class="historyList" :style="{height:pupHeight+'px'}" v-show="showHistory">
      <transition name="slide">
        <div class="historyCon" v-show="showHistory">
          <div class="title">سجل اليانصيب<i class="close" @click="closeHistory()"></i></div>
          <div class="list">
            <p v-if="loading" class="loadingTips">جار التحميل...</p>
            <p class="noDateTips" v-if="record.length==0 && !loading">لا توجد معلومات اليانصيب!</p>
            <ul class="scrollable">
              <li v-for="(item,index) in record" :key="index">
                ...
                <div class="gift">
                  ...
                  <em>x99</em> -->
                  اسحب <em>{{item.name}}</em> من <em>{{item.status==0?'عجلة الحظ':'صندوق كنز مفاجئ'}}</em>
                </div>
              </li>
            </ul>
          </div>
