<template>
  <div class="starEffects">
    ...
    <span>
      <img src="../assets/img/bubble.png" alt=""> </span>
    <span>
      <img src="../assets/img/bubble.png" alt=""> </span>
    <span>
      <img src="../assets/img/bubble.png" alt=""> </span>
...
const fireTick = 1000; //每秒发射一个新的星星
const moveTick = 1; //每帧移动的距离（单位：像素）
...
export default {
  ...
  methods: {
    fire() {
      ...
      const initBottom = 0;
      const initDirection = getRand(-.1, .1); //决定左右移动速度（向上移动是每帧moveTick像素）
      node.left = initLeft;
    ...
    update() {
      for (let i = 0; i < this.nodes.length; i++) {
        ...
        if (node.active) {
          ...
          node.style.transform = `scale(${node.scale}) `;
