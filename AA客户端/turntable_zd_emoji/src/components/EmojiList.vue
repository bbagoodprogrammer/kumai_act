<template>
  <div class="emojiList" :class="{overTurn:!this.type}" :style="{maxHeight:!this.type?pupMaxHeight+'px':'none'}">
    ...
    <div class="list">
      <p v-if="loading" class="loading">جار التحميل...</p>
      <div class="emojiItem">
    ...
    <div class="getBtn" @click="showPup()"><i></i> تحويل جزء {{listItem.pay_num}}</div>
...
export default {
  ...
  data() {
    return {
      ...
      showGiftPup: false,
      lateImg: '',
      giftPupName: 'تهانينا على نجاحك في الحصول على الملصق ، يمكنك استخدامه في غرفة الغناء!',
      dataArr: [],
      loading: false,
      user_wallet: {},
      tastMsg: '',
  ...
  methods: {
    ...
    showPup() {
      if (Number(this.user_wallet.icon_puzzle) >= Number(this.listItem.pay_num)) {
        ...
        this.tastMsg = 'ليس لديك أجزاء كافية من الملصقات ~ اذهب واحصل على الأجزاء!！'
        this.showT = true
      }
    },
    closePup() {
