...
function getDefault(t) {
    return axios.get(`/big_turntable/init.php?token=${token}&status=${t}`);
}
//抽奖
function luckDraw(t, count) {
    return axios.get(`/big_turntable/go.php?token=${token}&status=${t}&count=${count}`);
}
//榜单
function getRank(from) {
    return axios.get(`/big_turntable/rank.php?token=${token}&from=${from}`)
}
